	...
	while (1)
	{
		...
		if ((cdr.become_worse || ((train.phase_offset_data) >= cdr.max_phase_os)) 
			...
			if( cdr.direction == 1)
				...
				train.phase_offset_data -= tag_CDR_PHASE_STEP_SIZE; 			
            train.phase_offset_data = clamp(train.phase_offset_data, cdr.max_phase_os, cdr.min_phase_os);
 			if(skip_dfe_adapt_cdr_midpoint)
				phase_control_func();
			//delay(T_3us); //test
			//check_cdr_status();
			...
			/*tag_THRES_K_BASE=16*/
			thres_small = ((cdr.opt_f0p*tag_CDR_MIDPOINT_SMALL_THRES_K)>>4) + tag_CDR_MIDPOINT_SMALL_THRES_C; //8 0
			thres_large = ((cdr.opt_f0p*tag_CDR_MIDPOINT_LARGE_THRES_K)>>4) + tag_CDR_MIDPOINT_LARGE_THRES_C; //9 2
			cdr.become_worse = 
