...
void EOM_Clock_Alignment(void)
{
	...
	do {	
		...
		if( lnx_EOM_PHASE_UI_ALIGN_FAILED_LANE == _EXIT_SUCCESS ) {
			...
			//DBG_Printf(" eom_en_sde(%d,%d,%d)",EOM_EN_S,EOM_EN_D,EOM_EN_E);
				...
				if( norm_ofst > train.ofst )
					...
					eom_vote_acc -= 1;
				if(abs(norm_ofst-train.ofst)<=1)  eom_data_align_success=1;
				...
				if( norm_ofst > train_ofst )
					...
					eom_vote_acc -= 1;
				if(abs(norm_ofst-train_ofst)<=1)  eom_data_align_success=1;
				else eom_data_align_success=0;
#endif
			//if(abs(norm_ofst-train.ofst)<=1)  eom_data_align_success=1;
			//else eom_data_align_success=0;
		...
		if( cds.f1 > tag_DFE_F1_SAT_THRES || cds.f1 < -tag_DFE_F1_SAT_THRES || train.f0a >= tag_DFE_F0_SAT_THRES || ofst_sat ) {
			// protect saturation
...
void Data_EOM_Align90_Adapt_EyeWidthMode(void)
{
	...
	do 	{	
		...
		if( (lnx_EOM_PHASE_UI_ALIGN_FAILED_LANE == _EXIT_SUCCESS) && eom_data_align_success) 
		{
...
void EOM_clock_analog_align(void)
{
	...
	data_slic_fb_mode = 0; //set_mmse_fb_mode();
	#endif
