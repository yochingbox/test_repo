...
void sampler_sel(uint8_t sw)
{
	...
	SAMPLER_CAL_SAVE = (uint8_t*)&CAL_SAVE_DATA11_LANE; SAMPLER_CAL_SAVE+=sw;
...
void sampler_cal() {
	...
	//reg_ANA_RX_DFE_F1_POL_EN_D_FORCE_LANE = 1;
	//reg_ANA_RX_DFE_F1_POL_EN_S_FORCE_LANE = 1;
	//reg_ANA_RX_DATA_SLICER_PATH_SWITCH_E_FORCE_LANE = 1;
	//reg_ANA_RX_DATA_SLICER_PATH_SWITCH_O_FORCE_LANE = 1;
	...
	for(i=0; i<10; i++)	{
		...
		while (j <= 3) {
			...
			*NEG = offset;	
			...
			out_of_bound = 0;
			delay(T_2us); // 2us, 4us, 8us and 16us programmable
	...
	for(i=0; i<10; i++) {
		...
		for (ntimes=0; ntimes < n_programmable; ntimes++) {
			for( offset=-ofst_bnd; offset<=ofst_bnd; offset++ ) { //80% sweep
				set_sampler(offset);
				delay(T_p5us); // 250ns, 500ns, 1us and 2us programmable
				if(VOFF_POS == 1) break; 
			}
		...
		//average offset
		if( lnx_SAMPLER_CAL_AVG_MODE_LANE_1_0 == 0)		
			offset = (offset_sum_neg + offset_sum_pos) >> 1;
		else if( lnx_SAMPLER_CAL_AVG_MODE_LANE_1_0 == 1) offset = offset_sum_pos;
	...
	SMPLR_CAL_EN = 0;	reg_SMPLR_CAL_EN_DLY_LANE = 0;
	...
	//reg_ANA_RX_DFE_F1_POL_EN_D_FORCE_LANE = 0;
	//reg_ANA_RX_DFE_F1_POL_EN_S_FORCE_LANE = 0;
	//reg_ANA_RX_DATA_SLICER_PATH_SWITCH_E_FORCE_LANE = 0;
	//reg_ANA_RX_DATA_SLICER_PATH_SWITCH_O_FORCE_LANE = 0;
