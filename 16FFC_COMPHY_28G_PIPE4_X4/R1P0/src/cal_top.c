...
void Calibration(void) {
	...
	for(genno=min_gen; genno<= max_gen; genno++) {  
		if(speedtable[genno][spdoft_tx_ck_sel_lane]==1) ring_pll_enabled = 1;	
		else 	lc_pll_used = 1;
	}

	if( force_exit_cal ) {
		CONTROL_CONFIG1.VAL = 0x00000000; //all *cal_en=0
		cmx_TXDCC_PDIV_CAL_EN = 0;
		all_cal_ext();		                                     
		pll_rate	= speedtable[gen_tx][spdoft_pll_rate_sel_tx]; 
	...
	//enable TSEN
		...
		if(force_exit_cal) {
			if(use_ring_pll) rate = !pll_rate;
			else rate = pll_rate;
		}	
		else if( tx_status_pcie_mode || lc_pll_used==0) rate = 0; //select 12.5G to avoid crosstalk
		else rate = 0;		
		...
		//reg_MCU_DEBUG6_LANE_7_0 = rate + 0x70;			
			...
			reg_RESET_ANA = 0;
			if(cmx_PLL_CAL_EN) pll_cal();				
			if( cmx_PLLDCC_CAL_EN ) plldcc_cal();		
			
			
			if(no_pllspdchg || force_exit_cal ) break; //SERDAS always use LC PLL rate0
			if( tx_status_pcie_mode || lc_pll_used==0) break;
		}
		...
		if( ring_pll_enabled) {
			...
			for(; rate<2; rate++) {
				...
				if( cmx_RING_PLL_CAL_EN ) ring_pll_cal();				
	...
	for( genno=min_gen; genno<=max_gen; genno++)  
	{
		...
		//if(cmx_EOM_ALIGN_CAL_EN) eom_align_cal();	removed		
...
skip:	
	...
	//tsen_dat_cal = read_tsen();   //no need
...
void Cal_Cont(void) {
	...
	case 5: if(mcuid==master_mcu && cmx_PLLDCC_CAL_EN && cmx_PLLDCC_CAL_CONT_EN) plldcc_cal();	break;
	}
	dcc_cont++;
	if(dcc_cont>5) dcc_cont = 0;	
		...
		case 4:						
				if( timeout == 0 ) break;
	if( mcuid==master_mcu && cmx_PLL_TEMP_CAL_EN && timeout) {
		pll_temp_cal(); 
		if(BYPASS_DELAY==0) {
		...
		case 5:		
				...
				if( mcuid==master_mcu && cmx_PLLAMP_CAL_CONT_EN ) pll_amp_cal_cont();	
		break;		
		case 6:
				if((mcuid==master_mcu && reg_ANA_PLL_LOCK_RING_RD && cmx_RING_PLL_CAL_EN)) ring_pll_cont(); 
...
int16_t read_tsen(void) {
	...
	#ifndef SIM_ENV
	if(reg_PWRON_SEQ &&  reg_MCU_COMMAND0_LANE_31_0_b0==0) {
