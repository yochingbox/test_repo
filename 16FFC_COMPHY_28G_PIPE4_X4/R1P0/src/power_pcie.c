...
void Power_P1_P2( void ) {
	...
	if( reg_PU_PLL_OR==0 ) {
		...
		while(reg_ANA_PU_PLL_RING);
	}
	
	reg_ANA_PU_TX_LANE = 0;	
	
	if(reg_PIN_REFCLK_DIS_RD && reg_P2_STATE_LANE==0) {
		if(cmx_BYPASS_MCU_STOP==1 && mcuid==master_mcu) {
