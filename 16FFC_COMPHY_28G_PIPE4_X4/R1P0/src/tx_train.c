...
void tx_train(void)
{
	...
	while (1)
	{
		if( reg_EYE_OPEN_EN_RD_LANE ) {
			if(lnx_BYPASS_RXTRAIN_LANE==0)	rx_train();
			Send_ctrl_to_tx(0);
...
void tx_train_PCIe(void) {
	...
	for(i=0; i<tag_PCIE_GN1_STEP_NUM; i++) {
		if(train.fn1 > tag_PCIE_GN1_FN1_THRE ) 	{
	...
	reg_MCU_DEBUGC_LANE_7_0 = (uint8_t)train.fn1; //test
}
