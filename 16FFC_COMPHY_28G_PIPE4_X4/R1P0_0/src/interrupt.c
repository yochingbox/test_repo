...
{
    PHY_INT_LANE = 13;
	if(reg_INT_REFCLK_DIS_CHG_ISR_LANE == 1)
...
void int3_isr (void) __interrupt (10)  __using (2) //refclk_dis
{
	...
	if( reg_INT_PU_IVREF_CHG_ISR_LANE ) {
		...
		if(reg_PIN_PU_IVREF_RD==0 && reg_PIN_PU_PLL_RD_LANE==0 && mcuid==MCU_LANE0 ) {
...
void int1_isr (void) __interrupt (2)  __using (2) //pcie
{
	...
	if( reg_INT_PU_PLL_OR_CHG_ISR_LANE && mcuid==MCU_LANE0) 
...
void int7_isr (void) __interrupt (8)  __using (2) // remote ctrl valid
{
   ...
   if( reg_REMOTE_CTRL_VALID_ISR_LANE == 1 ) {
		...
		if(reg_PIN_TX_TRAIN_ENABLE_MUX_RD_LANE==0) { 
			...
			else  if( (reg_REMOTE_CTRL_FIELD_RD_LANE_15_0_b1&0x0c)==0x8 && do_tx_apta==1) {
				do_tx_apta = 0; 
				reg_LOCAL_STATUS_FIELD_LANE_15_0 = 0;
				reg_LOCAL_STATUS_FIELD_VALID_LANE = 1; 
				reg_LOCAL_STATUS_FIELD_VALID_LANE = 0;
		...
		if( (do_train && reg_PIN_TX_TRAIN_ENABLE_MUX_RD_LANE==1) || (do_tx_apta && reg_PIN_TX_TRAIN_ENABLE_MUX_RD_LANE==0)) {
			...
			reg_MCU_DEBUG4_LANE_7_0 = reg_REMOTE_CTRL_FIELD_LANE_15_0_b1;
			reg_MCU_DEBUG5_LANE_7_0 = reg_REMOTE_CTRL_FIELD_LANE_15_0_b0;
		...
		//============================
ext_int:
		if(error_code>0) { //send error code  Don't open! cause recersive int.
			//reg_LOCAL_CTRL_FIELD_LANE_15_0 = (rm_ctrl.v&0x1fff)|0xe000; 	 
			reg_LOCAL_CTRL_FIELD_LANE_15_0_b1 |= 0xe0;
			//reg_LOCAL_CTRL_FIELD_VALID_LANE = 1; 
			//reg_LOCAL_CTRL_FIELD_VALID_LANE = 0;
			rm_stat = error_code;
		}
			...
			else reg_LOCAL_STATUS_FIELD_LANE_15_0 = rm_stat;
...
/*
void int6_isr (void) __interrupt (13)  __using (2) //train timer out, frame unlock
{
   EA = 0;
	
    PHY_INT_LANE = 6;
	
	if(reg_INT_TIMER1_ISR_LANE) { 
		reg_INT_TIMER1_ISR_LANE = 0;
		if( reg_PIN_TX_TRAIN_ENABLE_MUX_RD_LANE==0 && do_train==1 ) { //txtrain abort
			PHY_NS = ST_TRXTRAIN; 
			mcu_reset();
		}
	}
	
	if(reg_PIN_RX_TRAIN_ENABLE_RD_LANE==0 && do_rxtrain==1){ //abort
		 //PHY_NS = ST_TRXTRAIN; //todo
