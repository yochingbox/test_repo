...
void pll_fast_cal(void) {
	...
	TEMPC_MUX_HOLD_SEL = cmx_CAL_TEMPC_MUX_HOLD_SEL_7_0;
...
#if 0
		for(i=0; i<6; i++) {
			temp_level_todig = TEMPC_LEVEL_TODIG;
			delay(T_p5us);
			if (temp_level_todig == TEMPC_LEVEL_TODIG)
				current_level_todig = temp_level_todig;
			else
				continue;

			if (pre_level_todig != current_level_todig)
				pre_level_todig = current_level_todig;
			else{
				if( pre_level_todig == 0x2 && tempc_mux_hold_sel_2c < 11 ) {
					tempc_mux_hold_sel_2c++;
					TEMPC_MUX_HOLD_SEL = tempc_mux_hold_sel_2c ^ (tempc_mux_hold_sel_2c>>1);
				}
				else if( pre_level_todig == 0x1 && tempc_mux_hold_sel_2c > 0){
					tempc_mux_hold_sel_2c--;
					TEMPC_MUX_HOLD_SEL = tempc_mux_hold_sel_2c ^ (tempc_mux_hold_sel_2c>>1);
				}
				pre_level_todig = 0;
