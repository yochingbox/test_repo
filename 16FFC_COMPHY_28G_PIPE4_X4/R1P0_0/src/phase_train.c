...
void phase_control_func_0(int8_t ph_dat, int8_t ph_esm) 
{
	...
	if(ph_control_mode==0 ) {
	    ...
	    cdr_phase_update(cal_phase - ph_dat - ph_esm );
		#else
	    cdr_phase_update(cal_phase + ph_dat + ph_esm );
		#endif
		...
		#if (tag_CDR_ALIGN90_INC_IS_CLOCK_DELAY==1)	
	    cdr_phase_update(cal_phase - ph_dat );
		#else
	    cdr_phase_update(cal_phase + ph_dat );
		#endif
		...
		move_eom_phase((int16_t)(cal_eom_dpher + ph_esm) - (int16_t)reg_EOM_DPHER_LANE_6_0);
		#else
		//eom_phase_update( cal_eom_dpher - ph_esm );	
		move_eom_phase((int16_t)(cal_eom_dpher - ph_esm) - (int16_t)reg_EOM_DPHER_LANE_6_0);
		#endif
		...
		move_eom_phase((int16_t)(cal_eom_dpher + ph_dat + ph_esm) - (int16_t)reg_EOM_DPHER_LANE_6_0);
		#else
		//eom_phase_update( cal_eom_dpher - ph_dat - ph_esm  );		
		move_eom_phase((int16_t)(cal_eom_dpher - ph_dat - ph_esm) - (int16_t)reg_EOM_DPHER_LANE_6_0);
		#endif	
		...
		move_eom_phase((int16_t)(cal_eom_dpher -( tag_CDR_HALF_UI_PI_CODE + ph_dat + ph_esm)) - (int16_t)reg_EOM_DPHER_LANE_6_0);
		#else
		//eom_phase_update( cal_eom_dpher + ( tag_CDR_HALF_UI_PI_CODE + ph_dat + ph_esm  ));			
		move_eom_phase((int16_t)(cal_eom_dpher +( tag_CDR_HALF_UI_PI_CODE + ph_dat + ph_esm)) - (int16_t)reg_EOM_DPHER_LANE_6_0);
		#endif
	...
	if( ph_control_mode<2) {
		...
		set_sel_sdge_dly_tb(ph_dat + ph_esm);
	}
	else if( ph_control_mode==3) {
		set_sel_sdge_dly_tb_mode3(tag_CDR_HALF_UI_PI_CODE + ph_dat + ph_esm);
	}
	#endif
...
uint8_t phase_lookup_tracking(uint8_t step_size) {
	uint8_t dir, hit_boundary, pd, locked;
	uint16_t cnt, loop;
	
	//PHY_STATUS = ST_RXALIGN90_TRACK;
	
	cnt = 0; loop = 0; locked = 0;
	pd = EDGE2DATA_PD; dir = pd;
 	#if (_DBG_PHASE)
	if(cont_cal_on==0) 
	Printf(" :gm=%d, dac=%d, dm=%d -->", gm_bin, dac, dm);
	...
	do 
	{
		...
		if(cont_cal_on==0)  
		{
			...
			Printf(" gm=%d, dac=%d, dm=%d pd(%d):%lx\n", gm_bin, dac, dm, pd, MCU_DEBUG_LANE.VAL);				
		}
		#endif
		if(dir != pd ) {
			dir = pd;
 	...
 	#if (_DBG_PHASE)
	if(cont_cal_on==0) 
	Printf(" %d-n%d:gm/dac/dm=(%d,%d,%d) hit_boundary=%d", ALIGN90_REF, loop, gm_bin, dac, dm, hit_boundary);
	#endif
...
void decode_to_state(void) {
	...
	#ifdef _LAB
	DBG_Printf(" : #%d %d = %d", gm_bin, DAC, state);
	#endif
...
void advanced_clk_align(void) {
	...
	#if (_DBG_PHASE_TRAIN)
    DBG_PS("\r\n+++(advanced_clk_align) align90=%d", reg_ALIGN90_REF_LANE_5_0);
	#endif
	...
	DBG_Printf(" %d, %d:%d", train.f1, clk_sum, clk_step);  
	#endif
	...
	#if (_DBG_PHASE_TRAIN)
    DBG_PS("\r\n---(advanced_clk_align) : train.f1=%d, f1_avg=%d, align90=%d", train.f1, f1_avg, reg_ALIGN90_REF_LANE_5_0);
	#endif
	
	lnx_AVG_F1_LANE_7_0 = (uint8_t)f1_avg;		
