...
void Phase_Adaptation(void)
{
	...
	#if tag_force_phase_control	
	if(lnx_FORCE_PHASE_OFFSET_DATA_EN_LANE) { //test 
			train.phase_offset_data = lnx_TRAIN_PH_OS_DATA_2C_LANE_7_0;
			lnx_F0D_MEASURE_DONE_LANE = 0;
...
void EOM_Clock_Alignment(void)
{
	...
	do {	
		...
		if( lnx_EOM_PHASE_UI_ALIGN_FAILED_LANE == _EXIT_SUCCESS ) {
			...
			#if _DEBUG
			//DBG_Printf("\r\n\t\t -(Error_Path) %d: data_ofst=%d eom_ofst=%d ph_esm=%d(%d)",rpta_loop_cnt, train.ofst, norm_ofst, train.phase_offset_esm, reg_EOM_DPHER_LANE_6_0);
			//DBG_Printf(" eom_en_sde(%d,%d,%d)",EOM_EN_S,EOM_EN_D,EOM_EN_E);
			#endif

			if( norm_ofst > train.ofst )
				eom_vote_acc += 1;
			else if ( norm_ofst < train.ofst )
				eom_vote_acc -= 1;
			...
			if(rpta_vote!=0) {
				#if _DEBUG
			   // DBG_Printf("\r\n\t\t -(Error_Path) %d: eom_ph=%d data_ofst=%d eom_ofst=%d, data_f0=%d eom_f0=%d ",rpta_loop_cnt, eom_ph,train.ofst,norm_ofst, train.f0a, norm_f0);
		...
		//Printf(" cds.ofst_data: %d  cds.ofst_slicer: %d ", cds.ofst_data, cds.ofst_slicer);		
		#endif
				#if _DEBUG
				DBG_Printf("\r\n\t\t -(Error_Path) %d:%d: data_ofst=%d eom_ofst=%d ph_esm=%d(%d)",EOM_CONV_CNT, rpta_loop_cnt, train.ofst, norm_ofst, train.phase_offset_esm, reg_EOM_DPHER_LANE_6_0);
		...
		if( /*train.ofst > tag_DFE_F1_SAT_THRES || train.ofst < -tag_DFE_F1_SAT_THRES ||*/ train.f0a > tag_DFE_F0_SAT_THRES ) {
			// protect saturation
...
void Data_EOM_Align90_Adapt_EyeHeihgtMode(void)
{
	...
	do 	{	
		...
		if( lnx_EOM_PHASE_UI_ALIGN_FAILED_LANE == _EXIT_SUCCESS ) {
			...
			#if _DEBUG
			//DBG_Printf("\r\n\t\t -(1) %d: f0d_c=%d, f0d_l=%d, f0d_r=%d, f0d_c_pre=%d, f0d_l_pre=%d, f0d_r_pre=%d", rpta_loop_cnt,cds.f0d_slicer_center, cds.f0d_slicer_left, cds.f0d_slicer_right, f0d_c_pre, f0d_l_pre, f0d_r_pre);
			...
			DBG_Printf(" f0d_c=%d, f0d_l=%d, f0d_r=%d phase_vote_acc=%d, vote=%d", cds.f0d_slicer_center, cds.f0d_slicer_left, cds.f0d_slicer_right, phase_vote_acc, vote);
			#endif
			
			//-- Compare with pre-F0d 
			if( (f0d_c_pre -  cds.f0d_slicer_center) > F0D_THRESHOLD ) 	cds.f0d_slicer_center = f0d_c_pre;
			if( (f0d_l_pre -  cds.f0d_slicer_left) > F0D_THRESHOLD ) 	cds.f0d_slicer_left = f0d_l_pre;
			if( (f0d_r_pre -  cds.f0d_slicer_right) > F0D_THRESHOLD ) 	cds.f0d_slicer_right = f0d_r_pre;
	
			f0d_c_pre = cds.f0d_slicer_center;
			f0d_l_pre = cds.f0d_slicer_left;
			f0d_r_pre = cds.f0d_slicer_right;
	
			if( (phase_offset>1) && (cds.f0d_slicer_left==0) && (cds.f0d_slicer_right==0) ) 
			{
				phase_offset--;	
				#if _DEBUG
			...
			{			
				//Early Late Vote	
				if( cds.f0d_slicer_left > cds.f0d_slicer_center )
					phase_vote_acc -= 1;
				else if( cds.f0d_slicer_right > cds.f0d_slicer_center )
					phase_vote_acc += 1;
				else if( cds.f0d_slicer_left > cds.f0d_slicer_right )
					phase_vote_acc -= 1;
				else if( cds.f0d_slicer_right > cds.f0d_slicer_left )
					phase_vote_acc += 1;	
				...
				if(rpta_vote!=0) 
				{
					...
					//DBG_Printf(" f0d_c=%d, f0d_l=%d, f0d_r=%d phase_vote_acc=%d, vote=%d", cds.f0d_slicer_center, cds.f0d_slicer_left, cds.f0d_slicer_right, phase_vote_acc, rpta_vote);
					#endif
...
void Data_EOM_Align90_Adapt_EyeWidthMode(void)
{
	...
	do 	{	
		...
		if( lnx_EOM_PHASE_UI_ALIGN_FAILED_LANE == _EXIT_SUCCESS ) 
		{
			...
			//-- Compare F0d 
			if( cds.f0d_slicer_left > MAX_F0D ) //2
				phase_l_offset -= STEP_SIZE; //1
			else if(cds.f0d_slicer_left < MIN_F0D ) //1
				phase_l_offset += STEP_SIZE;
			phase_l_offset = clamp( phase_l_offset, +32, -32);	
			
			if( cds.f0d_slicer_right > MAX_F0D )
				phase_r_offset += STEP_SIZE;
			else if( cds.f0d_slicer_right < MIN_F0D ) 
				phase_r_offset -= STEP_SIZE;	
			phase_r_offset = clamp( phase_r_offset, +32, -32);				
			
			#if _DEBUG
			DBG_Printf("\r\n\t\t -(2) %d(%d):f0d_c=%d %d(%d):f0d_l=%d, %d(%d):f0d_r=%d pl/pr(%d,%d)", 
			norm_eom_ph_os_c, reg_MCU_DEBUGA_LANE_7_0, cds.f0d_slicer_center,
			norm_eom_ph_os_l, reg_MCU_DEBUGB_LANE_7_0, cds.f0d_slicer_left, 
			norm_eom_ph_os_r, reg_MCU_DEBUGC_LANE_7_0, cds.f0d_slicer_right, 
			...
			if(rpta_vote!=0) 
			{
				...
				DBG_Printf(" %d:f0d_c=%d %d:f0d_l=%d, %d:f0d_r=%d pl/pr=(%d,%d)", norm_eom_ph_os_c, cds.f0d_slicer_center,norm_eom_ph_os_l, cds.f0d_slicer_left, norm_eom_ph_os_r,cds.f0d_slicer_right, phase_l_offset, phase_r_offset);
				DBG_Printf(" phase_vote_acc=%d, vote=%d", phase_vote_acc, rpta_vote);
				#endif
				...
				((cal_phase + train.phase_offset_data + rpta_vote) == tag_CDR_PHASE_MIN) ) {
					...
					#if tag_force_phase_control	
					if(lnx_FORCE_PHASE_OFFSET_DATA_EN_LANE) {	
						//lnx_F0D_MEASURE_DONE_LANE = 1;
					}
					else 
					#endif
					{ 
				...
				//lnx_EYE_LEFT_2C_LANE_7_0 = norm_eom_ph_os_l;
				//lnx_EYE_RIGHT_2C_LANE_7_0 = norm_eom_ph_os_r;
				//lnx_EYE_MID_2C_LANE_7_0 = train.phase_offset_esm; //norm_eom_ph_os_c;		
				//lnx_ADAPT_PHASE_OFFSET_DATA_2C_LANE_7_0 = train.phase_offset_data;
				//lnx_TRAIN_F0D_LANE_7_0	= cds.f0d_slicer_center;		
...
void Data_Path_DFE_Adapt(void)
{
	...
	train.f0 = (cds.f0_data + cds.f0_slicer + 1)>>1;
	train.f0a = (cds.f0a_data + cds.f0a_slicer + 1)>>1;
	train.ofst = cds.ofst_slicer;//(cds.ofst_data + cds.ofst_slicer)/2;
	//Cal_Cont();
}
...
void EOM_clock_analog_align(void)
{
	...
	lnx_EOM_PHASE_UI_ALIGN_FAILED_LANE = cds.eye_check_pass==1? _EXIT_SUCCESS:_EXIT_FAIL;
