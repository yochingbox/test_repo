...
#define master_sync_request()	{ if(serdes_ring_lane_en==0) { SYNC_STATUS_LANE = ACK; SYNC_REQUEST_LANE = WAIT; \
		while(!reg_MCU_STATUS1_AND_31_0_b0); }}

#define master_sync_clean()	{ if(serdes_ring_lane_en==0) { SYNC_STATUS_LANE = IDLE; SYNC_REQUEST_LANE = IDLE; \
		while(reg_MCU_STATUS1_OR_31_0_b0); }}
	
		
#define slave_sync_wait()		{ if(serdes_ring_lane_en==0) {	while(SYNC_REQEST_LANE_READ!=WAIT); \
...
#define pcie_master_sync_request()	{ if(serdes_ring_lane_en==0) { SYNC_STATUS_LANE = ACK; SYNC_REQUEST_LANE = WAIT; \
		sync_check = 0; \
		do { \
...
#define pcie_master_sync_clean()	{ if(serdes_ring_lane_en==0) { SYNC_STATUS_LANE = IDLE; SYNC_REQUEST_LANE = IDLE; \
		sync_check = 0; \
		do { \
...
#define PowerUp_Seq_Simple() PowerUp_Seq_Test()
	...
	void load_init_temp_table(uint8_t rate);
