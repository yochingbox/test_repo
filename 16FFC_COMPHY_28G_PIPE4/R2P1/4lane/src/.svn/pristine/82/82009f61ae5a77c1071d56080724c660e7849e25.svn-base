...
extern void txdcc_clk_toggle( void ) BANKING_CTRL;
...
void txdcc_cal_01(void) BANKING_CTRL {
	...
	if (cmx_CAL_DONE == 0) {                        //power-on calibration
		...
		if (ext_en){                                     //use ext value for power-on, bypass calibration
			dcc_cnt = lnx_calx_txdcc[PWR][tx_pll_rate_x];
			TXDCC_HG = lnx_calx_txdcc_hg[tx_pll_rate_x];      //load the HG setting.
		}
		...
		dcc_cnt = lnx_calx_txdcc[PWR + cmx_TXDCC_CAL_CONT_CUR_LOAD_EN][tx_pll_rate_x];
		...
		TXDCCCAL_EN = 1;
		TRXDCCIN_SEL = 4;
		dcc_cnt = lnx_calx_txdcc[CONT][tx_pll_rate_x];    //always load cont result
	...
	if (cmx_CAL_DONE == 0) {                        //if power-on calibration (including ext_en)
		lnx_calx_txdcc[PWR][tx_pll_rate_x] = dcc_cnt;      //save to power-on result
		lnx_calx_txdcc_hg[tx_pll_rate_x] = ( TXDCC_HG & 0x1 ) | 0x0;       //save the HG setting for current rate, will be loaded by continious mode
	}
	//no matter power-on or LOAD or continious, need to save to [CONT] for next continious mode to read
	lnx_calx_txdcc[CONT][tx_pll_rate_x] = dcc_cnt;
	TXDCCCAL_EN = 0;
	lnx_TXDCC_CAL_DONE_LANE = 1;
} //txdcc_cal_01(void)
...
void txdcc_pdiv_cal_01(void) BANKING_CTRL {
	...
	if (cmx_CAL_DONE == 0) {     //power-on calibration, only performed when speed_div != 0
		...
		if (ext_en)                                     //use ext value for power-on (stored into PWR), bypass calibration
			dcc_cnt = lnx_calx_txdcc_pdiv[PWR][tx_pll_rate_x];
		else {                                          //use normal power-on calibration
			TXDCCCAL_PDIV_EN = 1;
			TXDCCCAL_EN = 1;
			TRXDCCIN_SEL = 4;
		...
		//either load power-on, or previous cont cal result for target speed
		dcc_cnt = lnx_calx_txdcc_pdiv[PWR + cmx_TXDCC_PDIV_CAL_CONT_CUR_LOAD_EN][tx_pll_rate_x];
		...
		TXDCCCAL_EN = 1;
		TRXDCCIN_SEL = 4;
		dcc_cnt = lnx_calx_txdcc_pdiv[CONT][tx_pll_rate_x];    //always load cont result
		in_cont_mode = 1;                              //when continious mode, use fraction code
	}
	else {                                          //Continious mode, but disabled, or SPEED_DIV != 0
		lnx_TXDCC_PDIV_CAL_DONE_LANE = 1;
	...
	if (cmx_CAL_DONE == 0) {                        //if power-on calibration
		lnx_calx_txdcc_pdiv[PWR][tx_pll_rate_x] = dcc_cnt;      //save to power-on result
	}
	//no matter power-on or LOAD or continious, need to save to [CONT] for next continious mode to read
	lnx_calx_txdcc_pdiv[CONT][tx_pll_rate_x] = dcc_cnt;
	TXDCCCAL_PDIV_EN = 0;
	TXDCCCAL_EN = 0;
	lnx_TXDCC_PDIV_CAL_DONE_LANE = 1;


} //txdcc_pdiv_cal_01(void)
...
void rxdcc_dll_cal_01(void) BANKING_CTRL {
  ...
  if (cmx_CAL_DONE == 0) {                        //power-on calibration
    ...
    if (ext_en) {                                     //use ext value for power-on, bypass calibration
      dcc_cnt = lnx_calx_rxdcc_dll[PWR][rx_pll_rate_x];
      RXDCC_HG_DLLCLK = lnx_calx_rxdcc_dll_hg[rx_pll_rate_x];          //load the HG setting.
    }
    else {                                          //use normal power-on calibration
    ...
    rxdcc_dll_step_cnt = RXDCC_DLL_CAL_STEP_SIZE;      //for power-on (inc. EXT_EN), reset fraction code to middle
                                                  ...
                                                  //either load power-on, or previous cont cal result for target speed
    dcc_cnt = lnx_calx_rxdcc_dll[PWR + cmx_RXDCC_DLL_CAL_CONT_CUR_LOAD_EN][rx_pll_rate_x];
    ...
    RXDCC_CAL_EN = 1;
    TRXDCCIN_SEL = 0;
    dcc_cnt = lnx_calx_rxdcc_dll[CONT][rx_pll_rate_x];    //always load cont result
    in_cont_mode = 1;                                  //when continious mode, use fraction code
  }
  else {                                               //Continious mode, but disabled, or SPEED_DIV != 0
...
//main loop
  ...
  //save calibration result
  if (cmx_CAL_DONE == 0) {                            //if power-on calibration
    lnx_calx_rxdcc_dll[PWR][rx_pll_rate_x] = dcc_cnt;       //save to power-on result
    lnx_calx_rxdcc_dll_hg[rx_pll_rate_x] = RXDCC_HG_DLLCLK; //save the HG setting for current rate, will be loaded by continious mode
  }
  lnx_calx_rxdcc_dll[CONT][rx_pll_rate_x] = dcc_cnt;     //no matter power-on or LOAD or continious, need to save to [CONT]
                                                      //for next continious mode to read
  RXDCC_CAL_EN = 0;
  lnx_RXDCC_DLL_CAL_DONE_LANE = 1;


} //rxdcc_dll_cal_01(void)
...
void rxdcc_eom_cal_01(void) BANKING_CTRL {
	...
	do {
		...
		if( cmx_CAL_DONE == 0 && RX_SPEED_DIV > 3 ) break;                  //power-on, but SPEED_DIV = 1xx, skip and keep using init code (0x20)
		if( cmx_CAL_DONE == 1 && time_out_cnt > 1) break;                   //continious mode, has ran 1 round
		if( cmx_CAL_DONE == 0 && is_num_toggle_pat( dir, time_out_cnt - 1, 2 ))  { lnx_RXDCC_EOM_CAL_PASS_LANE = 1;  break; }     //power-on, toggled
