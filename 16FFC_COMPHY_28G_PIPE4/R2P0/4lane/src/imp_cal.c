...
void tximp_cal( void ) BANKING_CTRL {
	...
	for(i=0; i<2; i++) {
		...
		dir = 0;
			...
			TXIMP_TUNEN0 = tximp_thermo_tb[tximp_l];
			
			dir = (dir << 1 ) | ana_tximpcal_out;
			if ( is_toggle_pat( dir, cnt + 1 ) ) break;
	...
	ave = u16div( sum >> 1,  6); //ave = (sum / 2) / 6
	ave_0 = (uint16_t)((sum - u16mul(ave, 12)) >> 1);
	lnx_CAL_TXIMP_TUNEN_LANE_7_0 = (uint8_t)(ave & 0x0f);	
	
	tximp_l = ave_0 > 2 ? 3 : ave_0;  //how about 5/6, 4/6 ??
	lnx_CAL_TXIMP_TUNEN0_LANE_7_0 = tximp_thermo_tb[ tximp_l ];
	...
	for(i=0; i<4; i++) {
		...
		dir = 0;
			...
			TXIMP_TUNEP0 = tximp_thermo_tb[tximp_l];
	...
	ave = u16div( sum >> 2, 6 ); //ave = (sum / 2) / 6
	ave_0 = (uint16_t)((sum - u16mul(ave, 24)) >> 2);
	lnx_CAL_TXIMP_TUNEP_LANE_7_0 =(uint8_t)(ave & 0x0f);

	tximp_l = ave_0 > 2 ? 3 : ave_0;
	lnx_CAL_TXIMP_TUNEP0_LANE_7_0 = tximp_thermo_tb[ tximp_l ];
