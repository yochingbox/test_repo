...
void Power_Slumber( void ) BANKING_CTRL {
	...
	if(	reg_PU_PLL_OR==0 ) 	{
		...
		#endif
		{
			...
			while( SYNC_SATUS_PLL_OR_CHG==0 ) {
				if(reg_PU_PLL_OR) break;
			}
		}
		
	}	
	else if( reg_PU_PLL_OR==1 && (SYNC_SATUS_PLL_OR_CHG_READ) ) {
		pu_pll_on();
	}
...
//	reg_MCU_CLK_SEL_1 = 0; 
	...
	if(	(reg_PU_PLL_OR==1 && SYNC_SATUS_PLL_OR_CHG_READ) /*|| (serdes_ring_lane_en && tag_ringpll_ind_en)*/)
		pu_pll_on();	
	...
	if( reg_PIN_PU_TX_RD_LANE==1 && reg_PIN_PU_RX_RD_LANE==1) 
	{
		...
		if( reg_PIN_PU_RX_RD_LANE==0 ) PHY_STATUS = ST_PSLUMBER_RX;
	}
}
