...
/*
uint8_t remote_error_check(void) {
	...
	if((device_remote_control_bits_hi&0xe0)==0xe0) error_code = 0xff;
...
//			if(error_status) {
//				reg_LOCAL_STATUS_FIELD_LANE_15_0 = save_local_status;
//			}	
...
void Send_ctrl_to_tx(uint16_t dat)
{
 	...
 	//DBG_Printf("\r\n++(Send_ctrl_to_tx) : %x", dat);
		...
		if( tx_pipe4_en && reg_EYE_OPEN_EN_RD_LANE && cmx_TRAIN_SIM_EN==0 ) {
