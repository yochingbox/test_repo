...
void ring_pll_cal( void ) BANKING_CTRL {
	...
	if(cmx_RINGPLL_CAL_TIMEOUT_DIS==0) {
		timeout_start((uint16_t)50000); //1mS  //50mS
	}	
...
void ring_pll_fast_cal(void) BANKING_CTRL {
	...
	timeout=0;	
	if(cmx_RINGPLL_CAL_TIMEOUT_DIS==0) {
        timeout_start((uint16_t)50000); //1mS 
	}	
		...
		reg_FBC_PLLCAL_RING_EN = 1;
		delay01(20);  //programmable 2us/4us/8us/16us => need 6us setting from cmx_TSHRTR_7_0.
		...
		//		if (((Ftarget_ring - Ftarget_code) <= Fmea) && (Fmea <= (Ftarget_ring + Ftarget_code))) {
			...
			ringpll_dac_fine_output();
              		...
              		delay01(40); // programmable 1/2/3/4 us.
		}
                i++;
		delay01(40);
	...
	delay01(10); // programmable 1/2/3/4us. 
	ring_size_counter = ring_size_counter_mid;
	PLL_CAL_EN_RING = 0;
	PLL_SHRTR_RING = 0;
