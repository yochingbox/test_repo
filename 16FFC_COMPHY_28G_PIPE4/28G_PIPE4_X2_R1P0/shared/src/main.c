...
void main(){
    ...
    while(1){
		...
		switch(PHY_STATUS){
			...
			if(cmx_APTA_TRAIN_SIM_EN || reg_PHY_MCU_REMOTE_REQ_LANE) 
				apta_txtrain(); break;	
			#endif	
			...
			case ST_NORMAL: 
							...
							#ifdef SUPPORT_RXFFE_ACCAP_EN //#if tag_RXFFE_ACCAP_EN==1
		...
		if(do_rxinit) {PHY_STATUS = ST_DTL; }
			...
			do_rxtrain = 1; PHY_STATUS = ST_TRXTRAIN;
		}
		#ifdef SUPPORT_APTA_TX_TRAIN
		...
		//#ifdef _28G_X2				
		//else if(reg_PHY_MCU_REMOTE_REQ_LANE && reg_PHY_MCU_REMOTE_ACK_LANE==0) 
		//	cmd_if();				
		//#endif
