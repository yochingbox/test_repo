...
void dfe_adaptation( void )  BANKING_CTRL {
	...
	save_cds_flg = reg_CDS_DFE_ADAPT_DC_VREF_EN_LANE;
	if( cds_tb_inx == CDS_EN_RESTRAIN ) {
		reg_CDS_DFE_ADAPT_DC_VREF_EN_LANE = 0;
	}
    ...
    for (dfe_cnt=0;dfe_cnt<dfe_max_cnt;dfe_cnt++)
	{
		...
		else
		{
			...
			#endif
			{
				cds_tb_inx = ctrl_cdr_phase_on? CDS_EN_PHASETRAIN : cds_call;
				drv_cdr_dfe_scheme(cds_table[cds_tb_inx]);
...
if((lnx_CLI_CMD_LANE_7_0 == 0xF8 || lnx_CLI_CMD_LANE_7_0 == 0xE6) && lnx_CLI_START_LANE == 1)
{
	...
	//print_train();
	//DBG_Printf("\r\n [%06lx]", (uint32_t)cds_table[cds_call]);
	...
	//DBG_Printf(" f0a_max:%d, (r,c):(%d,%d), dfe_res:%d, sum_vref_top/bot(%d/%d), f1p5(%d,%d), ",train.f0a_max, train.r,train.c,ctrl_step_res_dfe, sum_vref_top, sum_vref_bot,reg_DFE_F1P5_E_LANE_5_0,reg_DFE_F1P5_O_LANE_5_0 );
	//DBG_Printf(" saturated:%d, sumFtap:%d, boost:%d lvl:%d, phase:%d(%d)=%d", train.saturated,train.sumFtap,train.boost, train.level, train.phase_offset_data, (reg_RX_ALIGN90_CAL_SETTING_LANE_7_0&0x7f), drv_PH_OS_DAT);
	...
	reg_CDS_DFE_ADAPT_DC_VREF_EN_LANE = save_cds_flg;
...
//dfe res table
// round(mv/code*64)
__code int16_t dfe_res_f0_sumFtap[] = { 48,87,120,131};
...
__code int16_t dfe_res_f5to7_sumFtap[] = {32,45};
__code int16_t dfe_res_f8to10_sumFtap[] = {24, 34};
__code int16_t dfe_res_vref_sumFtap[] = {65,109,131,175};
...
//#endif
	...
	if(!pam2_en) {
		...
		sum_my_abs_fx = ((abs16(sum_vref_top)+abs16(sum_vref_bot))>>1) * vref_res;
	}
	#endif
