...
#define dfe_pre_adapt_en        0x1000000
...
#define     Tdfe_pre        reg_CDS_TDFE_PRE_LANE_15_0
...
void drv_cdr_dfe_scheme(uint32_t cds_en) BANKING_CTRL
{
    ...
    // if(cds_en & dfe_pre_adapt_en)            // adapt F0,1,2,VREF to help DTL lock
    if(cds_en & dtl_init_en)            
    {
        if (reg_CDS_DFE_PRE_EN_LANE)
        {
            reg_ANA_RX_CDR_PATTERN_EN_LANE = 0; // cdr_pattern_index=x: 0X<->1X, same as cdr_mode=0 in 112G
            reg_SEL_MU_F_LANE = 0;
            reg_ANA_RX_SEL_MU_F_LANE = 0;

            // set foffset to large value, e.g. 400-1000ppm
            reg_INIT_RXFOFFS_LANE_12_0_b0 = 244;    // lower 8 bits: 244
            reg_INIT_RXFOFFS_LANE_12_0_b1 = 1;      // higher 5 bits: 1 -> 256, init_rxfoffs = 500 -> 1000ppm (unit=1.95ppm)
            reg_DTL_FLOOP_EN_LANE = 0;

            // Feedback voting sharing to ALL samplers: D/S, E/O, T/M/B
            set_dfe_adapt_path(PATHall);
            set_dfe_fbmd(FBall);        // fbmd_f0, ds, eo, vref
            set_dfe_vldmd(VLDfast);     // tmb_vld f0,1,2,vref
            ...
            // Run for some time
            dfe_fsm(Tdfe_pre, Updatepre);

            // Set FOFFSET back to 0
            reg_DTL_FLOOP_EN_LANE = 1;
            reg_INIT_RXFOFFS_LANE_12_0_b0 = 0;
            reg_INIT_RXFOFFS_LANE_12_0_b1 = 0;
            reg_DTL_FLOOP_EN_LANE = 0;
    ...
    if(cds_en & dfe_adapt_en)
    {
        ...
        if(cds_en & f0a_coarse_en)
        {
            ...
            reg_DFE_VREF_COARSE_STEP_EN_LANE = 1;
            reg_DFE_F0_COARSE_STEP_EN_LANE = 1;
            if (!reg_RX_PAM2_EN_LANE && reg_CDS_DFE_ADAPT_DC_VREF_EN_LANE) // PAM4: adapt dc vref together
                reg_DFE_VREF_MODE_LANE_1_0 = 2;
            else
                reg_DFE_VREF_MODE_LANE_1_0 = 0;
        ...
        if(cds_en & dfe_coarse_en)
        {
            ...
            {
                reg_DFE_VREF_MODE_LANE_1_0 = 2;
                dfe_fsm(Tdfe_coarse, Updatevref | Updatedc | Update0 | Updateisi);
            } 
            else 
            {
                reg_DFE_VREF_MODE_LANE_1_0 = 0;
        ...
        if(cds_en & dfe_fine_en)
        {
            ...
            {
                reg_DFE_VREF_MODE_LANE_1_0 = 2;
                dfe_fsm(Tdfe_fine, Updatevref | Updatedc | Update0 | Updateisi);
            } 
            else 
            {
                reg_DFE_VREF_MODE_LANE_1_0 = 0;
        ...
        if(cds_en & dfe_accu_en)
        {
            ...
            {
                reg_DFE_VREF_MODE_LANE_1_0 = 2;
                dfe_fsm(Tdfe_accu, Updatevref | Updatedc | Update0 | Updateisi);
            } 
            else 
            {
                reg_DFE_VREF_MODE_LANE_1_0 = 0;
