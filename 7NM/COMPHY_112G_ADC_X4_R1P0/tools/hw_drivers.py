...
#from cap2_drivers import *
# real chip controls

# dummy ctle state control
ctle_R = 0
ctle_C = 0
ctle_gain_coarse = 4 # programmable
use_table = 0
# Initial CTLE bias currents - these need to be initialized from the speed table in "train_init"
ctle_cur1_init    = 0
ctle_cur2_init    = 0
ctle_cur_tia_init = 0
...
CE_P2_PRE_POST = 2
CE_P3_PRE_ONLY = 3
CE_CL72_INIT_PRE_POST = 1
CE_CL72_PRESET_NO_EQ = 2
...
trx_train_ctle_C_min = 0
trx_train_ctle_R_min = 0
trx_train_ctle_GC_min = 0
trx_train_ctle_C_max = 100
trx_train_ctle_R_max = 9
trx_train_ctle_GC_max = 5
trx_train_ctle_RS1_max = 8
...
def ctle_R_incdec(step):
    ...
    Rmax = trx_train_ctle_R_max # can be programmed
    sat_ind = 0
...
def ctle_C_incdec(step, iscode=False):
    ...
    Cmax = trx_train_ctle_C_max # max number of code
    sat_ind = 0
...
def ctle_gain_coarse_incdec(step):
    GCmin = trx_train_ctle_GC_min # min allowed - 0 pr speed related
    GCmax = trx_train_ctle_GC_max # maxallowed = 15 or speed related
    sat_ind = 0
...
def set_ctle(R,C,gain_coarse,
             trx_train_ctle_RS1_max = 8,
             trx_train_cur_tia_GC_map_force_speed_en = 0,
             use_table = 0):
    global ctle_R
    global ctle_C
    global ctle_gain_coarse
    ...
    if R > read('trx_train_ctle_r_max_lane[4:0]'):
        R = read('trx_train_ctle_r_max_lane[4:0]')
    if C > read('trx_train_ctle_c_max_lane[7:0]'):
        C = read('trx_train_ctle_c_max_lane[7:0]')
    if gain_coarse > read('trx_train_ctle_gc_max_lane[4:0]','two'):
        gain_coarse = read('trx_train_ctle_gc_max_lane[4:0]','two')    
    ctle_R = R
    ctle_C = C
    ...
    # List of CTLE bias currents to be controlled
    bias_sel = ('RX_CTLE_CUR2_SEL_LANE[3:0]','RX_CTLE_CUR1_SEL_LANE[3:0]','RX_CTLE_CUR_TIA_SEL_LANE[3:0]')

    # gain-coarse
    rx_speed = read('PHY_GEN_RX_LANE[5:0]')   
    if ctle_gain_coarse >= 0:
        ...
        # protect by speed
        if (rx_speed == 34 or rx_speed == 35 or rx_speed == 37 or rx_speed == 38 or trx_train_cur_tia_GC_map_force_speed_en):
            if ctle_gain_coarse >= 7:
                write('RX_CTLE_CUR_TIA_SEL_LANE[3:0]',ctle_cur_tia_init+2)
            elif ctle_gain_coarse == 6:
                write('RX_CTLE_CUR_TIA_SEL_LANE[3:0]',ctle_cur_tia_init+1)
            else:
                cur_tia = read('RX_CTLE_CUR_TIA_SEL_LANE[3:0]')
                if cur_tia >= ctle_cur_tia_init:
                    write('RX_CTLE_CUR_TIA_SEL_LANE[3:0]',ctle_cur_tia_init)
                else:
                    write('RX_CTLE_CUR_TIA_SEL_LANE[3:0]',cur_tia)
        else:
            # ensure bias currents are reset to initial values
            write('RX_CTLE_CUR2_SEL_LANE[3:0]', ctle_cur2_init)
            write('RX_CTLE_CUR1_SEL_LANE[3:0]', ctle_cur1_init)
            write('RX_CTLE_CUR_TIA_SEL_LANE[3:0]',ctle_cur_tia_init)

    else:
        # Set coarse gain register to zero
        write('rx_ctle_gain_coarse_lane[3:0]', 0)
        ...
        #
        gain_coarse_mag = abs(ctle_gain_coarse)
        ...
        if ctle_cur2 < 0:
            ctle_cur2 = 0
        write('RX_CTLE_CUR2_SEL_LANE[3:0]',ctle_cur2)
        # decrement the 1st-stage bias next
        ctle_cur1 = ctle_cur1_init - 1 - (gain_coarse_mag-2)//3
        if ctle_cur1 < 0:
            ctle_cur1 = 0
        write('RX_CTLE_CUR1_SEL_LANE[3:0]',ctle_cur1)
        # finally decrement the TIA bias current
        ctle_cur_tia = ctle_cur_tia_init - 1 - (gain_coarse_mag-3)//3
        if ctle_cur_tia < 0:
            ctle_cur_tia = 0
        write('RX_CTLE_CUR_TIA_SEL_LANE[3:0]',ctle_cur_tia)

    # DC gain

    rs1_max = trx_train_ctle_RS1_max #speed table register
    if ctle_R <= rs1_max:
        write('rx_ctle_rs1_ctrl_lane[3:0]', ctle_R)
        write('rx_ctle_rs2_ctrl_lane[4:0]', 0)
    else:
        write('rx_ctle_rs1_ctrl_lane[3:0]', rs1_max)
        write('rx_ctle_rs2_ctrl_lane[4:0]', 2*(ctle_R-rs1_max))
...
def set_TXFFE_preset(num_preset):
    ...
    set_lms_continous()
    req_local_ctrl(num_preset, ST_HOLD, ST_HOLD, ST_HOLD, ST_HOLD, ST_HOLD, CP_PAM4_GRAY)
    # for matlab use only
...
def mse_mnt(dtl_sel, len):
    ...
    if while_wait('mse_done_lane',1000) <= 0:
        ...
        if while_wait('mse_rd_ack_lane',1000) <= 0:
            val = -1
        else:
...
def adc_sig_mnt(abs_mode, th1, th2=0):
    ...
    if while_wait('sigmnt_adc_to_dp_done_lane',1000) <= 0:
        ...
        write('sigmnt_adc_to_dp_start_lane', 0)
        if while_wait('sigmnt_adc_to_dp_done_lane',1000,target_val=0) <= 0:
            val_mean = 0xFFFF
            val_th1 = 0xFFFF
            val_th2 = 0xFFFF
        else:
...
def sh_sig_mnt(abs_mode, th1, th2=0):
    ...
    if while_wait('sigmnt_shared_done_lane',1000) <= 0:
        ...
        write('sigmnt_shared_start_lane', 0)
        if while_wait('sigmnt_shared_done_lane',1000,target_val=0) <= 0:
            val_mean = 0xFFFF
            val_th1 = 0xFFFF
            val_th2 = 0xFFFF
        else:
...
def set_dp_hp(num_set):
    ...
    gain_hp = [240, 196, 280]
    write('rx_dp_lms_ffe_pre8_ext_lane[2:0]', 0, 'two')
    write('rx_dp_lms_ffe_pre7_ext_lane[2:0]', 0, 'two')
    write('rx_dp_lms_ffe_pre6_ext_lane[2:0]', 0, 'two')
    write('rx_dp_lms_ffe_pre5_ext_lane[2:0]', 0, 'two')
    write('rx_dp_lms_ffe_pre4_ext_lane[2:0]', 0, 'two')
    write('rx_dp_lms_ffe_pre3_ext_lane[3:0]', 0, 'two')
    write('rx_dp_lms_ffe_pre2_ext_lane[4:0]', ffe_hp[num_set][0], 'two')
    write('rx_dp_lms_ffe_pre1_ext_lane[6:0]', ffe_hp[num_set][1], 'two')
    write('rx_dp_lms_ffe_pst1_ext_lane[6:0]', ffe_hp[num_set][2], 'two')
    write('rx_dp_lms_ffe_pst2_ext_lane[5:0]', ffe_hp[num_set][3], 'two')
    write('rx_dp_lms_ffe_pst3_ext_lane[4:0]', ffe_hp[num_set][4], 'two')
    write('rx_dp_lms_ffe_pst4_ext_lane[3:0]', ffe_hp[num_set][5], 'two')
    write('rx_dp_lms_ffe_pst5_ext_lane[3:0]', ffe_hp[num_set][6], 'two')
    write('rx_dp_lms_ffe_pst6_ext_lane[3:0]', ffe_hp[num_set][7], 'two')
    write('rx_dp_lms_ffe_pst7_ext_lane[2:0]', ffe_hp[num_set][8], 'two')
    write('rx_dp_lms_ffe_pst8_ext_lane[2:0]', ffe_hp[num_set][9], 'two')
    write('rx_dp_lms_ffe_pst9_ext_lane[2:0]', ffe_hp[num_set][10], 'two')
    write('rx_dp_lms_ffe_pst10_ext_lane[2:0]', ffe_hp[num_set][11], 'two')
    write('rx_dp_lms_ffe_pst11_ext_lane[2:0]', 0, 'two')
    write('rx_dp_lms_ffe_pst12_ext_lane[2:0]', 0, 'two')
    write('rx_dp_lms_ffe_pst13_ext_lane[2:0]', 0, 'two')
    write('rx_dp_lms_ffe_pst14_ext_lane[2:0]', 0, 'two')
    write('rx_dp_lms_ffe_pst15_ext_lane[2:0]', 0, 'two')
    write('rx_dp_lms_ffe_pst16_ext_lane[2:0]', 0, 'two')
    write('rx_dp_lms_ffe_pst17_ext_lane[2:0]', 0, 'two')
    write('rx_dp_lms_ffe_pst18_ext_lane[2:0]', 0, 'two')
    write('rx_dp_lms_ffe_pst19_ext_lane[2:0]', 0, 'two')
    write('rx_dp_lms_ffe_pst20_ext_lane[2:0]', 0, 'two')
    write('rx_dp_lms_ffe_flt1_ext_lane[2:0]', 0, 'two')
    write('rx_dp_lms_ffe_flt2_ext_lane[2:0]', 0, 'two')
    write('rx_dp_lms_ffe_flt3_ext_lane[2:0]', 0, 'two')
    write('rx_dp_lms_ffe_flt4_ext_lane[2:0]', 0, 'two')
    write('rx_dp_lms_ffe_flt5_ext_lane[2:0]', 0, 'two')
    write('rx_dp_lms_ffe_flt6_ext_lane[2:0]', 0, 'two')
    write('rx_dp_lms_ffe_flt7_ext_lane[2:0]', 0, 'two')
    write('rx_dp_lms_ffe_flt8_ext_lane[2:0]', 0, 'two')
    write('rx_dp_lms_gain_ext_lane[10:4]', gain_hp[num_set] >> 4)
    write('rx_dp_lms_gain_ext_lane[3:0]', gain_hp[num_set] & 0xf)
    write('rx_dp_lms_blw_ext_lane[5:0]', 0, 'two') # TBD: probably set slightly positive
    write('rx_dp_lms_dfe_ext_lane[5:0]', 0,'two')

    write('rx_dp_lms_top_ld_lane', 1)
    write('rx_dp_lms_top_ld_lane', 0)
...
def set_dp_lp(num_set):
    ...
    write('rx_dp_lms_ffe_pre4_ext_lane[2:0]', 0, 'two')
    write('rx_dp_lms_ffe_pre3_ext_lane[3:0]', 0, 'two')
    write('rx_dp_lms_ffe_pre2_ext_lane[4:0]', ffe_lp[num_set][0], 'two')
    write('rx_dp_lms_ffe_pre1_ext_lane[6:0]', ffe_lp[num_set][1], 'two')
    write('rx_dp_lms_ffe_pst1_ext_lane[6:0]', ffe_lp[num_set][2], 'two')
    write('rx_dp_lms_ffe_pst2_ext_lane[5:0]', ffe_lp[num_set][3], 'two')
    write('rx_dp_lms_ffe_pst3_ext_lane[4:0]', ffe_lp[num_set][4], 'two')
    write('rx_dp_lms_ffe_pst4_ext_lane[3:0]', ffe_lp[num_set][5], 'two')
    write('rx_dp_lms_ffe_pst5_ext_lane[3:0]', ffe_lp[num_set][6], 'two')
    write('rx_dp_lms_ffe_pst6_ext_lane[3:0]', ffe_lp[num_set][7], 'two')
    write('rx_dp_lms_ffe_pst7_ext_lane[2:0]', ffe_lp[num_set][8], 'two')
    write('rx_dp_lms_ffe_pst8_ext_lane[2:0]', ffe_lp[num_set][9], 'two')
    write('rx_dp_lms_ffe_pst9_ext_lane[2:0]', ffe_lp[num_set][10], 'two')
    write('rx_dp_lms_ffe_pst10_ext_lane[2:0]', ffe_lp[num_set][11], 'two')
    write('rx_dp_lms_ffe_pst11_ext_lane[2:0]', 0, 'two')
    write('rx_dp_lms_ffe_pst12_ext_lane[2:0]', 0, 'two')
    write('rx_dp_lms_ffe_pst13_ext_lane[2:0]', 0, 'two')
    write('rx_dp_lms_ffe_pst14_ext_lane[2:0]', 0, 'two')
...
def check_cdr_lock():
    ...
    write('rx_cdr_lock_det_sel_lane[1:0]',1)
...
def req_local_ctrl(ce, c1, c0, cn1, cn2, cn3=ST_HOLD, ctrl_pat=CP_PAM4_GRAY):
...
def drv_adc_cal(type, mode):
	#hw driver call
    return 

def drv_pwr_opt(pwr_opt_en):
	#hw driver call
    return 
    
def dfe_opt(start=0, stop=31, ffe_pst1_max=60, verbose=0):
    #hw driver call
	return
...
def load_ctle_from_spt():
    global ctle_cur1_init, ctle_cur2_init, ctle_cur_tia_init
	#load values from speed table
    loadspeedtbl_rx()
    write('rx_ctle_rs1_mid_lane[2:0]', 0)
    write('rx_ctle_cs1_mid_lane[2:0]', 0)
    write('rx_ctle_mid_freq_en_lane',0)
    write('rx_ctle_cur_tia_sel2_lane[2:0]', 4)
    write('rx_ctle_cur1_sel2_lane[2:0]', 4)
    write('rx_ctle_cur2_sel2_lane[2:0]',4)
    write('rx_ctle_rl1_extra_lane[2:0]',0)
    # save default values of CTLE currents
    ctle_cur1_init    = read('RX_CTLE_CUR1_SEL_LANE[3:0]')
    ctle_cur2_init    = read('RX_CTLE_CUR2_SEL_LANE[3:0]')
    ctle_cur_tia_init = read('RX_CTLE_CUR_TIA_SEL_LANE[3:0]')
    return ctle_cur1_init, ctle_cur2_init, ctle_cur_tia_init
 
def ffe_flt_opt():
	# floating tap optimization - hw driver call
	return
...
def set_agc_continuous():
	#set agc 
	return

def disable_agc_continuous():  
	# reset agc
	return 

def drv_gb_fifo_clk_toggle():
    #reset fifo - hw driver call
    return

def dtl_phase_opt(reset):
    #phase optimization - hw driver call
    return
    
def ffe_len_opt():
    # ffe len optimization - hw driver call
    return
    
def ffe_dtl_phase_delay():
    # toggle read request
    write('rx_eq_rd_req_lane',0)
    write('rx_eq_rd_req_lane',1)
    write('rx_eq_rd_req_lane',0)
    ...
    den = main
    for i in [1, 2, 3]:
        pre_i = read('rx_dtl_lms_ffe_pre{}_coe_lane[{}:0]'.format(i,6-i),'two') # read in two's complement
        pst_i = read('rx_dtl_lms_ffe_pst{}_coe_lane[{}:0]'.format(i,6-i),'two') # read in two's complement
        den += pst_i+pre_i
        num += (pst_i-pre_i)*i # it is not a real multiplication it is *1,*2,*3
...
def dis_pat_prot():
    #diable pattern protection - hw driver call
    return

def set_pat_prot():
    #enable pattern protection - hw driver call
    return

def drv_tx_train_if_init():
    # initailize tx-train interface - hw driver call
    return

def drv_tx_train_comp():
    # initailize tx-train interface - hw driver call
    return

def set_tx_modulation(ctrl_pat = CP_PAM4_GRAY):
    mode_update_flag, tmp1, tmp2 = req_local_ctrl(CE_P1_NO_EQ, ST_HOLD, ST_HOLD, ST_HOLD, ST_HOLD,ST_HOLD,ctrl_pat)
    return mode_update_flag
