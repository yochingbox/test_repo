...
parser.add_argument('--tap_idx', type=int,
                    help='The tap index of set_txffe_tap', default=2)
parser.add_argument('--tx_ffe_dir', type=int,
                    help='The TX-FFE direction of set_txffe_tap', default=1)
parser.add_argument('--ctle_comp_en', type=int,
                    help='CTLE compensation enable of trx_train_txffe_set_preset', default=0)
#parser.add_argument('--test_arg', nargs='+', type=int, 
#                    help='the argument of the test')
...
class DvTestBench(unittest.TestCase):
    ...
    def test_trx_train_txffe_set_preset(self):
        self.init_mat()
        set_ctle(R=1, C=30, gain_coarse=4)
        ctle_comp_en = args.ctle_comp_en
    ...
    def test_set_TXFFE_tap(self):
        ...
        set_TXFFE_preset(num_preset = 1)
        #args = parser.parse_args()
        tap_idx = args.tap_idx
        tx_ffe_dir = args.tx_ffe_dir
        p2pfix_en = 1
        print("test_case = " + tests[args.test_case])
