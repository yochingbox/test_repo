...
//      drv_TXTRAIN_STATUS_C3 = 0;
       if(drv_TXTRAIN_PATTERN_LOCK_LOST_EN) {      
       reg_PATTERN_LOCK_LOST_TIMEOUT_EN_LANE = 1;
       }
    ...
    if(phy_mode != PCIE && phy_mode != USB && reg_AUTO_FORCE_DIS_RX_LANE ) {
       ...
       //while (!reg_TX_TRAIN_ENABLE_RD_LANE);
       ...
       //while (reg_PLL_READY_RX_LANE || reg_PLL_READY_TX_LANE);
      ...
      {
         //while (!reg_CLK_8BIT_READY_RD_LANE);
         wait_for(reg_CLK_8BIT_READY_RD_LANE,1);
      }
      else
      {
         //while (!reg_CLK_10BIT_READY_RD_LANE);
...
void set_remote_tx_pam2() {
	...
	//check ack	
	//while(!reg_INT_REMOTE_STATUS_ACK_LANE && !reg_TX_TRAIN_FAILED_LANE && reg_TX_TRAIN_ENABLE_RD_LANE && !reg_TRX_TRAIN_STOP_ISR_LANE);
