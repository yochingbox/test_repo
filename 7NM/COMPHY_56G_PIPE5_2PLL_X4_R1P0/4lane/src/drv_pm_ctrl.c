...
*	4/24/2020   Heejeong Ryu        Added patch for PCIe speed change hang
*   
*/
...
void drv_power_on_dig_prot_dis(void) BANKING_CTRL {
  ...
  if (!reg_TX_SFT_RST_CHG_RISING_ISR_LANE && !timeout) { //HR: hardware doesn't have PLL unlock protection to PIN_PLL_READY_TX/RX.
    reg_PIN_PLL_READY_TX_LANE = 1;
  }

  if (!reg_RX_SFT_RST_CHG_RISING_ISR_LANE && !timeout) {
...
void drv_tx_on_dig_prot_dis(void) BANKING_CTRL {
  ...
  if (!reg_TX_SFT_RST_CHG_RISING_ISR_LANE && !timeout) {
    reg_PIN_PLL_READY_TX_LANE = 1;
  }
}
