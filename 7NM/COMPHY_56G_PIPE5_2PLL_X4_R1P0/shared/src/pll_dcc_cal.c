...
void set_ts_bypass_en(uint8_t val) BANKING_CTRL {
	...
	if(lnx_CALIBRATION_DEBUG0_LANE_7_0==0) { //debug
...
void set_rs_plldcc_bypass_en(void) BANKING_CTRL {
...
void rs_plldcc_cal(void) BANKING_CTRL {
	...
	//lnx_RS_PLL_DCC_CAL_TIME_LANE_15_0 = (uint16_t)(get_time()-cur_time);
	
	lnx_RX_PLLDCC_CAL_DONE_LANE = 1;
}


void set_rs_pllvdda_cal_bypass_en(void) BANKING_CTRL {
...
void rs_pllvdda_cal(uint8_t enable_flag) BANKING_CTRL {
	...
	if(cmx_PLL_VDDA_CAL_MODE && !reg_ANA_PLL_RS_LOCK_RD_LANE) return;
	...
	// increase step avoid timeout
    	//reg_PLL_RS_VDDA_FBDIV_CAL_TIMEOUT_STEPS_LANE_2_0 = 7;
    	//reg_PLL_RS_VDDA_INTP_CAL_TIMEOUT_STEPS_LANE_2_0 = 7;
    	//reg_PLL_RS_VDDA_PFD_CAL_TIMEOUT_STEPS_LANE_2_0 = 7;
	...
	///lnx_PLL_RS_VDDA_CAL_TIME_LANE_15_0 = (uint16_t)(get_time()-cur_time);		
	lnx_RX_PLLVDDA_CAL_DONE_LANE = 1;
}

// ---

void set_ts_plldcc_bypass_en(void) BANKING_CTRL {
	...
	if(reg_PLL_TS_LCPLL_POSTDIV_EN_LANE == 0)
		...
		if(lnx_CALIBRATION_DEBUG0_LANE_7_0==0)	reg_PLL_TS_DIV_DCC_CAL_BYPASS_EN_LANE = 1;
	}
	else
 	{
    		reg_PLL_TS_ANA_RSVDA_LANE_7_0 &= 0x7f; // set ana_rsvda[7] = 0
		reg_PLL_TS_DCC_CAL_DIR_INV_LANE = 1;
		if(lnx_CALIBRATION_DEBUG0_LANE_7_0==0)	reg_PLL_TS_DIV_DCC_CAL_BYPASS_EN_LANE = 0;
	}
...
void ts_plldcc_cal(void) BANKING_CTRL {
	...
	//lnx_TS_PLL_DCC_CAL_TIME_LANE_15_0 = (uint16_t)(get_time()-cur_time);

	lnx_TX_PLLDCC_CAL_DONE_LANE = 1;
}


void set_ts_pllvdda_cal_bypass_en(void) BANKING_CTRL {
	if(lnx_CALIBRATION_DEBUG0_LANE_7_0==0) { //debug
...
void ts_pllvdda_cal(uint8_t enable_flag) BANKING_CTRL {
	...
	if(cmx_PLL_VDDA_CAL_MODE && !reg_ANA_PLL_TS_LOCK_RD_LANE) return;
	...
	// increase step avoid timeout
	//reg_PLL_TS_VDDA_FBDIV_CAL_TIMEOUT_STEPS_LANE_2_0 = 7;
    	//reg_PLL_TS_VDDA_INTP_CAL_TIMEOUT_STEPS_LANE_2_0 = 7;
    	//reg_PLL_TS_VDDA_PFD_CAL_TIMEOUT_STEPS_LANE_2_0 = 7;
	...
	if(TX_PLL_RATE_INX==1 || phy_mode != PCIE) { //8G for PCIe 16G  
		if(lnx_CALIBRATION_DEBUG1_LANE_7_0==1) {// && reg_PLL_TS_VDDA_FBDIV_CAL_RESULT_RD_LANE_3_0==0xe) {
			lnx_SPEED_CHANGE_DEBUG0_LANE_7_0 = 1;
			while(lnx_SPEED_CHANGE_DEBUG0_LANE_7_0);
		}
	}
	...
	//lnx_PLL_TS_VDDA_CAL_TIME_LANE_15_0 = (uint16_t)(get_time()-cur_time);		
	lnx_TX_PLLVDDA_CAL_DONE_LANE = 1;
}
