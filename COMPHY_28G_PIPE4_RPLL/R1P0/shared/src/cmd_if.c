...
reg_MCU_DEBUG2_LANE_7_0 = cmd_code;
	...
	if(cmd_type == CMD_TYPE_ANA0) { //ana_ctrl
		...
		case CMD_BYPASS_CTLE_TRAIN_SET:	result = cmd_bypass_ctle_train(cmd_code); break;
		}
	}
...
uint8_t cmd_dfe_res(uint8_t gen, uint8_t cmd_code) CMD_BANK {
	...
	if(gen==0x80) {//current
		...
		if(cmd_code==CMD_RX_DFE_RES_GET) {
			lnx_PHY_LOCAL_VALUE_LANE_31_0_b0 = UPHY14_TRX_ANAREG_BOT_20.BT.B0; //dfe_res_f0/f1/f234
...
uint8_t cmd_sq_thrs_ratio(uint8_t gen, uint8_t cmd_code) CMD_BANK {
	...
	if(gen==0x80) {//current
		if(cmd_code==CMD_SQ_THRS_RATIO_GET) {
			...
			sq_thrs_ratio = lnx_PHY_REMOTE_CTRL_VALUE_LANE_31_0_b0&0x1f;
			if(sq_thrs_ratio==0) 
		...
		if(cmd_code==CMD_SQ_THRS_RATIO_GET) {
			...
			sq_thrs_ratio_tb[gen] = lnx_PHY_REMOTE_CTRL_VALUE_LANE_31_0_b0&0x1f;
...
uint8_t cmd_tx_slew_rate(uint8_t gen, uint8_t cmd_code) CMD_BANK {
	...
	if(gen==0x80) {//current ana
		if(cmd_code==CMD_TX_SLEW_RATE_GET) {
			...
			if(lnx_PHY_REMOTE_CTRL_VALUE_LANE_31_0_b2==1 || lnx_PHY_REMOTE_CTRL_VALUE_LANE_31_0_b2==2) return CMD_ST_INVALID_VALUE;
...
uint8_t cmd_bypass_ctle_train(uint8_t cmd_code) CMD_BANK {
	if(cmd_code==CMD_BYPASS_CTLE_TRAIN_GET) {
		lnx_PHY_LOCAL_VALUE_LANE_31_0_b0 = lnx_BYPASS_CTLE_TRAIN_LANE;
	}
	else {
		lnx_BYPASS_CTLE_TRAIN_LANE = lnx_PHY_REMOTE_CTRL_VALUE_LANE_31_0_b0 & 0x01;
	}
	return 0;
}
...
uint8_t cmd_tx_margin(uint8_t cmd_code) CMD_BANK {
	...
	if (cmd_code==CMD_TX_MARGIN_SET) {
		reg_TX_MARGIN_V0_LANE_2_0 = lnx_PHY_REMOTE_CTRL_VALUE_LANE_31_0_b0&0x07;
	}
	lnx_PHY_LOCAL_VALUE_LANE_31_0_b0 = reg_TX_MARGIN_V0_LANE_2_0;
...
uint8_t cmd_tx_preset(uint8_t cmd_code) CMD_BANK {
	...
	inx = lnx_PHY_REMOTE_CTRL_VALUE_LANE_31_0_b2 & 0x0f;
	...
	lnx_PHY_LOCAL_VALUE_LANE_31_0_b2 = inx;
...
uint8_t cmd_ssc(uint8_t gen,  uint8_t cmd_code) CMD_BANK {
	...
	if(gen==0x80) { //current ana
		...
		if(cmd_code==CMD_TX_SSC_GET) {
			...
			if(use_ring_pll) {
				#ifdef _LANE_RPLL
				lnx_PHY_LOCAL_VALUE_LANE_31_0_b1 = reg_SSC_DSPREAD_TX_TX_RING_LANE;
				lnx_PHY_LOCAL_VALUE_LANE_31_0_b0 = reg_SSC_AMP_TX_RING_LANE_6_0;
				#else // CMN_RPLL
			...
			if(use_ring_pll) {
				#ifdef _LANE_RPLL
				reg_SSC_DSPREAD_TX_TX_RING_LANE	=  lnx_PHY_REMOTE_CTRL_VALUE_LANE_31_0_b1&0x01;
				reg_SSC_AMP_TX_RING_LANE_6_0 =  lnx_PHY_REMOTE_CTRL_VALUE_LANE_31_0_b0;
				#else // CMN_RPLL
