...
void ring_pll_cal( void ) BANKING_CTRL {
	...
	if(phy_mode == SERDES) {
		while(1) {
			if(cmx_PLL_CAL_DONE==1) {
				...
				if(mcuid==MCU_LANE1 && cmx_LCPLL_LANE_SEL==MCU_LANE0 && SYNC_STATUS_LANE0_EN_READ==0) break;  
				if(mcuid==MCU_LANE0 && cmx_LCPLL_LANE_SEL==MCU_LANE1 && SYNC_STATUS_LANE1_EN_READ==0) break;  
			}	
		}	
	}
	#endif
	...
	// -- SPEED
	 ...
	 do {
		#ifndef SUPPORT_RINGPLL_FBCCNTRDY_CONT_TIMEOUT
