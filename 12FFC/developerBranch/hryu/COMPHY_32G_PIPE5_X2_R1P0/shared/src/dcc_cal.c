...
void dcc_cal_cont(void) BANKING_CTRL {
	...
	if (reg_TX_SPEED_DIV_LANE_2_0 == 0) {
		...
		reg_TX_DCC_CAL_POST_DIV_BYPASS_EN_LANE = 0;
    		...
    		// load TX DCC cal result
		reg_TX_DCC_CAL_RESULT_EXT_LANE_5_0 = lnx_cal_txdcc[PWR + cmx_TXDCC_CAL_CONT_CUR_LOAD_EN][tx_pll_rate_x];
`
		reg_TX_DCC_CAL_BYPASS_EN_LANE = 0; //1; set this to 0 so the TX DCC cal result can be loaded
		//reg_ANA_RSVDA_LANE_7_0 |= (1 << 3);  // ANA_RSVDA[3]
		//set_txdcccal_pdiv_en_lane();
