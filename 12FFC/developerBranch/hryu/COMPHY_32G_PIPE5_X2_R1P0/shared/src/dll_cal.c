...
void dll_vdda_init_cal(void) BANKING_CTRL {
	...
	if( cmx_EOM_DLL_CAL_EXT_EN ) {
		reg_EOM_DLL_VDDA_CAL_RESULT_EXT_LANE_5_0 = lnx_cal_vdda_dll_eom_sel[PWR][rx_pll_rate_x];
		reg_EOM_DLL_VDDA_CAL_BYPASS_EN_LANE = 0;
	...
	//if( !cmx_EOM_DLL_CAL_EXT_EN )
		lnx_cal_vdda_dll_eom_sel[PWR][rx_pll_rate_x] = reg_EOM_DLL_VDDA_CAL_RESULT_RD_LANE_5_0;
...
void dll_gm_vdda_cal(void) BANKING_CTRL {
	...
	if( cmx_EOM_DLL_CAL_EXT_EN ) {
		reg_EOM_DLL_GM_CAL_RESULT_EXT_LANE_2_0 = lnx_cal_dll_eom_gmsel[rx_pll_rate_x];
