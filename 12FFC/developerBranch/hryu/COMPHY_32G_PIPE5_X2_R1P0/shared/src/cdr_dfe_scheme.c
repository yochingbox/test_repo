...
//    uint8_t dfe_res_run_idx;
...
#ifdef SUPPORT_EXT_MEM
			dfe_load_type_ram(0); //load reset
		dc_load_ram(0); //load cal 
#else
...
#ifdef SUPPORT_EXT_MEM
                cds_save_ram(Savf0d_l);
#else
                cds_save(Savf0d_l);
#endif
            else
#ifdef SUPPORT_EXT_MEM
                cds_save_ram(Savf0d_r);
#else
                cds_save(Savf0d_r);
#endif
    ...
    if((cds_tb & eye_chk_en) && (lnx_EYE_CHECK_BYPASS_LANE==0))
    {
...
#ifdef SUPPORT_EXT_MEM
			dfe_load_type_ram(1);
			set_esm_voltage_ram(get_ec_voltage());
#else
			dfe_load_type(1);
...
#ifdef SUPPORT_EXT_MEM
			set_esm_voltage_ram(lnx_ESM_VOLTAGE_LANE_7_0);
#else
		...
		//reg_EOM_EN_LANE = 0;
#ifdef SUPPORT_EXT_MEM
		cds28.eye_check_pass = check_eye_ram();
#else
 ...
 */
 /* //removed because duplicated with current CDS
void set_slicer_data_sampler_adapt(uint8_t sel) BANKING_CTRL
{
    if(
...
#ifdef SUPPORT_EXT_MEM
        f0_min = min_ram(f0_min,cds28.f0d.d_n_e);
        f0_min = min_ram(f0_min,cds28.f0d.s_p_e);
        f0_min = min_ram(f0_min,cds28.f0d.s_n_e);
        f0_min = min_ram(f0_min,cds28.f0d.d_p_o);
        f0_min = min_ram(f0_min,cds28.f0d.d_n_o);
        f0_min = min_ram(f0_min,cds28.f0d.s_p_o);
        f0_min = min_ram(f0_min,cds28.f0d.s_n_o);
#else
