...
void ringloadSpdtbl_4_fcnt(void) {
	uint8_t fbck, ck1g, pll_rate;

	fbck = 0;
	ck1g = ring_use_250m;
	#ifdef _SATA_SERDES_BUILD
	if(phy_mode==SERDES) pll_rate = gen_pll_rate;
	else
	#endif
		pll_rate = PLL_RATE_SEL_RING;

	#ifdef _LANE_RPLL
	reg_PLL_FBDIV_RING_LANE_7_0 = ring_speedtable[ck1g][fbck][pll_rate][spdoft_fbdiv_cal_ring];
	reg_PLL_FBDIV_RING_LANE_9_8 = ring_speedtable[ck1g][fbck][pll_rate][spdoft_fbdiv_cal_ring+1];
	reg_ANA_FBCK_SEL_TX_RING_LANE = 0;
	#else // CMN_RPLL
	reg_PLL_FBDIV_RING_7_0 = ring_speedtable[ck1g][fbck][pll_rate][spdoft_fbdiv_cal_ring];
	reg_PLL_FBDIV_RING_9_8 = ring_speedtable[ck1g][fbck][pll_rate][spdoft_fbdiv_cal_ring+1];
