	...
	if(phy_mode == SERDES) {
		while(1) {
			if(cmx_PLL_CAL_RING_DONE==1) {
				delay01(10);
				if(cmx_PLL_CAL_RING_DONE==1 && reg_PLL_CAL_EN_RING==0) break; //wait for RINGPLL calibration done
			}	
		}	
	}
	#endif
...
void pll_fast_cal(void) BANKING_CTRL {
	...
	if(cmx_FORCE_LCPLL_SHRTR_1==0) check_pll_lock_0();
...
void check_pll_lock_0(void) BANKING_CTRL {
	...
	if(cmx_SKIP_LCPLL_LOCK_CHECK) return;
	
	timeout = 0;
	if(cmx_PLL_CAL_TIMEOUT_DIS==0)	{
		timeout_start((uint16_t)Ttimeout_timer_0); //10mS	
	}	
