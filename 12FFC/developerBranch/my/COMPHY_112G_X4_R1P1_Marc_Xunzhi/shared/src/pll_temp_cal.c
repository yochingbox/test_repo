...
void pll_temp_cal( void ) {
	...
	dir = TEMPC_LEVEL_TODIG; dir<<=2; delay_ram(T_5us);
	dir += TEMPC_LEVEL_TODIG; dir<<=2; delay_ram(T_5us);
...
//#define TEMPC_MUX_SEL						reg_LCPLL_TEMPC_MUX_SEL_LANE_3_0
//#define TEMPC_MUX_HOLD_SEL					reg_LCPLL_TEMPC_MUX_HOLD_SEL_LANE_3_0
//#define TEMPC_DAC_SEL						reg_LCPLL_TEMPC_DAC_SEL_LANE_7_0 //reg_LCPLL_TEMPC_MUX_SEL_LANE_3_0
//#define TEMPC_DAC_VAILD						reg_LCPLL_TEMPC_DAC_VALID_LANE

void load_init_temp_table(uint8_t pll_rate) BANKING_CTRL
{
	int16_t tsen_dat;
	...
	if (tsen_dat < -20)//-377)	//less than -20 celcius
	{
		tempc_mux_sel = 3;
		tempc_mux_hold_sel = 3;
	}
	else if (tsen_dat >= -20 && tsen_dat < 10)	//-20 to 10 celcius -377~-301
	{
		tempc_mux_sel = 4;
		tempc_mux_hold_sel = 4;
	}
	else if (tsen_dat >= 10 && tsen_dat < 40)	//10 to 40 celcius -301~-226
	{
		tempc_mux_sel = 5;
		tempc_mux_hold_sel = 5;
	}
	else if (tsen_dat >= 40 && tsen_dat < 70)	//40 to 70 celcius -226~-149
	{
		tempc_mux_sel = 6;
		tempc_mux_hold_sel = 6;
	}
	else if (tsen_dat >= 70 && tsen_dat < 100)	//70 to 100 celcius -149~-73
	{
		tempc_mux_sel = 7;
		tempc_mux_hold_sel = 7;
	}
	else if (tsen_dat >= 100)	//greater than 100 celcius -73
	{
		tempc_mux_sel = 8;
		tempc_mux_hold_sel = 8;
	}
	// bin to grey 2,6,7,5,4,c
	reg_LCPLL_TEMPC_MUX_SEL_EXT_LANE_3_0 = tempc_mux_sel ^ (tempc_mux_sel>>1);
	reg_LCPLL_TEMPC_MUX_HOLD_SEL_EXT_LANE_3_0 = tempc_mux_hold_sel ^ (tempc_mux_hold_sel>>1);
