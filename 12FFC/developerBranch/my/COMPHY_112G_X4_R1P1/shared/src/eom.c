...
#define EYECHECK 0x400000
// cds_table[CDS_EN_EYECHECK] has banking risk
...
void esm_exit()
{
	...
	if( eom_ext_mode==1 && lnx_ESM_EN_LANE==0 )
	{
		...
		// 20200225 Jianmin: eom dll cal should be on during eom; and off before eom_clk off
...
void EOM_clock_analog_align(void) // BANKING_CTRL 
{
	...
	do {
		...
		//if(cds_eye_check_pass) {
			...
			cdr_dfe_scheme(EYECHECK);// check twice
#endif
			if(cds112.f.eye_check_pass){
