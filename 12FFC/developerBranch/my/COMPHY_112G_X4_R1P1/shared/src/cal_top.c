...
check_pause_state(PAUSE_ST_PLL_CAL);
	...
	if( mcuid != MCU_CMN ) {
		...
		if(cmx_RXIMP_CAL_EXT_EN == 0){
			// issue request
			switch(mcuid){
				case MCU_LANE0: mcu_request_lane0 |= 1;break;
				case MCU_LANE1: mcu_request_lane1 |= 1;break;
				case MCU_LANE2: mcu_request_lane2 |= 1;break;
				case MCU_LANE3: mcu_request_lane3 |= 1;break;			
			}
			short_delay();
