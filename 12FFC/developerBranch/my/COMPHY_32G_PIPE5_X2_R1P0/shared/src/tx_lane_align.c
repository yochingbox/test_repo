...
void txlane_align(void) BANKING_CTRL {
	uint8_t save_txdcc_en, save_txdcccal_en, save_txdcccal_pdiv_en, tmp;
	...
	if(!reg_PHY_ALIGN_OFF) {
		...
		#ifdef _LANE_RPLL
		reg_ANA_TXCLK_SYNC_EN_RPLL_FORCE_LANE = 1;
		reg_ANA_TXCLK_SYNC_EN_RPLL_LANE = 1;
		#endif
		if(mcuid==master_mcu) {
			#ifndef _LANE_RPLL
		...
		#ifdef _LANE_RPLL
		reg_ANA_TXCLK_SYNC_EN_RPLL_FORCE_LANE = 0;
		reg_ANA_TXCLK_SYNC_EN_RPLL_LANE = 0;
		#endif
		if(mcuid==master_mcu) {
			#ifndef _LANE_RPLL
