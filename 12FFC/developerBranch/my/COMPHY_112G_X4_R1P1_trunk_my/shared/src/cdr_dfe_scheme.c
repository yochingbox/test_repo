...
void set_dfe_cont(bool on) BANKING_CTRL
{
    ...
    else if(reg_DFE_ADAPT_CONT_LANE) // off
    {
        ...
        while((wtag=0x18)&&!reg_DFE_DONE_LANE && !timeout);
        reg_DFE_START_LANE = 0;
        while((wtag=0x19)&&reg_DFE_DONE_LANE && !timeout);
        timeout_stop;
        if(timeout) {
...
#else
        while((wtag=0x1A)&&!reg_DFE_DONE_LANE)       delay_ram(Tus);
        //while((wtag=0x1B)&&reg_DIG_TEST_BUS_15_0_b1) delay_ram(Tus);
        //while((wtag=0x1C)&&reg_DIG_TEST_BUS_15_0_b1) delay_ram(Tus);
        while((wtag=0x1D)&&!reg_DFE_UPDATED_LANE)    delay_ram(Tus);
        while((wtag=0x1E)&&!reg_DFE_UPDATED_LANE)    delay_ram(Tus);
        reg_DFE_START_LANE = 0;
        delay_ram(Tus);
#endif
    }
...
void cds_load_vref() BANKING_CTRL
{
    ...
    tgl_fen_vref();
    while((wtag=0x1F)&&!reg_DFE_UPDATED_LANE);
}
/* **************************************************
 * CDS_LOAD_F0
 * Args:
...
void cds_load_f0(uint8_t type) BANKING_CTRL
{
    ...
    tgl_fen0();
    while((wtag=0x20)&&!reg_DFE_UPDATED_LANE);
}
...
void dfe_load(uint8_t type) BANKING_CTRL
{
    ...
    while((wtag=0x21)&&!reg_DFE_UPDATED_LANE);
}
...
void dc_load(uint8_t type) BANKING_CTRL
{
	...
	*/
    ...
    while((wtag=0x22)&&!reg_DFE_UPDATED_LANE);
}
...
void dfe_fsm(uint16_t lpnum, uint16_t tap_en) BANKING_CTRL
{
    ...
    while((wtag=0x23)&&!reg_DFE_DONE_LANE && !timeout);
    timeout_stop;
    if(timeout) {
    ...
    reg_DFE_START_LANE = 0;
#else
    while((wtag=0x24)&&!reg_DFE_DONE_LANE);
    reg_DFE_START_LANE = 0;
#endif
    return;
}
...
void dfe_setoff(void) BANKING_CTRL {
    ...
    if (reg_DFE_ADAPT_CONT_LANE) {
      ...
      // Wait for DFE done.
      while((wtag=0x25)&&!reg_DFE_DONE_LANE);
      reg_DFE_START_LANE = 0;
