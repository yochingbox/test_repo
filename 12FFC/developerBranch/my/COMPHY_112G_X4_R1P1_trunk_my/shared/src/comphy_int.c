...
static bool poll1_comphy_int_in1(void){
		...
		while((wtag=0x2D)&&!timeout && !comphy_int_in1);
        	timeout_stop;
		if(!timeout) return 1;
		else return 0;
}
static bool poll0_comphy_int_in1(void){
		...
		while((wtag=0x2E)&&!timeout && comphy_int_in1);
        	timeout_stop;
		if(!timeout) return 1;
		else return 0;
}
static bool poll1_comphy_int_in0(void){
		...
		while((wtag=0x2F)&&!timeout && !comphy_int_in0 && !reg_RX_INIT_RD_LANE && !reg_TX_TRAIN_ENABLE_RD_LANE);
        	timeout_stop;
...
void comphy_int(void) BANKING_CTRL{
	...
	while((wtag=0x30)&&poll1_comphy_int_in0()){
