...
lnx_TRAIN_DEBUG5_LANE_7_0=1;
	while((wtag=0xB1)&&1)
	{
		...
		if (ctrl_tx_train_done)
			...
			tx_ffe_train();	
		if(lnx_TRAIN_DEBUG5_LANE_7_0<0xf0)
			lnx_TRAIN_DEBUG5_LANE_7_0++;
	}
lnx_TRAIN_DEBUG5_LANE_7_0++;
	tx_train_end();
lnx_TRAIN_DEBUG5_LANE_7_0 = 0xff;
}
...
void tx_preset_sweep(void)
{
	...
	while((wtag=0xB4)&&index <= tag_TX_NUM_TX_PRESET) // while the index is not the last one
	{
