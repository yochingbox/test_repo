...
/*	cds_init();*/
	...
	reg_CDS_F0DK_RES_CUSTOM_EN_LANE = 0x1;
...
//#ifdef NO_REDUCE_CODE
	...
	if( mcuid != MCU_CMN ) {
		...
		// Read tsen_dat
		while((wtag=0x5)&&reg_TSEN_ADC_RDY==0);
	}
	// --- CAL start
	...
	else while((wtag=0x6)&&!PHY_CHECK_LANE0_READ);
	...
	if( mcu_en1) {
		...
		else while((wtag=0x7)&&!PHY_CHECK_LANE1_READ);
	}
	...
	if( mcu_en2) {
		...
		else while((wtag=0x8)&&!PHY_CHECK_LANE2_READ);
	}
	...
	if( mcu_en3) {
		...
		else while((wtag=0x9)&&!PHY_CHECK_LANE3_READ);
	}
#else
	do {
		while((wtag=0xA)&& SYNC_LINE_HOLD_READ != SYNC_IDLE ) {
			//wait for other lane
		};
	...
	} while((wtag=0xB)&&!lnx_VDD_CAL_DONE_LANE);
#endif
#endif
	if( mcuid != MCU_CMN ) {
		//rxdcc_dll_cal();
		...
		if(cmx_RXIMP_CAL_EXT_EN == 0){
			...
			if(mcu_en0) {
				if(mcuid==MCU_LANE0) {
					while((wtag=0xC)&&(reg_MCU_DEBUG_CMN_0_7_0&0x1)==0);
					rximp_cal_ram();
			...
			if(mcu_en1) {
				if(mcuid==MCU_LANE1) {
					while((wtag=0xD)&&((reg_MCU_DEBUG_CMN_0_7_0>>1)&0x1)==0);
					rximp_cal_ram();
			...
			if(mcu_en2) {
				if(mcuid==MCU_LANE2) {
					while((wtag=0xE)&&((reg_MCU_DEBUG_CMN_0_7_0>>2)&0x1)==0);
					rximp_cal_ram();
			...
			if(mcu_en3) {
				if(mcuid==MCU_LANE3) {
					while((wtag=0xF)&&((reg_MCU_DEBUG_CMN_0_7_0>>3)&0x1)==0);
					rximp_cal_ram();
		...
		if(cmx_TXIMP_CAL_EXT_EN == 0){
			...
			if(mcu_en0) {
				if(mcuid==MCU_LANE0) {
					while((wtag=0x10)&&(reg_MCU_DEBUG_CMN_0_7_0&0x1)==0);
					tximp_cal_ram();
					cmx_MCU_SYNC_LANE0 = 0;
				}
			}
			if(mcu_en1) {
				if(mcuid==MCU_LANE1) {
					while((wtag=0x11)&&((reg_MCU_DEBUG_CMN_0_7_0>>1)&0x1)==0);
					tximp_cal_ram();
					cmx_MCU_SYNC_LANE1 = 0;
				}
			}
			if(mcu_en2) {
				if(mcuid==MCU_LANE2) {
					while((wtag=0x12)&&((reg_MCU_DEBUG_CMN_0_7_0>>2)&0x1)==0);
					tximp_cal_ram();
					cmx_MCU_SYNC_LANE2 = 0;
				}
			}
			if(mcu_en3) {
				if(mcuid==MCU_LANE3) {
					while((wtag=0x13)&&((reg_MCU_DEBUG_CMN_0_7_0>>3)&0x1)==0);
					tximp_cal_ram();
					cmx_MCU_SYNC_LANE3 = 0;
				}
			}
...
void mcu_align_0(void) BANKING_CTRL{
	...
	if(mcuid==MCU_LANE3)
		cmx_MCU_SYNC_LANE3 = 0;
	}while((wtag=0x14)&&cmx_MCU_SYNC_LANE0!=0||cmx_MCU_SYNC_LANE1!=0||cmx_MCU_SYNC_LANE2!=0||cmx_MCU_SYNC_LANE3!=0);
	/*if( mcuid != MCU_CMN ) {
		PHY_CHECK_LANE = 0;
		while((wtag=0x15)&&(PHY_CHECK_LANE0_READ|PHY_CHECK_LANE1_READ|PHY_CHECK_LANE2_READ|PHY_CHECK_LANE3_READ));
	}*/
}
