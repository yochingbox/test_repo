...
#define reg_PT_CNT_MAX_LANE_31_0    PT_COUNTER5.VAL
		...
		while((wtag=0x26)&&reg_PT_CNT_LANE_31_0!=reg_PT_CNT_MAX_LANE_31_0 || reg_PT_CNT_LANE_47_32!=reg_PT_CNT_MAX_LANE_47_32){
		...
		if (reg_PT_ERR_CNT_LANE_47_32 !=0) {
			...
			while((wtag=0x27)&&temp_counter>0){
...
//				DBG_Printf(" %x",global_debug[i+1]);
	...
	case 0xd3:{//
		...
		while((wtag=0x2A)&&!timeout2_check(0)){
		DBG_Printf("%lu\r\n", timer2_check(0));
		timeout_start(50000);
		while((wtag=0x2B)&&!timeout);
		}
		DBG_Printf("%lu\r\n", timer2_check(0));
		break;
	}
