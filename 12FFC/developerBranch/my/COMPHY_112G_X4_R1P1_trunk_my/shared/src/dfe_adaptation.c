...
if((lnx_CLI_CMD_LANE_7_0 == 0xF8 || lnx_CLI_CMD_LANE_7_0 == 0xE6) && lnx_CLI_START_LANE == 1)
{
	...
	lnx_TRAIN_BOOST_LANE_15_0 = train.boost;
        ...
        if (reg_MCU_DEBUGF_LANE_7_0 ){ 
            while((wtag=0x31)&&reg_MCU_DEBUGF_LANE_7_0 ); // rapotor set it to 0 to continue for one iteration
...
void DFE_Final_Calculation( void ) BANKING_CTRL
{
...
#endif
	train.f0_data = (uint8_t)((((combine_f0(cds112.s.f0k) + (f0d_factor>>1))/f0d_factor) + half_shift) >> ctrl_max_dfe_adapt_shift);
   ...
   /*train.boost = (uint16_t) abs16(
	...
	if (rate_mode==QR) {
		...
		train.boost -= sumf_boost_target_c;
...
// Added by JL
void calculate_train_fn1_fx_tmb(){
