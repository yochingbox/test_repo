...
void set_remote_tx_pam2() {
	...
	//check ack	
	//while((wtag=0xC1)&&!reg_INT_REMOTE_STATUS_ACK_LANE && !reg_TX_TRAIN_FAILED_LANE && reg_TX_TRAIN_ENABLE_RD_LANE);
	while((wtag=0xC2)&&!reg_INT_REMOTE_STATUS_ACK_LANE && reg_TX_TRAIN_ENABLE_RD_LANE && !reg_TRX_TRAIN_STOP_ISR_LANE);
...
void set_remote_tx_pam4() {
	...
	//check ack
	//while((wtag=0xC3)&&!reg_INT_REMOTE_STATUS_ACK_LANE && !reg_TX_TRAIN_FAILED_LANE && reg_TX_TRAIN_ENABLE_RD_LANE);
	while((wtag=0xC4)&&!reg_INT_REMOTE_STATUS_ACK_LANE && reg_TX_TRAIN_ENABLE_RD_LANE && !reg_TRX_TRAIN_STOP_ISR_LANE);
...
void set_remote_tx() {
	...
	while((wtag=0xC5)&&reg_INT_REMOTE_STATUS_ACK_LANE);
