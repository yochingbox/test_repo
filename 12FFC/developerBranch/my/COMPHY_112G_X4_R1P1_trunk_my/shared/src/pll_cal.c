...
void pll_cal( void ) BANKING_CTRL {
	...
	
	{
		...
		do {	
			...
			reg_FBC_CNT_START = 1; 
			if( cnt_rdy ) while((wtag=0x56)&&reg_FBC_PLLCAL_CNT_READY && !timeout);
			while((wtag=0x57)&&!reg_FBC_PLLCAL_CNT_READY && !timeout);
			reg_FBC_CNT_START = 0; 				
			if(timeout) break;
		...
		do {
			...
			cnt_rdy = reg_FBC_PLLCAL_CNT_READY;
			reg_FBC_CNT_START = 1;
			if( cnt_rdy ) while((wtag=0x59)&&reg_FBC_PLLCAL_CNT_READY && !timeout);
			while((wtag=0x5A)&&!reg_FBC_PLLCAL_CNT_READY && !timeout);
			reg_FBC_CNT_START = 0;
			if(timeout) break;
			Fmea = reg_FBC_PLLCAL_CNT_15_0;
		...
		} while((wtag=0x5B)&&(Fmea < Ftarget) && !timeout); 
...
void do_fast_cal(uint8_t lccap_msb_index) {
	...
	uint16_t Fmea, save_FBDIV_h, save_FBDIV_l; 
		...
		do {
			cnt_rdy = reg_FBC_PLLCAL_CNT_READY;
			reg_FBC_CNT_START = 1;
			if( cnt_rdy ) while((wtag=0x5D)&&reg_FBC_PLLCAL_CNT_READY && !timeout);
			while((wtag=0x5E)&&!reg_FBC_PLLCAL_CNT_READY && !timeout);
			reg_FBC_CNT_START = 0;
			if(timeout) break;
			Fmea = reg_FBC_PLLCAL_CNT_15_0;
		...
		} while((wtag=0x5F)&&loop_cnt-- > 0 && !timeout);	
...
void check_pll_lock(void) BANKING_CTRL {
	...
	while((wtag=0x60)&&!reg_ANA_PLL_LOCK_RD && !timeout);
...
void pll_cal( void ) BANKING_CTRL {
	...
	//while((wtag=0x61)&&!lnx_PLL_CAL_DONE_LANE)
	do{
		switch (state)
		{
		case CAL_AMP:
		...
		case MEASURE_FREQ:
			...
			FBC_CNT_START = 1;
			if (cnt_rdy)
				while((wtag=0x62)&&FBC_PLLCAL_CNT_READY && !timeout);
			while((wtag=0x63)&&!FBC_PLLCAL_CNT_READY && !timeout);
			FBC_CNT_START = 0;
	...
	}while((wtag=0x64)&&!lnx_PLL_CAL_DONE_LANE);
...
void check_pll_lock(void) BANKING_CTRL {
	...
	while((wtag=0x65)&&!reg_ANA_PLL_LOCK_RD_LANE && !timeout);
	//while((wtag=0x66)&&!reg_ANA_PLL_LOCK_RD_LANE);
...
//******************************************************************************
	...
	VCOAMP_VTH_SEL = TH_NORMAL; //default midas val is 0x0C uint8_t VCOAMP_VTH_NORMAL_7_0                    : 8;   /* [15:8]     r/w 8'h0c*/
...
void pll_amp_cal(void) BANKING_CTRL {
	...
	while((wtag=0x67)&&!reg_PLL_AMP_TOP_DONE_LANE && !timeout);*/
	while((wtag=0x68)&&!reg_PLL_AMP_TOP_DONE_LANE);
