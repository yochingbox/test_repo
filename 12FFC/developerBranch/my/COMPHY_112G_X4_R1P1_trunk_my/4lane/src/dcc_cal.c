...
void txdcc_cal_ram(void) BANKING_CTRL {
	...
	while((wtag=0xCD)&&!reg_TX_ALIGN90_DCC_TOP_DONE_LANE && !timeout);
	if(timeout)
		time_out_record = 1;*/
	delay_ram(T_p5us);
	while((wtag=0xCE)&&!reg_TX_ALIGN90_DCC_TOP_DONE_LANE);
	reg_TX_ALIGN90_DCC_TOP_START_LANE = 0;
...
void rx_clk_cal_init_ram(void) BANKING_CTRL {
	...
	if ((reg_TXSPEED_DIV_LANE_2_0>>2&0x1)==1) {
...
void rx_clk_cal_ram(void) BANKING_CTRL {
	...
	while((wtag=0xCF)&&!reg_RX_CLK_TOP_DONE_LANE && !timeout);
	if(!timeout)*/
	while((wtag=0xD0)&&!reg_RX_CLK_TOP_DONE_LANE);
...
void rx_eom_cal_init_ram(void) BANKING_CTRL {
...
void rx_eom_cal_ram(void) BANKING_CTRL {
	...
	while((wtag=0xD1)&&!reg_RX_EOM_TOP_DONE_LANE && !timeout);*/
	while((wtag=0xD2)&&!reg_RX_EOM_TOP_DONE_LANE);
