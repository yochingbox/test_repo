...
void tximp_cal_init_ram_notInUse(void) BANKING_CTRL  { //BANKING_CTRL {
...
void tximp_cal_n_ram_notInUse(void) BANKING_CTRL  { //BANKING_CTRL {
	uint8_t i, TXIMP_N;
	...
	//{
		...
		for (i=0; i<=1; i++)
		{
			...
			while((wtag=0xE3)&&!reg_TX_IMP_TOP_DONE_LANE);
...
void tximp_cal_p_ram_notInUse(void) BANKING_CTRL  { //BANKING_CTRL {
	uint8_t i, TXIMP_P;
	...
	for (i=0; i<=3; i++)
	{
		...
		/*timeout = 0;
		timeout_start((uint16_t)TIMEOUT_TXIMP_CAL);*/
		while((wtag=0xE4)&&!reg_TX_IMP_TOP_DONE_LANE);
...
void tximp_ext_ram_notInUse(void) BANKING_CTRL  { //BANKING_CTRL {
#if 0
	reg_TX_IMP_N_CAL_RESULT_EXT_LANE_5_0 = 0x20;
	reg_TX_IMP_P_CAL_RESULT_EXT_LANE_5_0 = 0x20;
	reg_TX_IMP_N_CAL_INDV_EXT_EN_LANE = 1;
...
void tximp_cal_ram_notInUse( void ) BANKING_CTRL  { //BANKING_CTRL {

	PHY_STATUS = ST_TXIMP_CAL;

	lnx_TXIMP_CAL_DONE_LANE = 0;
	lnx_TXIMP_CAL_PASS_LANE = 1;
	//tximp_cal_init_ram_notInUse();

	/*if( cmx_TXIMP_CAL_EXT_EN == 1 ) {
		tximp_ext_ram_notInUse();
		return;
	}*/

	tximp_cal_n_ram_notInUse();
	tximp_cal_p_ram_notInUse();
...
void rximp_cal_ram_notInUse( void ) BANKING_CTRL  { //BANKING_CTRL {
	...
	while((wtag=0xE5)&&!reg_RX_IMP_TOP_DONE && !timeout);
	if (!timeout)*/
	while((wtag=0xE6)&&!reg_RX_IMP_TOP_DONE);
