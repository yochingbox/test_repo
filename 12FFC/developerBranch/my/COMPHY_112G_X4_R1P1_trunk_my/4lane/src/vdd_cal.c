...
void vdd_cal_sel( uint8_t sel ) BANKING_CTRL {
	...
	if(cont_cal_on == 0)
		set_test_pattern(0); //PRBS7 //TODO: need to use RAM bank function!
	...
	if(cont_cal_on == 0)
		recover_test_pattern(); //TODO: need to use RAM bank function!
	lnx_VDD_CAL_DONE_LANE = 1;
