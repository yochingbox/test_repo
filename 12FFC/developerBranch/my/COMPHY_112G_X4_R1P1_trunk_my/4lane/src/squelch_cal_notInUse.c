...
void rx_sq_ofst_cal_ram_notInUse(void) BANKING_CTRL {
	...
	while((wtag=0x100)&&!reg_RX_SQ_TOP_DONE_LANE);
	...
	/*timeout = 0;
	timeout_start((uint16_t)TIMEOUT_SQ_OFST_CAL);
	while((wtag=0x101)&&!reg_RX_SQ_TOP_DONE_LANE && !timeout);
	if(timeout)*/
	while((wtag=0x102)&&!reg_RX_SQ_TOP_DONE_LANE);
	if(reg_SQ_CAL_TIMEOUT_RD_LANE)
		lnx_SQ_OFST_CAL_PASS_LANE = 0;
	reg_RX_SQ_TOP_START_LANE = 0;
...
void rx_sq_thresh_cal_ram_notInUse(void) BANKING_CTRL {
	...
	while((wtag=0x103)&&!reg_RX_SQ_TOP_DONE_LANE && !timeout);
	if(timeout)*/

	while((wtag=0x104)&&!reg_RX_SQ_TOP_DONE_LANE);
	...
	/*timeout = 0;
	timeout_start((uint16_t)TIMEOUT_SQ_THRESH_CAL);
	while((wtag=0x105)&&!reg_RX_SQ_TOP_DONE_LANE && !timeout);
	if(timeout)*/
	while((wtag=0x106)&&!reg_RX_SQ_TOP_DONE_LANE);
	if(reg_SQ_CAL_TIMEOUT_RD_LANE)
		lnx_SQ_THRESH_CAL_PASS_LANE = 0;
	reg_RX_SQ_TOP_START_LANE = 0;
