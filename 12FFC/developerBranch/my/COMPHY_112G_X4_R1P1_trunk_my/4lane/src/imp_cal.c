...
void tximp_cal_n_ram(void) BANKING_CTRL  { //BANKING_CTRL {
	...
	//{
		...
		for (i=0; i<=1; i++)
		{
			...
			while((wtag=0xDF)&&!reg_TX_IMP_TOP_DONE_LANE);
...
void tximp_cal_p_ram(void) BANKING_CTRL  { //BANKING_CTRL {
	...
	for (i=0; i<=3; i++)
	{
		...
		/*timeout = 0;
		timeout_start((uint16_t)TIMEOUT_TXIMP_CAL);*/
		while((wtag=0xE0)&&!reg_TX_IMP_TOP_DONE_LANE);
...
void rximp_cal_ram( void ) BANKING_CTRL  { //BANKING_CTRL {
	...
	while((wtag=0xE1)&&!reg_RX_IMP_TOP_DONE && !timeout);
	if (!timeout)*/
	while((wtag=0xE2)&&!reg_RX_IMP_TOP_DONE);
