...
void rx_sq_ofst_cal_ram(void) BANKING_CTRL {
	...
	delay_ram(T_p5us);
	while((wtag=0xF9)&&!reg_RX_SQ_TOP_DONE_LANE);
	...
	/*timeout = 0;
	timeout_start((uint16_t)TIMEOUT_SQ_OFST_CAL);
	while((wtag=0xFA)&&!reg_RX_SQ_TOP_DONE_LANE && !timeout);
	if(timeout)*/
	delay_ram(T_p5us);
	while((wtag=0xFB)&&!reg_RX_SQ_TOP_DONE_LANE);
	if(reg_SQ_CAL_TIMEOUT_RD_LANE)
		lnx_SQ_OFST_CAL_PASS_LANE = 0;
	reg_RX_SQ_TOP_START_LANE = 0;
...
void rx_sq_thresh_cal_ram(void) BANKING_CTRL {
	...
	while((wtag=0xFC)&&!reg_RX_SQ_TOP_DONE_LANE && !timeout);
	if(timeout)*/
	delay_ram(T_p5us);
	while((wtag=0xFD)&&!reg_RX_SQ_TOP_DONE_LANE);
	...
	/*timeout = 0;
	timeout_start((uint16_t)TIMEOUT_SQ_THRESH_CAL);
	while((wtag=0xFE)&&!reg_RX_SQ_TOP_DONE_LANE && !timeout);
	if(timeout)*/
	delay_ram(T_p5us);
	while((wtag=0xFF)&&!reg_RX_SQ_TOP_DONE_LANE);
	if(reg_SQ_CAL_TIMEOUT_RD_LANE)
		lnx_SQ_THRESH_CAL_PASS_LANE = 0;
	reg_RX_SQ_TOP_START_LANE = 0;
