...
void txdcc_cal_ram_notInUse(void) BANKING_CTRL {
	...
	else if(reg_TXSPEED_DIV_LANE_2_0 == 1)
	{
		...
		while((wtag=0xD3)&&!reg_TX_ALIGN90_DCC_TOP_DONE_LANE && !timeout);
		if(timeout)
		{
			time_out_record = 1;
		}*/
		while((wtag=0xD4)&&!reg_TX_ALIGN90_DCC_TOP_DONE_LANE);
	...
	else
	{
		...
		/*timeout = 0;
		timeout_start((uint16_t)TIMEOUT_TXDCC_CAL);
		while((wtag=0xD5)&&!reg_TX_ALIGN90_DCC_TOP_DONE_LANE && !timeout);
		if(timeout)
		{
			time_out_record = 1;
		}*/
		while((wtag=0xD6)&&!reg_TX_ALIGN90_DCC_TOP_DONE_LANE);
		reg_TX_ALIGN90_DCC_TOP_START_LANE = 0;
	...
	/*timeout = 0;
	timeout_start((uint16_t)TIMEOUT_TXDCC_CAL);
	while((wtag=0xD7)&&!reg_TX_ALIGN90_DCC_TOP_DONE_LANE && !timeout);
	if(timeout)
		time_out_record = 1;*/
	while((wtag=0xD8)&&!reg_TX_ALIGN90_DCC_TOP_DONE_LANE);
	reg_TX_ALIGN90_DCC_TOP_START_LANE = 0;
...
void rx_clk_cal_ram_notInUse(void) BANKING_CTRL {
	...
	while((wtag=0xD9)&&!reg_RX_CLK_TOP_DONE_LANE && !timeout);
	if(!timeout)*/
	while((wtag=0xDA)&&!reg_RX_CLK_TOP_DONE_LANE);
...
void rx_eom_cal_ram_notInUse(void) BANKING_CTRL {
	...
	while((wtag=0xDB)&&!reg_RX_EOM_TOP_DONE_LANE && !timeout);*/
	while((wtag=0xDC)&&!reg_RX_EOM_TOP_DONE_LANE);
