...
__code uint8_t refclk_tb_01[] = { 25, 30, 40, 50, 62, 100, 125, 156 };
...
void proc_cal(void) BANKING_CTRL {
	...
	//while((wtag=0xEB)&&!reg_ANA_PROCESSMON_FCLK_RDY_RD);*/
	...
	//while((wtag=0xEC)&&!reg_ANA_PROCESSMON_FCLK_RDY_RD);
	...
	do {
		...
		while((wtag=0xED)&&!PRC_CNT_READY && !timeout);
		//timeout_stop;
#else
		do {
			PRC_CNT_START =	0;
			...
			delay_ram(T_2us);}
				//timeout_start((uint16_t)100);	//10uS timeout
		while((wtag=0xEE)&&!PRC_CNT_READY);// && !timeout);
				//timeout_stop;
#endif
		PRC_CNT_START =	0;
			...
			if(process_val_raw <= ((subss<<4) + (ss2tt<<3))) 
				{
					...
					i = 0;
					if(ss2tt!=0) {
						while((wtag=0xEF)&&process_val_cal > ss2tt){
					...
					i = 0;
					if(tt2ff!=0) {
						while((wtag=0xF0)&&process_val_cal > tt2ff){
