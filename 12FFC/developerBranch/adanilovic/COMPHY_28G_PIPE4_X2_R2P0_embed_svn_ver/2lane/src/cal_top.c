...
void Calibration(void) BANKING_CTRL {
	...
	//reg_MCU_DEBUG1_LANE_7_0 = serdes_ring_lane_en;
	...
	reg_MCU_DEBUG1_LANE_7_0 = gen_pll_rate;
	if(phy_mode==SERDES && SPDCHG_ON_READ) {
		if(gen_pll_rate!=pre_gen_pll_rate) {
			while(1){
				if(mcuid==master_mcu) break;
				if(cmx_PLL_RATE_SEL_0_VALID) break;
			}	
	...
	#endif
	{
	 ...
	 if( mcuid== master_mcu ) {
		...
		//reg_MCU_DEBUG6_LANE_7_0 = rate + 0x70;
			...
			if( phy_mode <= SAS && max_gen < 0x4 ) break;
	...
	for(genno = min_gen; genno<=max_gen; genno++)  
	{
		...
		if(phy_mode==SERDES) { //serdes need to calibrate on own speed 	 		
			...
			if(mcuid == master_mcu) cmx_PLL_RATE_SEL_0_VALID = 1;
			else cmx_PLL_RATE_SEL_1_VALID = 1;
...
void Cal_Cont(void) BANKING_CTRL {
	...
	//do {
		...
		case 5:		
			...
			case 3: if(reg_PIN_PU_TX_RD_LANE && reg_PIN_PU_RX_RD_LANE && reg_PIN_TX_IDLE_RD_LANE==0) {
...
void Cont_Cal_Power_state(void) BANKING_CTRL {
	...
	PLLDCC_CAL_STEP_SIZE = 1;
	cmx_TEMPC_STEP_CTRL_2_0 = 4;
	...
	if(mcuid==master_mcu && slave_phy_on==0) {
		if( cmx_PLL_TEMP_CAL_CONT_EN) pll_temp_cal(); 
		if( cmx_PLLDCC_CAL_CONT_EN)   plldcc_cal();	
	}
	
	if(reg_ANA_RX_PU_DLY2_LANE ==0 ) {
		cont_cal_on = 0;
		PHY_STATUS = ST_NORMAL;
		return;
	}	

	RXDCC_DATA_CAL_STEP_SIZE = 1;
	RXDCC_DLL_CAL_STEP_SIZE = 1;
	ALIGN90_CAL_STEP_SIZE = 1;     	
	lnx_DLL_VDDA_TRACKING_ON_LANE = 1;
	
	//RX
	if(cmx_ALIGN90_CAL_CONT_EN)   find_align90_lock(); 
	if( cmx_RXDLL_CAL_CONT_EN)    dll_vdda_cal(DLL_PHASE_STEP);
	rxdcc_dll_cal(); 
	rxdcc_data_cal();
