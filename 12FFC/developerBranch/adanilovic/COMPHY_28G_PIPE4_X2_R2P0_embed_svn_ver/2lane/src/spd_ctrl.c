...
void SpeedChange(){
  ...
  new_gen_rx = reg_PIN_PHY_GEN_RX_RD_LANE_3_0; new_gen_rx = (new_gen_rx<cmx_PHY_GEN_MAX_3_0)? new_gen_rx: cmx_PHY_GEN_MAX_3_0;
    ...
    //no need, ASIC/SOC has protection; data not valid after MAC change speed.  
    //#ifdef _PCIE_USB_BUILD
	//if(phy_mode == PCIE && (reg_PHY_CONFIG_1_0&0x02)) { //shared pll need phy sync 
	//	
	//	if(reg_RESET_DTX == 0) {
	//		if( mcuid==master_mcu ) {			
	//			if(slave_phy_on==0) {
	//				check_master_phy_status(CHECK_MP_SPDCHG); //wait for slave phy enter spdchg
	//			}		
	//			
	//			spdchg_on = 1;	
	//		}	
  ...
  if((new_gen_tx != gen_tx || new_gen_rx != gen_rx) && phy_mode!=SERDES ){
  ...
  ph_control_mode = 0xff; //clear ph mode to resync
  ...
  #ifdef SUPPORT_CROSS_TALK_WA
  if(cmx_LC_PLL_RATE_SEL_7_0 == cmx_RING_PLL_RATE_SEL_7_0 && ring_lane_sel) {
	  ring_pll_off();
  }	  
  #endif
...
void loadspeedtbl_ringpll(uint8_t rate) {
	...
	if(phy_mode==SERDES) rate = gen_pll_rate;
	#ifdef SUPPORT_CROSS_TALK_WA
	cmx_RING_PLL_RATE_SEL_7_0 = rate;
	#endif
...
*/
	...
	if(phy_mode!=SATA /*||PHY_STATUS!=ST_SPDCHG */|| pwrsq_on) {
	#ifndef SUPPORT_CROSS_TALK_WA
	reg_TX_CK_SEL_LANE 						= use_ring_pll|(serdes_ring_lane_en && ring_lane_sel==1);
	#endif
	...
	#ifndef _SERDES_BUILD
	if(phy_mode <= SAS) {
		train_short_mode = (uint8_t)train_save_tb[gen].train_short_en;
		//if(train_short_mode) {
		//	//train_short_mode = 1;
...
void set_ringpll(void) {
	
	ring_lane_sel = 1;
	DTL_DTX_DFE_clkoff_reset_1(1);
	pll_clk_ready_ring_0();
	loadspeedtbl_ringpll(gen_pll_rate);
	...
	reg_TX_CK_SEL_LANE = 1;
	reg_RX_CK_SEL_LANE = 1;
	if(mcuid == master_mcu) {
		cmx_PLL_CLK_SEL_0_7_0 = 1;
		cmx_PLL_RATE_SEL_0_7_0 = gen_pll_rate;
	}
	else {
		cmx_PLL_CLK_SEL_1_7_0 = 1;
		cmx_PLL_RATE_SEL_1_7_0 = gen_pll_rate;		
	}	
	pre_gen_pll_rate = gen_pll_rate;
...
void set_lcpll(void) {
	...
	reg_TX_CK_SEL_LANE = 0;
	reg_RX_CK_SEL_LANE = 0;
	if(mcuid == master_mcu) {
		cmx_PLL_CLK_SEL_0_7_0 = 0;
		cmx_PLL_RATE_SEL_0_7_0 = gen_pll_rate;
	}
	else {
		cmx_PLL_CLK_SEL_1_7_0 = 0;
		cmx_PLL_RATE_SEL_1_7_0 = gen_pll_rate;
	}	
	pre_gen_pll_rate = gen_pll_rate;
...
void pll_selection(void) {
	
	reg_MCU_DEBUG0_LANE_7_0 = 0x07;
	
	if(mcuid == master_mcu) {	
		
		if( gen_pll_rate==cmx_PLL_RATE_SEL_1_7_0) {
			reg_TX_CK_SEL_LANE = cmx_PLL_CLK_SEL_1_7_0;
			reg_RX_CK_SEL_LANE = cmx_PLL_CLK_SEL_1_7_0;
			cmx_PLL_CLK_SEL_0_7_0 = cmx_PLL_CLK_SEL_1_7_0;
			cmx_PLL_RATE_SEL_0_7_0 = gen_pll_rate;
			pre_gen_pll_rate = gen_pll_rate;
			return;	
		}				
		else if(reg_TX_CK_SEL_LANE==0) {		
				
			if(cmx_PLL_CLK_SEL_1_7_0==0 && pre_gen_pll_rate==cmx_PLL_RATE_SEL_1_7_0) {
				set_ringpll();
			}
			else {
				set_lcpll();
		...
		else if(reg_TX_CK_SEL_LANE==1) {		
				
			if(cmx_PLL_CLK_SEL_1_7_0==1 && pre_gen_pll_rate==cmx_PLL_RATE_SEL_1_7_0) {
				set_lcpll();
			}
			else {
				set_ringpll();
		...
		if( gen_pll_rate==cmx_PLL_RATE_SEL_0_7_0) {
			reg_TX_CK_SEL_LANE = cmx_PLL_CLK_SEL_0_7_0;
			reg_RX_CK_SEL_LANE = cmx_PLL_CLK_SEL_0_7_0;
			cmx_PLL_CLK_SEL_1_7_0 = cmx_PLL_CLK_SEL_0_7_0;
			cmx_PLL_RATE_SEL_1_7_0 = gen_pll_rate;
			pre_gen_pll_rate = gen_pll_rate;			
			return;
		}				
			...
			if((cmx_PLL_CLK_SEL_0_7_0==0 && pre_gen_pll_rate==cmx_PLL_RATE_SEL_0_7_0) || pwrsq_on) { 
				set_ringpll();
			}
			else {
				set_lcpll();
			...
			if(cmx_PLL_CLK_SEL_1_7_0==1 && pre_gen_pll_rate==cmx_PLL_RATE_SEL_0_7_0) { 
				set_lcpll();	
			}				
			else {
				set_ringpll();
...
void ring_pll_off(void) {
	...
	//abandon RINGPLL	
	cmx_RING_PLL_RATE_SEL_7_0 = 0xff;
	reg_ANA_PLL_CLK_READY_RING = 0;
	//rate = cmx_RING_PLL_RATE_SEL_7_0;
	//rate = rate+1; 
	//if(rate==2) rate++;
	//rate%=4;	
	//pll_clk_ready_ring_0();
	...
	//reg_RESET_ANA_RING = 0;					
	//ring_pll_cal();				
	reg_PLL_SLLP_DAC_COARSE_RING_3_0 = 0;
	reg_PLL_SLLP_DAC_VALID_RING = 1;
