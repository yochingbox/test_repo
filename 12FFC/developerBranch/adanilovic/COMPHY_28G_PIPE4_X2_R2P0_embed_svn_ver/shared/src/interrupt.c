...
void int2_isr (void) __interrupt (9) __using (2) //gen
{
	...
	if(phy_mode == SERDES) {
			reg_ANA_TX_IDLE_FORCE_LANE = 1; //workaound for Voltron
			reg_ANA_TX_IDLE_LANE = 1;	
			
			if(mcuid == master_mcu) cmx_PLL_RATE_SEL_0_VALID = 0;
			else cmx_PLL_RATE_SEL_1_VALID = 0;
	}
...
{	
	...
	if( reg_MCU_REMOTE_REQ_ISR ) {
		...
		else if(reg_PIN_PU_PLL_RD_LANE == 0) reg_MCU_LOCAL_STATUS_7_0 = reg_MCU_REMOTE_COMMAND_RD_7_0;
