...
void pll_cal( void ) BANKING_CTRL {
	...
	if(phy_mode == SERDES) {
		while(1) {
			#ifdef _LANE_RPLL
			if (lnx_PLL_CAL_RING_DONE_LANE == 1) {
				delay01(10);
				if (lnx_PLL_CAL_RING_DONE_LANE==1 && reg_PLL_CAL_EN_RING_LANE==0)
