...
void cdr_phase_maxf0p(void) BANKING_CTRL
{
	...
	while (1)
	{
		...
		{ //Phase Adjustment State
			...
			train.phase_offset_data = clamp_ram(train.phase_offset_data,cdr.max_phase_os,cdr.min_phase_os);
#else
...
#ifdef SUPPORT_EXT_MEM
	cdr.opt_phase_offset = clamp_ram(cdr.opt_phase_offset, cdr.max_phase_os, cdr.min_phase_os);
#else
...
void cdr_phase_opt_final_processing(void) BANKING_CTRL
{
	...
	#endif
	//skip_cdr = 0;
	train.phase_offset_data = cdr.opt_phase_offset + cdr_midpoint_phase_os; 
    //phase_control_func();
	dfe_adaptation();
	calculatef0p();
...
void advanced_clk_align(void) BANKING_CTRL {
	...
	adapt_slicer_en = 1;
		...
		cdr.ini_phase_offset = train.phase_offset_data;
#ifdef SUPPORT_EXT_MEM
		adapted_phase = clamp_ram( train.phase_offset_data, cdr.max_phase_os, cdr.min_phase_os);
#else
