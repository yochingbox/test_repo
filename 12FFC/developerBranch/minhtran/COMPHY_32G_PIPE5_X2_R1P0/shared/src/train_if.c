...
void request_local_status(void)
{
	uint16_t dat;
	uint8_t err_status;
	...
	if(phy_mode <= SAS &&  reg_PIN_PAPTA_TRAIN_ENABLE_RD_LANE==0) {	
		...
		//if(remote_error_check()!=_EXIT_SUCCESS) return; 	
		//#else
			while(lnx_ERROR_RESPONSE_TTIU_DETECTED_LANE){ 
...
uint8_t remote_error_check(void) BANKING_CTRL {
	...
	uint8_t remote_control_bits_hi, remote_control_bits_lo;
	...
	remote_control_bits_hi = (uint8_t)(remote_ctrl_16>>8);
	remote_control_bits_lo = (uint8_t)(remote_ctrl_16);

	if( (remote_control_bits_hi&0xe0) == 0) { //pattern check
		ce = (uint8_t)((remote_control_bits_hi>>2) & 0x03);
	}
	else if( (remote_control_bits_hi&0xe0) == 0xe0) { 
		lnx_ERROR_RESPONSE_TTIU_DETECTED_LANE = 1;
		//error code coming, do nothing and hang
	...
	//check reserve bits
	if( (((remote_control_bits_hi&0xe0)==0)&&((remote_ctrl_16 & 0x13c0) ||  //0001 0011 1100 0000
		(remote_status_16 & 0x1fc0))) || //0001 1111 1100 0000
		 (((remote_control_bits_hi&0xe0)==0xe0)&&((remote_ctrl_16 & 0x13c0) ||  //0001 0011 1100 0000
	...
	if(ce>0 && ((remote_control_bits_lo&0x3f) != 0)) {
		error_code = RESERVED_COMB_COE_REQ; 
		goto error_out;
	}	
	ce = remote_control_bits_lo & 0x3f;			
...
error_out:	
	if(error_code>0) {	
		...
		EA = 0;
		reg_LOCAL_CTRL_FIELD_LANE_15_0_b1 = (remote_control_bits_hi&0x0c)|0xe0 ; //(rm_ctrl.v&0x1fff)|0xe000; 	 
		reg_LOCAL_CTRL_FIELD_LANE_15_0_b0 = remote_control_bits_lo&0x3f;
...
bool request_local_ctrl(uint16_t dat)
{
	uint8_t err_status, stat;
	stat = _EXIT_SUCCESS;
	#ifdef _SUPPORT_TRAIN_IF_RX_DEBUG
	...
	if(phy_mode <= SAS && reg_PIN_PAPTA_TRAIN_ENABLE_RD_LANE==0) {	
		//#ifdef SUPPORT_MAIN_REMOTE_ISR_CHECK
		//if(remote_error_check()==_EXIT_SUCCESS) 
		//{ 	
		//}
		//else 
		//	stat = _EXIT_FAIL;
...
bool get_tx_status(uint8_t check)
{
	...
	timeout_start(2000); //2mS
	 do {				
		...
		//	err_status = remote_error_check();		
		//	if(err_status) continue;//wait until correct TTIU come
		//}
		//#endif
		//#else 			
		if(remote_status_in!=1) continue;		
		//#endif //SUPPORT_MAIN_REMOTE_ISR_CHECK
	...
	//reg_REMOTE_STATUS_VALID_ISR_LANE = 0;
	//#endif
	//#else //SUPPORT_MAIN_REMOTE_ISR_CHECK
	remote_status_in = 0;	
	//#endif //SUPPORT_MAIN_REMOTE_ISR_CHECK
...
void trainif_init(void)
{
	...
	if(	(reg_LOCAL_STATUS_FIELD_LANE_15_0_b1&0x40) && phy_mode != SERDES) { //!Ethernet

		reg_MCU_DEBUG0_LANE_7_0 = 0x79;
		request_local_status();  //clear local train_init
#ifdef SUPPORT_EXT_MEM
		short_delay_ram();//delay01(10);
#else
		short_delay();//delay01(10);
		...
		while((/*reg_REMOTE_STATUS_VALID_ISR_LANE==0 ||*/ (reg_REMOTE_STATUS_FIELD_RD_LANE_15_0_b1&0x40)) ){
			if(cmx_TRAIN_SIM_EN==0) {// && !tx_short_mode) {
				cds_tb = cds_table[CDS_DATAADAPT];  //dfe_acc 
				cdr_dfe_scheme();	
				if(lnx_CDS_ERR_CODE_LANE_7_0) break;
			}				
			if(reg_PIN_TX_TRAIN_ENABLE_MUX_RD_LANE==0) break;
		}					
	}	
...
void txtrain_force_restart(void) {
