...
void plldcc_cal(void) BANKING_CTRL {
	...
	do {
		...
		if( cmx_CAL_DONE == 0 && is_toggle_pat( dir, time_out_cnt - 1 ) ) { lnx_PLLDCC_CAL_PASS_LANE = 1;  break; }     //power-on, toggled
		if( cmx_CAL_DONE == 0 && time_out_cnt >= PLLDCC_LOOP_MAX && cmx_PLLDCC_CAL_TIMEOUT_DIS == 0) break;     //power-on, reached maximum rounds
