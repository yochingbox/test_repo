...
void rx_clk_cal_init_ram(void) BANKING_CTRL {
	...
	reg_RX_ALIGN90_DD_CAL_SETTING_LANE_3_0 = 0x0;//0x4;
	reg_RX_ALIGN90_DD_CAL_SETTING_LANE_11_4 = 0x18;
	reg_RX_ALIGN90_DD_CAL_SETTING_LANE_12 = 0x0;
	...
	if (reg_RX_HALFRATE_EN_LANE == 1)
	{
		...
		// IPCE_COMPHY-1564: 0x26*8+0x5=0x135
//		reg_RX_ALIGN90_DE_CAL_SETTING_LANE_2_0 = (0x135 & 0x7);
//		reg_RX_ALIGN90_DE_CAL_SETTING_LANE_10_3 = (0x135 & 0x7f8)>>3;
//		reg_RX_ALIGN90_DE_CAL_SETTING_LANE_12_11 = (0x135 & 0x1800)>>11;
		// 
//		reg_RX_ALIGN90_DD_CAL_SETTING_LANE_3_0 = 0x0;
//		reg_RX_ALIGN90_DD_CAL_SETTING_LANE_11_4 = 0x18;
//		reg_RX_ALIGN90_DD_CAL_SETTING_LANE_12 = 0x0;
	}
	...
	// 20200212 programmable default RX_ALIGN90_DE_CAL_SETTING for 56g/112g (John)
	if(reg_RX_PAM2_EN_LANE==0){
		uint8_t data = 0;
		if(reg_RX_HALFRATE_EN_LANE){// 53G
			if(force_default_de_setting_56g_rd){
				data = default_de_setting_56g;
			...
			if(force_default_de_setting_112g_rd){
				data = default_de_setting_112g;
				reg_RX_ALIGN90_DE_CAL_SETTING_LANE_10_3 = (reg_RX_ALIGN90_DE_CAL_SETTING_LANE_10_3&0xe0)|(data>>3&0x1f);
				reg_RX_ALIGN90_DE_CAL_SETTING_LANE_2_0 = data & 0x7;
			}	
		}
...
static void rx_eom_on_demand_cal_init_ram() {
    ...
    // 100us * (4-EOM_DLL_VDDA_CAL_CONT_MODE_STEPSIZE_LANE)
    // 20200226 EOM_DLL_VDDA_CAL for 1 code is 24us~48us
    // max code is 63. OK for current sequence to only consider the possibility of phase shift between two eom request
   	// and double the delay for PLL factor consideration
    //for(i=0;i<4-min_ram(reg_EOM_DLL_VDDA_CAL_CONT_MODE_STEPSIZE_LANE_2_0,4);i++){
    temp = lnx_EOM_DEBUG1_LANE_7_0;
//    timer2_start(TEST_TIMER);
    if(temp&0x7f){
    	time = (uint16_t)(temp&0x7f)*((temp>>7&0x1)?1000:100);
    }else{
    	time = 100;
    }
    timeout_start(time);
...
//    timer2_stop(TEST_TIMER);
    //delay_ram(T_100us);
	//}
	// 20200225 Jianmin: eom dll cal should be on during eom
