...
if(substr($filename,-8) eq "main.dat")
{
	$target_length=0x1fffc;
	#padding till $target_length
	die "Error: out of memory $length" if ($length>$target_length);
...
# Match r1.2 HW feature: let main.dat have checksum 1
$checksum_0 = (0xFFFFFFFF - $checksum) + 1 + 1;
#Protection for 0x100000000 case
$checksum_0 &= 0xFFFFFFFF;
...
if(substr($filename,-8) eq "main.dat"){
	...
	print FP_MEM sprintf("@%06x $data4 $data3 $data2 $data1\n",$target_length);
}
print FP_CHKSUM sprintf("0x%08x\n", ($checksum + $checksum_0)&0xFFFFFFFF);
print FP_DAT sprintf("%08x\n", $checksum_0);
