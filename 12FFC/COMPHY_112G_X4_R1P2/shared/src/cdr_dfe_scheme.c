...
__code uint32_t cds_table[]   = {
   ...
   0x420FFF, // CDS_EN_GAINTRAIN,//0x424FFF,//
   0x803F80, // CDS_EN_FINAL,
   0x803FFF, // CDS_EN_DEFAULT_PM,
   0x00F100, // CDS_EN_PHASE_TRAIN_ADV   
   0x00D100, // CDS_EN_RTPCA
   0x02C100, // CDS_EN_FINAL_MEASURE
   0x400000, // CDS_EN_TEST1
   0x420F30, // CDS_EN_RECOVER_HR
};
...
//    uint8_t finish;
    ...
    // abort dfe if it is in continuous mode
    ...
    for (p=0; p<NUM_PATH; ++p) {
        dc_sum = cds112.s.dfe_2c[p].dc[0][0] + cds112.s.dfe_2c[p].dc[1][0];
        if (dc_sum > thresh_2 || dc_sum < -thresh_2) {
            enable = 1;
            break;
        }
        dc_sum = cds112.s.dfe_2c[p].dc[0][2] + cds112.s.dfe_2c[p].dc[1][2];
        if (dc_sum > thresh_2 || dc_sum < -thresh_2) {
            enable = 1;
            break;
        }
...
//        path_en[p] = 0;
        dc_sum = cds112.s.dfe_2c[p].dc[0][0] + cds112.s.dfe_2c[p].dc[1][0];
        vref_bot_2c = cds112.s.dfe_2c[p].vref[0];
        vref_bot_sm = sav_dfe_sm.vref_bot_p1;
        if (vref_bot_2c < 0) { 
            if (dc_sum < -thresh_1 && vref_bot_2c > -63) {   // vref -1
                reg_DFE_FEXTVREF_BOT_P1_LANE_6_0 = vref_bot_sm + 1; // neg sm, +1 means -1
                reg_DFE_FENVREF_P1_LANE_2_0_b0 = 1;
                reg_DFE_FENVREF_P1_LANE_2_0_b0 = 0;
                path_en++;//[p] = 1;
        ...
        dc_sum = cds112.s.dfe_2c[p].dc[0][2] + cds112.s.dfe_2c[p].dc[1][2];
        vref_top_2c = cds112.s.dfe_2c[p].vref[2];
        vref_top_sm = sav_dfe_sm.vref_top_p1;
        if (vref_top_2c > 0) { 
            if (dc_sum < -thresh_1 && vref_top_2c > 2) {   // vref -1
                reg_DFE_FEXTVREF_TOP_P1_LANE_6_0 = vref_top_sm - 1; // pos sm, -1 means -1
                reg_DFE_FENVREF_P1_LANE_2_0_b1 = 2;
                reg_DFE_FENVREF_P1_LANE_2_0_b1 = 0;
                path_en++;//[p] = 1;
...
//        path_en[p] = 0;
        dc_sum = cds112.s.dfe_2c[p].dc[0][0] + cds112.s.dfe_2c[p].dc[1][0];
        vref_bot_2c = cds112.s.dfe_2c[p].vref[0];
        vref_bot_sm = sav_dfe_sm.vref_bot_p2;
        if (vref_bot_2c < 0) { 
            if (dc_sum < -thresh_1 && vref_bot_2c > -63) {   // vref -1
                reg_DFE_FEXTVREF_BOT_P2_LANE_6_0 = vref_bot_sm + 1; // neg sm, +1 means -1
                reg_DFE_FENVREF_P2_LANE_2_0_b0 = 1;
                reg_DFE_FENVREF_P2_LANE_2_0_b0 = 0;
                path_en++;//[p] = 1;
            } else if (dc_sum > thresh_1 && vref_bot_2c < -2) { // vref +1
                reg_DFE_FEXTVREF_BOT_P2_LANE_6_0 = vref_bot_sm - 1; // neg sm, -1 means +1    
                reg_DFE_FENVREF_P2_LANE_2_0_b0 = 1;
        ...
        dc_sum = cds112.s.dfe_2c[p].dc[0][2] + cds112.s.dfe_2c[p].dc[1][2];
        vref_top_2c = cds112.s.dfe_2c[p].vref[2];
        vref_top_sm = sav_dfe_sm.vref_top_p2;
        if (vref_top_2c > 0) { 
            if (dc_sum < -thresh_1 && vref_top_2c > 2) {   // vref -1
                reg_DFE_FEXTVREF_TOP_P2_LANE_6_0 = vref_top_sm - 1; // pos sm, -1 means -1
                reg_DFE_FENVREF_P2_LANE_2_0_b1 = 2;
                reg_DFE_FENVREF_P2_LANE_2_0_b1 = 0;
                path_en++;//[p] = 1;
            } else if (dc_sum > thresh_1 && vref_top_2c < 63) { // vref +1
                reg_DFE_FEXTVREF_TOP_P2_LANE_6_0 = vref_top_sm + 1; // pos sm, +1 means +1    
                reg_DFE_FENVREF_P2_LANE_2_0_b1 = 2;
...
//        path_en[p] = 0;
        dc_sum = cds112.s.dfe_2c[p].dc[0][0] + cds112.s.dfe_2c[p].dc[1][0];
        vref_bot_2c = cds112.s.dfe_2c[p].vref[0];
        vref_bot_sm = sav_dfe_sm.vref_bot_p3;
        if (vref_bot_2c < 0) { 
            if (dc_sum < -thresh_1 && vref_bot_2c > -63) {   // vref -1
                reg_DFE_FEXTVREF_BOT_P3_LANE_6_0 = vref_bot_sm + 1; // neg sm, +1 means -1
                reg_DFE_FENVREF_P3_LANE_2_0_b0 = 1;
                reg_DFE_FENVREF_P3_LANE_2_0_b0 = 0;
                path_en++;//[p] = 1;
            } else if (dc_sum > thresh_1 && vref_bot_2c < -2) { // vref +1
                reg_DFE_FEXTVREF_BOT_P3_LANE_6_0 = vref_bot_sm - 1; // neg sm, -1 means +1    
                reg_DFE_FENVREF_P3_LANE_2_0_b0 = 1;
        ...
        dc_sum = cds112.s.dfe_2c[p].dc[0][2] + cds112.s.dfe_2c[p].dc[1][2];
        vref_top_2c = cds112.s.dfe_2c[p].vref[2];
        vref_top_sm = sav_dfe_sm.vref_top_p3;
        if (vref_top_2c > 0) { 
            if (dc_sum < -thresh_1 && vref_top_2c > 2) {   // vref -1
                reg_DFE_FEXTVREF_TOP_P3_LANE_6_0 = vref_top_sm - 1; // pos sm, -1 means -1
                reg_DFE_FENVREF_P3_LANE_2_0_b1 = 2;
                reg_DFE_FENVREF_P3_LANE_2_0_b1 = 0;
                path_en++;//[p] = 1;
            } else if (dc_sum > thresh_1 && vref_top_2c < 63) { // vref +1
                reg_DFE_FEXTVREF_TOP_P3_LANE_6_0 = vref_top_sm + 1; // pos sm, +1 means +1    
                reg_DFE_FENVREF_P3_LANE_2_0_b1 = 2;
...
//        path_en[p] = 0;
        dc_sum = cds112.s.dfe_2c[p].dc[0][0] + cds112.s.dfe_2c[p].dc[1][0];
        vref_bot_2c = cds112.s.dfe_2c[p].vref[0];
        vref_bot_sm = sav_dfe_sm.vref_bot_p4;
        if (vref_bot_2c < 0) { 
            if (dc_sum < -thresh_1 && vref_bot_2c > -63) {   // vref -1
                reg_DFE_FEXTVREF_BOT_P4_LANE_6_0 = vref_bot_sm + 1; // neg sm, +1 means -1
                reg_DFE_FENVREF_P4_LANE_2_0_b0 = 1;
                reg_DFE_FENVREF_P4_LANE_2_0_b0 = 0;
                path_en++;//[p] = 1;
            } else if (dc_sum > thresh_1 && vref_bot_2c < -2) { // vref +1
                reg_DFE_FEXTVREF_BOT_P4_LANE_6_0 = vref_bot_sm - 1; // neg sm, -1 means +1    
                reg_DFE_FENVREF_P4_LANE_2_0_b0 = 1;
        ...
        dc_sum = cds112.s.dfe_2c[p].dc[0][2] + cds112.s.dfe_2c[p].dc[1][2];
        vref_top_2c = cds112.s.dfe_2c[p].vref[2];
        vref_top_sm = sav_dfe_sm.vref_top_p4;
        if (vref_top_2c > 0) { 
            if (dc_sum < -thresh_1 && vref_top_2c > 2) {   // vref -1
                reg_DFE_FEXTVREF_TOP_P4_LANE_6_0 = vref_top_sm - 1; // pos sm, -1 means -1
                reg_DFE_FENVREF_P4_LANE_2_0_b1 = 2;
                reg_DFE_FENVREF_P4_LANE_2_0_b1 = 0;
                path_en++;//[p] = 1;
            } else if (dc_sum > thresh_1 && vref_top_2c < 63) { // vref +1
                reg_DFE_FEXTVREF_TOP_P4_LANE_6_0 = vref_top_sm + 1; // pos sm, +1 means +1    
                reg_DFE_FENVREF_P4_LANE_2_0_b1 = 2;
...
/*        if (finish) break;*/
        ...
        reg_DFE_ADAPT_SPLR_EN_LANE_7_0 = 0xFF;
...
void dc_vref_adj(uint8_t transfer_lpnum, uint8_t dfe_lpnum) BANKING_CTRL
{
    ...
    uint8_t splr_dis;
    uint8_t splr_done;
    uint8_t thresh_1, thresh_2;
    int8_t dc_sum;
    uint8_t path_en[NUM_PATH] = {0};
    uint8_t finish;
    ...
    for (j=0; j<transfer_lpnum; ++j) {
    	...
    	cds_save(Savvref);
    	cds_save(Savdc);
		// Check if dc_sum is below thresh_1
        finish = 1;
        for (p=0; p<NUM_PATH; ++p) {
            path_en[p] = 0;
            for (tmb=0; tmb<NUM_TMB; ++tmb) {
                dc_sum = cds112.s.dfe_2c[p].dc[0][tmb] + cds112.s.dfe_2c[p].dc[1][tmb];
                if (dc_sum > thresh_1 || dc_sum < -thresh_1) {
                    path_en[p] = 1;
                    break;
                }
            }
            if (!path_en[p])    splr_done |= 0x11 << p;
            else                finish = 0;
        }
#if DC_VREF_ADJ_SPEED_UP==1
        if (finish) break;
#endif
        splr_dis = reg_CDS_ADAPT_SPLR_DIS_LANE_7_0; // disable sampler for different rate mode

        // disable dc_vref transfer for path with vref_bot > -3 when vref_bot_inc or vref_top < 3 when vref_top_dec
        // vref_bot_inc when dc_bot_sum > threshold
        // vref_top_dec when dc_top_sum < -threshold
        // if vref==3, vref-2=1, vref_pos or vref_neg equal to 0
        for (p=0; p<NUM_PATH; ++p) {
            if ((cds112.s.dfe_2c[p].vref[0] > -4 && (cds112.s.dfe_2c[p].dc[0][0] + cds112.s.dfe_2c[p].dc[1][0]) > 0) ||   // vref_bot>-4 & dc_d_bot+dc_s_bot>0
                (cds112.s.dfe_2c[p].vref[2] < 4  && (cds112.s.dfe_2c[p].dc[0][2] + cds112.s.dfe_2c[p].dc[1][2]) < 0))     // vref_top< 4 & dc_d_top+dc_s_top<0    

                splr_dis |= 0x11 << p;
        }

        // DC VREF transfer for 1 DC code and 2 VREF code for each sampler
        // if i==0, dc_d_p1 and vref_p1 transfer 1 code
        // if i==4, dc_s_p1 and vref_p1 transfer 1 code
        // In total, for P1, dc change 1 code, vref change 2 codes
        reg_DFE_ADAPT_ADJ_VREF_DC_EN_LANE = 1;
        for (i=0; i<NUM_PATH*NUM_DS; ++i) {
            reg_DFE_ADAPT_SPLR_EN_LANE_7_0 = 1 << i;
            reg_DFE_ADAPT_SPLR_EN_LANE_7_0 &= ~splr_dis;    // splr with vref approach boundary
#if DC_VREF_ADJ_SPEED_UP==1
            reg_DFE_ADAPT_SPLR_EN_LANE_7_0 &= ~splr_done;   // splr with dc_vref transfer done
#endif
            if(reg_DFE_ADAPT_SPLR_EN_LANE_7_0==0)
                continue;
            dfe_fsm(0, 0);  // adapt_lpnum = 0 to ensure only 1 code transfer in each DFE call
        }
	    reg_DFE_ADAPT_ADJ_VREF_DC_EN_LANE = 0;
...
void cds_load_vref() BANKING_CTRL
{
    ...
    reg_DFE_FEXTVREF_BOT_P1_LANE_6_0   = val;
...
void dfe_fsm(uint16_t lpnum, uint16_t tap_en) BANKING_CTRL
{
	...
	// 20200206 Mo: Updated dfe_fsm() to prevent vref cross 0 boundary
    ...
    if (reg_DFE_UPDATE_VREF_EN_LANE) 
    {
        ...
        if (cds112.s.dfe_2c[0].vref[0] > -2) 
        {
            reg_DFE_FEXTVREF_BOT_P1_LANE_6_0 = 0x42;
            reg_DFE_FENVREF_P1_LANE_2_0_b0 = 1;
            reg_DFE_FENVREF_P1_LANE_2_0_b0 = 0;
        }
        if (cds112.s.dfe_2c[0].vref[2] <  2) 
        {
            reg_DFE_FEXTVREF_TOP_P1_LANE_6_0 =  0x2;
            reg_DFE_FENVREF_P1_LANE_2_0_b1 = 2;
            reg_DFE_FENVREF_P1_LANE_2_0_b1 = 0;
        }
        if (cds112.s.dfe_2c[1].vref[0] > -2) 
        {
            reg_DFE_FEXTVREF_BOT_P2_LANE_6_0 = 0x42;
            reg_DFE_FENVREF_P2_LANE_2_0_b0 = 1;
            reg_DFE_FENVREF_P2_LANE_2_0_b0 = 0;
        }
        if (cds112.s.dfe_2c[1].vref[2] <  2) 
        {
            reg_DFE_FEXTVREF_TOP_P2_LANE_6_0 =  0x2;
            reg_DFE_FENVREF_P2_LANE_2_0_b1 = 2;
            reg_DFE_FENVREF_P2_LANE_2_0_b1 = 0;
        }
        if (cds112.s.dfe_2c[2].vref[0] > -2) 
        {
            reg_DFE_FEXTVREF_BOT_P3_LANE_6_0 = 0x42;
            reg_DFE_FENVREF_P3_LANE_2_0_b0 = 1;
            reg_DFE_FENVREF_P3_LANE_2_0_b0 = 0;
        }
        if (cds112.s.dfe_2c[2].vref[2] <  2) 
        {
            reg_DFE_FEXTVREF_TOP_P3_LANE_6_0 =  0x2;
            reg_DFE_FENVREF_P3_LANE_2_0_b1 = 2;
            reg_DFE_FENVREF_P3_LANE_2_0_b1 = 0;
        }
        if (cds112.s.dfe_2c[3].vref[0] > -2) 
        {
            reg_DFE_FEXTVREF_BOT_P4_LANE_6_0 = 0x42;
            reg_DFE_FENVREF_P4_LANE_2_0_b0 = 1;
            reg_DFE_FENVREF_P4_LANE_2_0_b0 = 0;
        }
        if (cds112.s.dfe_2c[3].vref[2] <  2) 
        {
            reg_DFE_FEXTVREF_TOP_P4_LANE_6_0 =  0x2;
            reg_DFE_FENVREF_P4_LANE_2_0_b1 = 2;
            reg_DFE_FENVREF_P4_LANE_2_0_b1 = 0;
        }
        while(!reg_DFE_UPDATED_LANE);
...
void cds_init(void) BANKING_CTRL
{
   ...
   Tdc_accu              = 0x3FF;
    ...
    // cds part of midas missing issue
//	reg_EO_BASED_LANE = 0x1; // as script and midas
//	reg_CDS_F2T_VAL_LANE_3_0 = 0x1; // not 6, IPCE_COMPHY-1657 (20200106)
//	reg_CDS_F2T_VAL_FORCE_LANE = 0x1; // as midas
//	reg_CDS_F0K_RES_CUSTOM_LANE_1_0 = 0x3; // not 2, previously changed by script
//	reg_CDS_VREF_VAL_LANE_5_0 = 32;
...
void dfe_backon(void) BANKING_CTRL {
	...
	// Jackson: want DC_to_VREF Adjustment to be called in the beginning of the DFE duty cycle control when the DFE adaptation wakes up. This will ensure that DFE DC will not saturate.
	if(dfe_backon_with_dc_vref_adj_rd)
