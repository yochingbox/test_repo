...
void cdr_phase_opt(void)
{
	...
	//DBG_PS("\r\n++(cdr_phase_opt) ");
	#endif
lnx_TRAIN_DEBUG5_LANE_7_0 = 0x03;	
//Grace requested to comment out the if branch*********************
...
*/
	...
	cdr_phase_opt_init();
lnx_TRAIN_DEBUG5_LANE_7_0 = 0x04;
#if (tag_CDR_MAXF0P_EN)  //1
	cdr_phase_maxf0p(); 
#endif	
lnx_TRAIN_DEBUG5_LANE_7_0 = 0x05;
if (tag_CDR_MIDPOINT_EN)  //1
	cdr_phase_midpoint(); 
//#endif	
lnx_TRAIN_DEBUG5_LANE_7_0 = 0x06;
    cdr_phase_opt_final_processing();
	
lnx_TRAIN_DEBUG5_LANE_7_0 = 0x07;
}
...
//record_train_result(0);
			...
			phase_updated = 0;
			if ((train.level > cdr.opt_level) || ((train.level == cdr.opt_level) && (((cdr.train_f0p > cdr.opt_f0p) && train.eye_check_pass)))) {
				phase_updated = 1;
			}
...
void cdr_phase_opt_final_processing(void)
{
	...
	#else
	if ((tag_CDR_MIDPOINT_EN == 0 || !tag_CDR_EYE_WIDTH_EN) && (tag_SAME_SET_NO_UPDATE || cdr.opt_level > train.level)) //tag_CDR_MIDPOINT_EN=1
	{
