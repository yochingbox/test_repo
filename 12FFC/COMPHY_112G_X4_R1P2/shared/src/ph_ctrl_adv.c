...
#endif
#ifdef OPT_20200218
// MUST match PH_TRAIN_CTRL_t
__code uint8_t PH_TRAIN_CTRL_MAP[] = {0xa,0x5,0xa,0x5,1,2,4,8,0xf};
#endif
#ifdef OPT_20200218
uint16_t compute_avg_f0d(PH_TRAIN_CTRL_t control) BANKING_CTRL 
{
	...
	for (p_idx = 0; p_idx < NUM_PATH; p_idx++){
		if(PH_TRAIN_CTRL_MAP[control]>>p_idx&0x1){
			sum_f0d += train.f0d_tmb[p_idx][0] + train.f0d_tmb[p_idx][1];
		}
	}
#ifdef _PH_TRAIN_ADV_DEBUG
...
uint16_t compute_f1_fn1_h(int8_t f1, int8_t fn1) BANKING_CTRL 
{
    ...
    uint8_t a = 1;
    uint8_t b = 0;
    uint16_t f1_minus_kfn1;

    //DBG_Printf("\r\nhi"); 
    //abs_f1_minus_kfn1 = abs(f1 - (a*fn1) >> b);
	f1_minus_kfn1 = (f1 - (a*fn1) >> b);
    //DBG_Printf("\r\n,***** abs(%d - %d) = %d ***** ", f1, fn1, abs_f1_minus_kfn1); 
           
    return f1_minus_kfn1; 
}
...
uint16_t compute_f1_fn1(PH_TRAIN_CTRL_t control) BANKING_CTRL
{
	...
	for (p_idx = 0; p_idx < NUM_PATH; p_idx++){
		if(PH_TRAIN_CTRL_MAP[control]>>p_idx&0x1){
			if(control>=P1_DtoS && control<=P4_DtoS)
				temp = compute_f1_fn1_h(train.f1_tmb[p_idx][0], train.fn1_tmb[p_idx][0])
                	- compute_f1_fn1_h(train.f1_tmb[p_idx][1], train.fn1_tmb[p_idx][1]);
			else if(control==P2P4_DCC || control==P1P3_DCC)
				temp = compute_f1_fn1_h(train.f1_tmb[p_idx][0], train.fn1_tmb[p_idx][0])
                	+ compute_f1_fn1_h(train.f1_tmb[p_idx][1], train.fn1_tmb[p_idx][1]);
            else
            	temp = abs16(compute_f1_fn1_h(train.f1_tmb[p_idx][0], train.fn1_tmb[p_idx][0]))
                	+ abs16(compute_f1_fn1_h(train.f1_tmb[p_idx][1], train.fn1_tmb[p_idx][1]));
            // rxdcc metric uses Pn's f1-fn1 minus Pn+2's f1-fn1
            if(control==P2P4_DCC&&p_idx==3 || control==P1P3_DCC&&p_idx==2)
    			sum_abs_f1_minus_kfn1 -= temp;
            else
				sum_abs_f1_minus_kfn1 += temp;
		}
	}
...
#endif
    return abs16(sum_abs_f1_minus_kfn1);
}
#else
uint16_t compute_f1_fn1(PH_TRAIN_CTRL_t control) BANKING_CTRL 
{
...
void optimize_ckpx_tune(uint8_t x, PH_TRAIN_MODE_t train_mode, PH_TRAIN_PATH_SEL_t path_sel) BANKING_CTRL
{
    ...
    {
        case 1:
            setting = ph_train_opt.ckp1_tune_code_lane_4_0;
            break;
        case 2:
            setting = ph_train_opt.ckp2_tune_code_lane_4_0;
            break;
        case 3:
            setting = ph_train_opt.ckp3_tune_code_lane_4_0;
            break;
        case 4:
            setting = ph_train_opt.ckp4_tune_code_lane_4_0;
            break;
    }
	set_ckpx_tune_reg(x, setting);
...
#define OPTIMIZE_DD_MAX_CNT 4 //5//20
#define OPTIMIZE_DE_MAX_CNT 6 //5//20

// 20200130 programmable apt_ded_sweep_down_first
void sweep_align90_dd_down(PH_TRAIN_MODE_t train_mode, PH_TRAIN_PATH_SEL_t path_sel, uint8_t start_dd) BANKING_CTRL
{
	int8_t step_size = 1;
    ...
    uint8_t cnt = 0;
    uint8_t max_cnt = OPTIMIZE_DD_MAX_CNT;//20;
...
void optimize_rx_align90_dd(PH_TRAIN_MODE_t train_mode, PH_TRAIN_PATH_SEL_t path_sel) BANKING_CTRL
{
	...
	if(apt_dd_sweep_down_first_rd)
		sweep_align90_dd_down(train_mode, path_sel, start_dd);
    ...
    if(apt_dd_sweep_down_first_rd==0)
		sweep_align90_dd_down(train_mode, path_sel, start_dd);
...
// 20200130 programmable apt_de_sweep_down_first
void sweep_align90_de_down(PH_TRAIN_MODE_t train_mode, PH_TRAIN_PATH_SEL_t path_sel, uint8_t start_dd, uint8_t start_de) BANKING_CTRL
{
	int8_t step_size = 1;
    ...
    uint8_t cnt = 0;
    uint8_t max_cnt = OPTIMIZE_DE_MAX_CNT;//20;
...
void optimize_rx_align90_de(PH_TRAIN_MODE_t train_mode, PH_TRAIN_PATH_SEL_t path_sel) BANKING_CTRL
{
    ...
    uint8_t cnt = 0;
    uint8_t max_cnt = OPTIMIZE_DE_MAX_CNT;//5;//20;
	...
	if(apt_de_sweep_down_first_rd)
		sweep_align90_de_down(train_mode, path_sel, start_dd, start_de);
...
/*            save_rx_align90_dd(&ph_train_opt);*/
    ...
    if(apt_de_sweep_down_first_rd==0)
    	sweep_align90_de_down(train_mode, path_sel, start_dd, start_de);
...
/*    record_train_result(1);*/
    check_pause_state(PAUSE_ST_TEST_A);
...
/*            }*/
            check_pause_state(PAUSE_ST_TEST_B);      
            optimize_rx_align90_dd(train_mode, path_sel);
/*            if(outer_loop==0){*/
/*            	record_train_result(3);*/
/*            }*/
            check_pause_state(PAUSE_ST_TEST_C);
        }
//        for (i = 0; i < 2; i++)
        {
...
//            optimize_rx_align90_dd(train_mode, path_sel);
/*            if(outer_loop==0){*/
/*            	record_train_result(5);*/
/*            }*/
//          	check_pause_state(PAUSE_ST_TEST_E);
        }
//        for (i = 0; i < 2; i++)
        {
//            optimize_rx_dcc1or4(1, train_mode, path_sel); // dcc_p2p4
//            optimize_rx_dcc1or4(4, train_mode, path_sel); // dcc_p1p3
        ...
        if(bypass_optimize_ckpx_tune_rd==0){
//		    for (i = 0; i < 2; i++)
		    {
		    	for(j=1;j<=4;j++)
		        	optimize_ckpx_tune(j, train_mode, path_sel);
...
void phase_train_qr() BANKING_CTRL{
	int8_t metric = 0;
	uint8_t setting_de = convert_rx_align90_de_reg_to_code();
	int8_t i = 0;
...
//	set_dfe_cont(0);
	...
	for(i=0;i<8;i++){
		...
		metric = (int8_t)(train.fn1-train.f1);
		if(metric>0){// increase
			if(setting_de<0xff)
				setting_de++;
		}else{ // decrease
			if(setting_de>0)
...
// 20200226
void record_f1_fn1(uint8_t index) BANKING_CTRL
{
#if 0
	uint8_t p_idx,ds_idx;
//	index += 12;
	...
	for(p_idx=0;p_idx<4;p_idx++)
		for(ds_idx=0;ds_idx<2;ds_idx++)
		{
			dsp_debug[index++] = train.f1_tmb[p_idx][ds_idx];
			dsp_debug[index++] = train.fn1_tmb[p_idx][ds_idx];
		}
			
	dsp_debug[index++] = train.f1;
	dsp_debug[index++] = train.fn1;
	dsp_debug[index++] = convert_rx_align90_de_reg_to_code();
#endif
}
