...
uint32_t get_program_memory_checksum() BANKING_CTRL{
	...
	uint32_t temp = 0;
	for(i=0;;i+=4){
		temp = (uint32_t)get_program_memory_bank1(i+2)<<16;
		temp |= (uint32_t)get_program_memory_bank1(i);
		check_sum+=temp;
		if(i==0xfffc)
			break;
	}
	for(i=0;i<0x8000;i+=4){
		temp = (uint32_t)get_program_memory_bank2(i+0x8000+2)<<16;
		temp |= (uint32_t)get_program_memory_bank2(i+0x8000);
		check_sum+=temp;
	}
#ifdef _12FFC_COMPHY_112G_X4_R1P2
	for(i=0;i<0x8000;i+=4){
		temp = (uint32_t)get_program_memory_bank3(i+0x8000+2)<<16;
		temp |= (uint32_t)get_program_memory_bank3(i+0x8000);
		check_sum+=temp;
	}
	// HW req that main.dat has checksum 1
	check_sum--;
#endif
	return check_sum;
}
...
//	tag_TRAIN_PH_OS_ESM = (int8_t)lnx_TRAIN_PH_OS_ESM_2C_LANE_7_0;
    ...
    case 0x55: dc_vref_adj(40,0xf07); break;

	case 0xb0:{
#ifdef SUPPORT_PRBS_CHECKER	
	// phy test, assume prbs checker init has run (1e10 bits)
...
#define reg_PT_CNT_MAX_LANE_31_0    PT_COUNTER5.VAL
	...
	case 0xd1:{// get program memory by addr 0~0x17fff
		uint16_t addr = tag_CLI_ARGS;
		if(tag_CLI_ARG2==0)
			lnx_CLI_RETURN_LANE_15_0 = get_program_memory_bank1(addr);
		else if(tag_CLI_ARG2==1)
			lnx_CLI_RETURN_LANE_15_0 = get_program_memory_bank2(addr+0x8000);
		break;
	}
	case 0xd2:{
//		uint16_t value = (uint16_t)script_init;
		volatile uint32_t value = 0x44332211;
		//volatile uint32_t value[32] = {0x44332211};
		//value[31] = 0x88776655;
		#if 0
		__asm
		mov 	r1,#_script_init
		mov 	r2,#(_script_init>>8)
		
		// cannot embed marker?
		//lnx_CLI_RETURN_LANE_15_0 = data;
		mov		dptr,#(_CLI_ARG + 0x0002)
		mov		a,r1
		movx	@dptr,a
		inc		dptr
		mov		a,r2
		...
		movx	@dptr,a
		__endasm;
//		lnx_CLI_RETURN_LANE_15_0 = tag_CLI_ARGS;//*(uint16_t*)value;
//		lnx_CLI_RETURN_LANE_15_0 = get_program_memory_bank2(tag_CLI_ARGS);
//		lnx_CLI_RETURN_LANE_15_0 = *(uint16_t*)tag_CLI_ARGS;
		break;
	}
	case 0xd3:{
		timer2_start(TEST_TIMER);
		// get checksum
		tag_CLI_ARGS = get_program_memory_checksum();
		timer2_stop(TEST_TIMER);
#if 0
		__asm
		//mov 	r1,#_script_init
		//mov 	r2,#(_script_init>>8)
		mov 	dptr,#_script_init
		clr a
		movc	a,@a+dptr
		mov		r1,a
...
#if _DBG_CMN_TIMER
	case 0xd4:{
		reg_MCU_DEBUG_CMN_2_7_0 = tag_CLI_ARG0;
		while(reg_MCU_DEBUG_CMN_3_7_0);
		timer2_start(TEST_TIMER);
		while(reg_MCU_DEBUG_CMN_3_7_0!=reg_MCU_DEBUG_CMN_2_7_0);
		timer2_stop(TEST_TIMER);
		reg_MCU_DEBUG_CMN_2_7_0 = 0;	
		break;
	}
#endif
#if 0
	case 0xd5:{
		reg_MCU_DEBUG_CMN_4_7_0 = 0;
		reg_MCU_DEBUG_CMN_5_7_0 = 0;
		
		reg_MCU_DEBUG_CMN_2_7_0 = tag_CLI_ARG0;
		timeout2_start(TEST_TIMER, (uint32_t)10000);
		while(!timeout2_check(TEST_TIMER));
		reg_MCU_DEBUG_CMN_2_7_0 = 0;
		timer2_stop(TEST_TIMER);
		break;
	}
