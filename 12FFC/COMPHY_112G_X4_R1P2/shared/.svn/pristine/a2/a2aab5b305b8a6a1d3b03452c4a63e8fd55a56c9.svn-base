...
lnx_TRAIN_DEBUG4_LANE_7_0 = 0x51;
	while(1)
	{
		...
		count++;	
		if(0x51+count<0x5d)
			lnx_TRAIN_DEBUG4_LANE_7_0=0x51+count;
	}
lnx_TRAIN_DEBUG4_LANE_7_0 = 0x5e;
	tx_train_end();
lnx_TRAIN_DEBUG4_LANE_7_0 = 0x5f;
}
...
void req_local_ctrl(uint8_t ce, uint8_t c1, uint8_t c0, uint8_t cn1, uint8_t cn2)
{
	...
	// 20200715 Xu: mainly for first requests
//	if(0){// lnx_TRAIN_DEBUG4_LANE_7_0==0x28){// need another condition
//		drv_TXTRAIN_CTRL_PAT = 0;
//	}else
	{
...
{
	uint32_t temp = 0;
	// Debug drv_TXTRAIN_STATUS_VALID
	temp = MCU_DEBUG_LANE.VAL;
	temp = temp<<1;
	temp |= drv_TXTRAIN_STATUS_VALID;
	MCU_DEBUG_LANE.VAL = temp;
	lnx_MCU_RESERVEDX23_LANE_7_0++;
}
#endif
	...
	if(reg_ETHERNET_MODE_LANE_1_0 >= 2) { //pam4
		...
		// 20200312 Should not comment out: single fir min/max need to be detected correctly
...
void tx_ffe_train(void)
{
	...
	#endif 
	{
				...
				((train.eo < tag_TX_EO_BYPASS_THRES) || train.saturated))
		{
			...
			lnx_TRAIN_DEBUG4_LANE_7_0 = 0x61;
			record_train_result(13);
...
void tx_gn1_adjust(void) {
	...
	DBG_PS("\r\n	gn1_adjust");
	#endif
	lnx_TRAIN_DEBUG7_LANE_7_0=1;
	for(i=0; i<tag_TX_GN1_STEP_NUM; i++) {
lnx_TRAIN_DEBUG7_LANE_7_0++;
		cds_call = CDS_EN_PHASETRAIN2;
		dfe_cdr_phase_opt();
lnx_TRAIN_DEBUG7_LANE_7_0++;
		#if _DEBUG
...
lnx_TRAIN_DEBUG7_LANE_7_0 = 0xff;
	train2opt();
}	
