...
//#endif
...
#endif
#endif
	// Single mode sequence: assume Set TSEN_ADC_RD_REQ to 0, Set TSEN_ADC_START to 0
	delay_ram(T_10us);// protection from last ready
	reg_TSEN_ADC_RD_REQ = 1;
	...
	cmx_TSEN_DAT_CEL_7_0 = ((int32_t)dat*394 + 128900 + 500)/1000;
//	dat = ((int32_t)dat*394 + 128900 + 500)/125;//((int32_t)dat*394 + 128900 + 500)/1000;
//	cmx_TSEN_DAT_CEL_7_0 = dat>>3&0xff;
//	cmx_MCU_DEBUG_CMN_23_7_0 = dat&0x7;
	return dat;
}
#endif
// return 0x7fff if not updated
// split into states for faster response
int16_t read_tsen_ram(void) BANKING_CTRL  {
	int16_t dat = 0x7fff;
	switch(tsen_state){
	case 0://request
...
// mcu_lane should only call this
int8_t read_tsen_cel_ram(void) BANKING_CTRL  {
	return cmx_TSEN_DAT_CEL_7_0;
}
...
void timeout2_stop(uint8_t timer_index) BANKING_CTRL{
	...
	if(timeout2_running(timer_index)){
		...
		case TRAINING_TIMER:
			...
			reg_MCU_DEBUG7_LANE_7_0 = time>>24&0xff;
			break;
		#if 1
		case TEST_TIMER:
...
#define trap_reg lnx_CLI_DEBUG2_LANE_7_0 
	EA = 0;
	if(trap_reg == trap_number){
//		DBG_Printf("\r\nEntered trap number %d\r\n", trap_number);
		trap_reg = ~trap_reg;
		while((uint8_t)(~trap_reg) == trap_number);
//		DBG_Printf("\r\nExited trap number %d\r\n", trap_number);
	}
	EA = 1;
#endif
}
