...
void dll_vdda_cal( uint8_t mode ) BANKING_CTRL {
	uint8_t i, vref_rxdll_sel_ofst, val, loopcnt, dir, cmpout;
	...
	if( lnx_DLL_VDDA_TRACKING_ON_LANE == 0 && mode == DLL_VDDA_STEP) { 		
		...
		set_sellv_rxdll_ch_ram(i+0x0d);
#else
		VREF_RXDLL_SEL = get_vref_rxdll_sel();
		set_sellv_rxdll_ch_ram(VREF_RXDLL_SEL*2 + 7);
#endif
...
void dll_eom_vdda_cal( uint8_t mode ) BANKING_CTRL {
	...
	if( lnx_DLL_VDDA_TRACKING_ON_LANE==0 && mode == DLL_VDDA_STEP) { 
		...
		VREF_RXDLL_SEL = i + vref_rxdll_sel_ofst; 
		set_sellv_rxeomdll_ch_ram(i+0x0d);
#else
		VREF_RXDLL_SEL = get_vref_rxdll_sel(); 
		set_sellv_rxeomdll_ch_ram(VREF_RXDLL_SEL*2 + 7);
#endif
		delay01(10);
		loopcnt = 128;
	}
...
uint8_t get_sellv_rxeomdll_ch_ram(void) BANKING_CTRL {
    uint8_t val;    
    val = reg_SELLV_RXEOMDLL_CH1_5_0;    
    return val;
}     
