 ...
 *    Output: 
*/
#ifdef SUPPORT_POWER_SIMPLE // false
...
void PowerUp_Seq_Cal(void) BANKING_CTRL {
	...
	//while(reg_ANA_PU_PLL_DLY_LANE==0 && reg_ANA_PU_PLL_LANE==0);
	while(!reg_PIN_PU_PLL_RD_LANE);
...
//	while(!reg_PIN_PU_PLL_RD_LANE);
	...
	// Peng: for tx idle
	reg_ANA_TRX_PLL_TXCLK_SYNC_EN_LANE = 1;
	reg_ANA_TRX_PLL_TXCLK_SYNC_EN_FORCE_LANE = 1;
	reg_ANA_TX_TXCLK_SYNC_EN_LANE = 1;
	reg_ANA_TX_TXCLK_SYNC_EN_FORCE_LANE = 1;
...
void PowerUp_Seq_CMN() BANKING_CTRL {
	uint8_t data = 0;
	cmx_CAL_DONE = 0;
#ifdef _12FFC_COMPHY_112G_X4_R1P2
	reg_ANA_REG_SU_DLY_SEL_2_0 = 3;
	...
	set_timer_cnt();
	// do not check memory at FAST_POWER_ON_EN
	data = reg_MCU_DEBUG_CMN_1_7_0;
	if(!FAST_POWER_ON_EN && (data>>7&0x1)==0) {
		data&=~0x3;
		if(get_program_memory_checksum())// checksum not 0
			data|=0x3;
		else// checksum 0
			data|=0x1;
		reg_MCU_DEBUG_CMN_1_7_0 = data;
	}
...
void clear_xdata(void) BANKING_CTRL {
	...
	switch(mcuid){
		case MCU_CMN:
			for(i=0;i<4;i++)
				mcu_response_status[i] = 0;
			tsen_state = 0;
			break;
		default: mcu_request_lane[mcuid] = 0;break;
	}
...
// TODO: NOTE/ALERT: ifdef SUPPORT_PRBS_CHECKER, and it is possible to reset during the usage, this line shoud be enabled with another proper save_pt_en_lane init somewhere else. Or use another bit to guard the restoration.
...
// Remove as FW does not touch in change 0.10.12.8
 ...
 *    
*/
// Currently called before pu_pll:
// Easier for user overwriting sequence
// CMN regs decoupled with pu_plls
void init_xdata(void) BANKING_CTRL {
	...
	//pllcal_first_time = 1;
	// Should be only in cmn and max_gen is in lane speedtable
	if(mcuid==MCU_CMN)
		cmx_PHY_GEN_MAX_4_0 = 0x1f;
...
//	lnx_PT_OUT_TIME_TRAIN_COMP_LANE_15_0 = 1000; //TODO: remove. update xdat_lane
	...
	ctle_res1_sel_min = 0;
	ctle_res1_sel_max = 0xff;
	ctle_cap1_sel_min = 0;
	ctle_cap1_sel_max = 0xff;
	ctle_rl2_tune_min = 0;
	ctle_rl2_tune_max = 0xff;
	ctle_cap2_sel_min = 0;
	ctle_cap2_sel_max = 0xff;
#endif
...
//	}
	...
	reg_SELLV_TXCLK_CH0_3_0 = 9;
	reg_SELLV_TXCLK_CH1_3_0 = 9;
	reg_SELLV_TXCLK_CH2_3_0 = 9;
	reg_SELLV_TXCLK_CH3_3_0 = 9;
	reg_SELLV_TXDATA_CH0_3_0 = 9;
	reg_SELLV_TXDATA_CH1_3_0 = 9;
	reg_SELLV_TXDATA_CH2_3_0 = 9;
	reg_SELLV_TXDATA_CH3_3_0 = 9;
	
	// IPCE_COMPHY-1657
	reg_CDS_F2T_VAL_LANE_3_0 = 0x1; // not 6, IPCE_COMPHY-1657 (20200106)
	reg_CDS_F2T_VAL_FORCE_LANE = 0x1; // as midas
	{
	uint8_t data = reg_DFE_SAT_EN_LANE_1_0;
	data &= ~0x2;// clear bit 1
	reg_DFE_SAT_EN_LANE_1_0 = data;
	}
	
}

// WARNING: only timer0 is used the same way as lanes! 
// So mcu_cmn better only uses delay_ram or setup timer1 as lanes here!
...
void set_pu_tx(uint8_t on) BANKING_CTRL {
	if(on){
	if(reg_ANA_PU_TX_LANE==0){
#ifdef SUPPORT_TX_IDLE
...
// Use this other than interrupt
void set_pll_ready_tx(uint8_t on)  BANKING_CTRL{
	if(on){
		reg_PIN_PLL_READY_TX_LANE    = 1;
#ifdef SUPPORT_TX_IDLE
		// 20200706 after pll ready for SoC to take over
		reg_ANA_TX_IDLE_FORCE_LANE = 0;
#endif
	}else{
