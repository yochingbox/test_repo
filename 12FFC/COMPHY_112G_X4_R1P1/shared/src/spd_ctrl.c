...
void loadspeedtbl_pll(uint8_t rate) BANKING_CTRL {
	...
	case 5:
		...
		break;
	// 20200604 Hui for Samuel
	case 1:
//		reg_VIND_BAND_SEL_LANE = 1;
		reg_PLL_ICP_LANE_4_0 = 0xe;
		reg_PLL_LPFR_LANE_1_0 = 3;
		reg_PLL_LPFC_LANE_1_0 = 0;
//		reg_LCCAP_USB_LANE = 1;
		break;
	}
	
	// 20200610 per lane setting for crosstalk(Hui)
	// 20201006 for rate0 GLC5616-613
	switch(rate){
	case 0:
	case 1:
		reg_LANE_ALIGN_POFFSET_FORCE_LANE = 1;
		reg_LANE_ALIGN_POFFSET_LANE_6_0 = lane_align_poffset_rate1;
		break;
	default:
		reg_LANE_ALIGN_POFFSET_FORCE_LANE = 0;
		reg_LANE_ALIGN_POFFSET_LANE_6_0 = 0;
		break;
	}
}
...
//---	reg_TX_EMPH1_DEFAULT3_LANE_4_0			= speedtable[gen][spdoft_tx_emph1_default3_lane];
	...
	// 20200519 Used wrong offset! Luckily X7121M does not use this clock.
	reg_RXCLK_25M_DIV_LANE_7_0 				= speedtable[gen][spdoft_rxclk_25m_div_lane];
	reg_RXCLK_25M_FIX_DIV_EN_LANE			= speedtable[gen][spdoft_rxclk_25m_fix_div_en_lane];
...
//https://essjira.marvell.com/browse/IPCE_COMPHY-1564
	if(reg_RX_PAM2_EN_LANE){
		...
		// 20200716 Debug for tx training
//		reg_CDR_MODE_LANE_2_0 = 0;
	}
	else{
...
void rxclk_nt_config() BANKING_CTRL {
	...
	if(reg_RXSPEED_DIV_LANE_2_0&0x04) {
		...
		case 4: ANA_PLL26.BT.B0 = (ANA_PLL26.BT.B0&0x04)|0x80; reg_RXCLK_NT_FIX_DIV_EN_LANE = 1; break;
		// Xu: 16.5T
		case 5: ANA_PLL26.BT.B0 = (ANA_PLL26.BT.B0&0x04)|0x59; reg_RXCLK_NT_FIX_DIV_EN_LANE = 1; break;
		}
	}
	else {
		switch(reg_PIN_RXDCLK_NT_SEL_RD_LANE_2_0) {
