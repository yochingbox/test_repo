...
void rx_sq_thresh_cal_ram(void) BANKING_CTRL {
	...
	// 20201006 SoC request: for all speeds change to 3 for bad channels
	if(reg_RX_PAM2_EN_LANE==0){
		if(reg_RX_HALFRATE_EN_LANE){// 53g
			if(lnx_CALIBRATION_DEBUG0_LANE_7_0&0x1){
				reg_SQ_REFTHR_LANE_4_0 = lnx_CALIBRATION_DEBUG1_LANE_7_0&0xf;
			}else{
				reg_SQ_REFTHR_LANE_4_0 = 3;//0x05;
			}
		}else{// 106g
			// IPCE_COMPHY-1564
			if(lnx_CALIBRATION_DEBUG0_LANE_7_0>>1&0x1){
				reg_SQ_REFTHR_LANE_4_0 = lnx_CALIBRATION_DEBUG1_LANE_7_0>>4&0xf;
			}else{
				reg_SQ_REFTHR_LANE_4_0 = 3;//0x05;
			}
		}
	}else{
		reg_SQ_REFTHR_LANE_4_0 = 3;//0x07;
	}
