...
void int5_isr (void) __interrupt (12) __using (2) //txtrain
{
	...
	if(reg_PIN_TX_TRAIN_ENABLE_MUX_RD_LANE && do_train==0) {
		...
		do_train = 1;
#ifdef NO_REDUCE_CODE
		if((phy_mode != PCIE) && (cmx_TX_TRAIN_MODE == 1))
	...
	//}
		...
		if(phy_mode==SERDES) { reg_TX_SEL_BITS_LANE = tx_sel_bit; reg_RX_SEL_BITS_LANE = rx_sel_bit; }
		#endif
		if(reg_LOCAL_TX_TRAIN_COMPLETE_LANE==0) { //restart in middle of train
			reg_REMOTE_TX_TRAIN_COMPLETE_LANE = 0;
			reg_LOCAL_TX_TRAIN_COMPLETE_LANE = 0;
			mcu_reset();
		}
...
{
#ifdef NO_REDUCE_CODE
	uint8_t n;//, rm_stat, error_code, c0, c1, cn1, ce, single_arg;
	//uint8_t ce;
	...
	TRX_TRAIN_IF_INTERRUPT_CLEAR_LANE.BT.B1 = 0x03; //clear remote_ctrl/status_isr
	...
	if(remote_ctrl_isr || remote_status_isr) {
		if( reg_TX_TRAIN_ON_LANE || reg_PIN_PAPTA_TRAIN_ENABLE_RD_LANE) {
			...
			if( (do_train && reg_PIN_TX_TRAIN_ENABLE_MUX_RD_LANE==1) ||
			    (do_tx_apta && reg_PIN_TX_TRAIN_ENABLE_MUX_RD_LANE==0 &&
			    reg_PIN_PAPTA_TRAIN_ENABLE_RD_LANE==1)) {
				...
				if(phy_mode == 4) { //ethernet
					...
					if( (remote_ctrl_h&0xe0) == 0) { //pattern check
						...
						reg_INT7_REMOTE_STATUS_FIELD_VALID_MUX_INT_EN_LANE = 0;
				...
				if(remote_status_isr) remote_status_in = 1;

				if(remote_ctrl_isr==0 && error_code==0) goto ext_int0;
				...
				#endif 
				{
					...
					if( link_train_mode==0 && do_tx_apta==0) {
						if( train_comp ) reg_LOCAL_STATUS_FIELD_LANE_15_0_b1 |= 0x80;
							...
							reg_LOCAL_STATUS_FIELD_LANE_15_0_b1 |= 0x40;
					}
...
void int4_isr (void) __interrupt (11)  __using (2) 
{
	...
	if( reg_MCU_REMOTE_REQ_ISR ) {
		...
		//((reg_MCU_REMOTE_COMMAND_RD_7_0 == CHECK_MP_SPDCHG) && spdchg_on) ||
