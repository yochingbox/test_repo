...
void dfe_adaptation( void ) BANKING_CTRL {
	...
	if( ctrl_max_dfe_adapt_shift > 0 ) {
		...
		memset0_ram((uint8_t *)&sum, sizeof(Status_SUM_CDS_t));		
#else
...
void pre_calculation(void) BANKING_CTRL {
	...
	val = cds28.f0a.s_p_e;
#ifdef SUPPORT_EXT_MEM
	val = max_ram(cds28.f0a.s_n_e, val);
	val = max_ram(cds28.f0a.s_p_o, val);
	train.f0a_max_slicer = max_ram(cds28.f0a.s_n_o, val);
#else
	...
	val = cds28.f0a.d_p_e;
#ifdef SUPPORT_EXT_MEM
	val = max_ram(cds28.f0a.d_n_e, val);
	val = max_ram(cds28.f0a.d_p_o, val);
	train.f0a_max_data = max_ram(cds28.f0a.d_n_o, val);
#else
	...
	val = cds28.f0d.s_p_e;
#ifdef SUPPORT_EXT_MEM
	val = min_ram(cds28.f0d.s_n_e, val);
	val = min_ram(cds28.f0d.s_p_o, val);
	train.eo_slicer = min_ram(cds28.f0d.s_n_o, val);
#else
	...
	val = cds28.f0d.d_p_e;
#ifdef SUPPORT_EXT_MEM
	val = min_ram(cds28.f0d.d_n_e, val);
	val = min_ram(cds28.f0d.d_p_o, val);
	train.eo_data = min_ram(cds28.f0d.d_n_o, val);	
#else
...
void DFE_Final_Calculation( void ) BANKING_CTRL {
...
#ifdef SUPPORT_EXT_MEM
	train.fn1 = clamp_ram(train.fn1, tag_DFE_FN1_MAX, tag_DFE_FN1_MIN );
#else
    ...
    // 5.saturate
#ifdef SUPPORT_EXT_MEM
	sum_my_abs_fx = abs_ram(train.f1)+abs_ram(train.f2)+abs_ram(train.f3)+abs_ram(train.f4)+abs_ram(train.f5);
#else
...
#ifdef SUPPORT_EXT_MEM
    train.dfe_saturate = (!tag_DFE_SATURATE_DISABLE) && ((abs_ram(train.f1) >= tag_DFE_F1_SAT_THRES) || (abs_ram(train.f2) >= tag_DFE_F2_SAT_THRES) );
#else
	...
	//7.f0t   
#ifdef SUPPORT_EXT_MEM
    //f0_minus_abs_fn1 = clamp_ram(f0_minus_abs_fn1, tag_DFE_F0_MAX, tag_DFE_F0_MIN);
#else
...
#ifdef SUPPORT_EXT_MEM
	train.sumFtap = clamp16_ram(train.sumFtap, 1024,-1024);
#else
	train.sumFtap = clamp16(train.sumFtap, 1024,-1024);
#endif
	  
   //9                                          //1                                 -2
   //train.boost = (uint16_t)abs16_ram(train.sumFtap - ((((int8_t)tag_SUMF_BOOST_TARGET_K) * train.f0a * dfe_res_f0_sumFtap[dfe_res_f0] )>>4) - sumf_boost_target_c);
#ifdef SUPPORT_EXT_MEM
   train.boost = (uint16_t)abs16_ram(train.sumFtap - sumf_boost_target_c);
#else
