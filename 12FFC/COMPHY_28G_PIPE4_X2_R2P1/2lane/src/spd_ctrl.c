...
void SpeedChange(){
  ...
  if( phy_mode==PCIE ) 
  {
        // reg_SELLV_RXSAMPELR_CH0_3_0 and reg_SELLV_RXSAMPELR_CH1_3_0   
        if(gen_tx==0) 
                UPHY14_CMN_ANAREG_TOP_146.BT.B0 = 0;        
        else 
                UPHY14_CMN_ANAREG_TOP_146.BT.B0 = 0x88;     
  }
  #endif //_PCIE_USB_BUILD
  rx_init_done = 0;
  cont_cal_inx = 0;
...
*/
	...
	//no need, duplication
	//#ifdef PCIE_USB_BUILD
	//if(phy_mode == PCIE && (gen==1 || gen==0) && ((TRAIN_CONTROL_9.BT.B1 & (0x01<<gen)) == (0x01<<gen)) ) {
