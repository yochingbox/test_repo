...
typedef union {
        struct {
		    uint8_t saved_ddc_dfe_clk_off 	 : 1 ;//1 bit
		    uint8_t saved_ddc_dis_s 		 : 1 ;//1 bit
		    uint8_t saved_ddc_pu_s_f1p_se 	 : 3 ;//3 bits
		    uint8_t saved_ddc_pu_s_f1p_so 	 : 3 ;//3 bits
		    uint8_t saved_ddc_pu_s_f1n_se 	 : 3 ;//3 bits
		    uint8_t saved_ddc_pu_s_f1n_so 	 : 3 ;//3 bits
		    uint8_t saved_ddc_cont_en 		 : 1 ;//1 bits
        } BF; 
...
#define saved_ddc_dis_s 	 SAVE_REG_STATE_DFE_OFF.BF.saved_ddc_dis_s 	         
#define saved_ddc_pu_s_f1p_se 	 SAVE_REG_STATE_DFE_OFF.BF.saved_ddc_pu_s_f1p_se 	 
#define saved_ddc_pu_s_f1p_so 	 SAVE_REG_STATE_DFE_OFF.BF.saved_ddc_pu_s_f1p_so 	 
#define saved_ddc_pu_s_f1n_se 	 SAVE_REG_STATE_DFE_OFF.BF.saved_ddc_pu_s_f1n_se 	 
#define saved_ddc_pu_s_f1n_so 	 SAVE_REG_STATE_DFE_OFF.BF.saved_ddc_pu_s_f1n_so 	 
#define saved_ddc_cont_en 	 SAVE_REG_STATE_DFE_OFF.BF.saved_ddc_cont_en 	         

extern __xdata bool dfe_power_saving_dfe_off_flag;
extern __xdata bool dfe_power_saving_temp_change_flag;
extern __xdata uint16_t dfe_power_saving_counter;
extern __xdata uint16_t dfe_power_saving_temp_old;
//extern __xdata bool power_saving_timer_on;
//extern __xdata bool power_saving_timeout;
#endif
//------------------------------------
// bit 0x20 ~ 0x2F
...
extern __xdata bool add_f0b_en;
...
extern __xdata bool dis_block_phase_train;

extern bool co_valid; 
...
//----------------------------------
//new cds memory
 __xdata __at(0x6100) Status_CDS_28G_t cds28; //114
 __xdata __at(0x6178) Status_dfe_sm_t dfe_sm; //64
 __xdata __at(0x6178+sizeof(Status_dfe_sm_t)) Status_dfe_sm_dc_t dfe_sm_dc; //8
 __xdata __at(0x6178+sizeof(Status_dfe_sm_t)+sizeof(Status_dfe_sm_dc_t)) Status_dfe_sm_t dfe_sm_save[3]; //64*3=192 //64*5 = 320 @61c0
 __xdata __at(0x6280) uint8_t rx_ffe_set_r_pcie_gen3_tb_usr[16];  
 __xdata __at(0x6290) uint8_t rx_ffe_set_r_pcie_gen4_tb_usr[16];  
 __xdata __at(0x62a0) uint8_t gt1_c_pcie_gen3_usr[8]; 
 __xdata __at(0x62a8) uint8_t gt1_c_pcie_gen4_usr[10]; 
...
// tx tb 
typedef struct {
	uint8_t pre										: 8;  //tx train preset tb pre
