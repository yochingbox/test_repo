...
{
	uint8_t n;
	BIT remote_ctrl_isr, remote_status_isr, dual_arg_done;
  ...
  if(remote_ctrl_isr || remote_status_isr) {
   ...
   single_arg = 0;
	   ...
	   if(remote_status_isr) 
	  ...
	  #ifdef SUPPORT_APTA_TX_TRAIN
		//if(reg_PIN_TX_TRAIN_ENABLE_MUX_RD_LANE==0 && reg_PIN_PAPTA_TRAIN_ENABLE_RD_LANE == 1) {
		if(reg_PIN_PAPTA_TRAIN_ENABLE_RD_LANE ||do_tx_apta) {
			co_valid = 1;
		   if(remote_ctrl_isr) {	
			if( ((do_tx_apta==1) &&	
			((remote_ctrl_l&0x30)==0x30 || 
		  ...
		  if(do_rx_apta) {
			if((remote_status_l&0x3f) == 0x3f) { //terminate
				apta_sim_state = 0;
			}		
		  }	
		}
		...
		if(do_train) {
		  ...
		  if(phy_mode<=SAS){
			...
			//normal pattern type
			ce = (uint8_t)((remote_ctrl_h>>2) & 0x03);			
			if((ce>0) && ((remote_ctrl_l&0x3f) != 0) && (remote_ctrl_l&0x3!=0x03) 
		...
		if(ce > 0) {	//ref1, ref2, no_eq
			...
			if( ce != 0 ) {
				...
				{	
						...
						switch (ce) {
						case 0x0a: single_arg = 1; break;    
						case 0x05: single_arg = 1; break;
						case 0x28: single_arg = 2; break;
						case 0x14: single_arg = 2; break;
						}	
						if(txffe_force_en==0 && co_valid) {
							tx_amp_is_max = (tx_amp_index >= tx_amp_max);
							tx_amp_is_min = (tx_amp_index <= tx_amp_min);
							tx_emph0_is_max = (tx_emph0_index >= tx_emph0_max);
							tx_emph0_is_min = (tx_emph0_index <= tx_emph0_min);
							...
							case 0x08: if(tx_amp_is_max==0)   tx_amp_index++;   break;
							case 0x04: if(tx_amp_is_min==0)   tx_amp_index--;   break;
							#ifndef SUPPORT_TTIU_SAS_DEBUG
							//Due to TX analog design limitation, cannot meet IEEE standard in SERDES.
							case 0x02: 
								if(cmx_SAS_TTIU_MODE_SEL_C1) {
									if( tx_emph0_is_max==0 && tx_amp_is_min==0) {
										tx_emph0_index++; tx_amp_index--; 
									}
									else {rm_stat = Cn1_is_min; dual_arg_done = 1;}								
								}
								else {
							...
							case 0x01: 
								if(cmx_SAS_TTIU_MODE_SEL_C1) {
									if(tx_emph0_is_min==0 && tx_amp_is_max==0) {
										tx_amp_index++; tx_emph0_index--; 
									}
									else {rm_stat = Cn1_is_max; dual_arg_done = 1;}
								}
								else {	
									...
									break;
							case 0x20: 
								if(cmx_SAS_TTIU_MODE_SEL_C3) {	
									if(tx_emph1_is_max==0 && tx_amp_is_min==0) {
										tx_emph1_index++; tx_amp_index--;   
									}
									else {rm_stat = C1_is_min; dual_arg_done = 1;}
								}
								else {	
							...
							case 0x10: 
								if(cmx_SAS_TTIU_MODE_SEL_C3) {	
									 if(tx_emph1_is_min==0 && tx_amp_is_max==0) {
										tx_emph1_index--;  tx_amp_index++; 
									}
									else {rm_stat = C1_is_max; dual_arg_done = 1;}
								}
								else {		
							...
							#else //SUPPORT_TTIU_SAS_DEBUG
							//Due TX analog design Limitation, SAS have to be P2P_HOLD only. 
							case 0x02: if( tx_emph0_is_max==0 && tx_amp_is_min==0) {
										tx_emph0_index++; tx_amp_index--; 
									}
									else {rm_stat = Cn1_is_min; dual_arg_done = 1;}
									//dual_arg = 1;
									break;
							case 0x01: if(tx_emph0_is_min==0 && tx_amp_is_max==0) {
										tx_amp_index++; tx_emph0_index--; 
									}
									else {rm_stat = Cn1_is_max; dual_arg_done = 1;}
									//dual_arg = 1;
									break;
							case 0x20: if(tx_emph1_is_max==0 && tx_amp_is_min==0) {
										tx_emph1_index++; tx_amp_index--;   
									}
									else {rm_stat = C1_is_min; dual_arg_done = 1;}
									//dual_arg = 1;
									break;
							case 0x10: if(tx_emph1_is_min==0 && tx_amp_is_max==0) {
										tx_emph1_index--;  tx_amp_index++; 
									}	
									else {rm_stat = C1_is_max; dual_arg_done = 1;}
									//dual_arg = 1;
