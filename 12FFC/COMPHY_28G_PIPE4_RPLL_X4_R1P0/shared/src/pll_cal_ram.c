...
void pll_amp_cal_cont_ram(void) BANKING_CTRL {
	...
	cmx_cal_lcvco_dac_lsb[CONT][0] = (reg_LCVCO_DAC_LSB_4_0 & 0x1f) | 0x00;  
	cmx_cal_lcvco_dac_msb[CONT][0] = (reg_LCVCO_DAC_MSB_2_0 & 0x07) | 0x00; 			
...
void pllcal_load_ram(uint8_t no) BANKING_CTRL {
	...
	lccap_msb_index = 0;
	set_lcvco_dac(cmx_cal_lcvco_dac_msb[no][0], cmx_cal_lcvco_dac_lsb[no][0]);			
	
	LCCAP_LSB 	  = cmx_cal_lccap_lsb[0];
	if(no==PWR) 
		lccap_msb_index = conv_thermo_2_bi( cmx_cal_lccap_msb[0] );	
	else	lccap_msb_index  = cal_lccap_msb_index[0];
...
void pllcal_save_ram(uint8_t no) BANKING_CTRL {
	cmx_cal_lcvco_dac_lsb[no][0] = (LCVCO_DAC_LSB & 0x1f) | 0x00; 
	cmx_cal_lcvco_dac_msb[no][0] = (LCVCO_DAC_MSB & 0x07) | 0x00; 
	cmx_cal_lccap_lsb[0]     = (LCCAP_LSB & 0x1f) | 0x00; 
	cmx_cal_lccap_msb[0]     = (LCCAP_MSB & 0x0f) | 0x00; 
}
