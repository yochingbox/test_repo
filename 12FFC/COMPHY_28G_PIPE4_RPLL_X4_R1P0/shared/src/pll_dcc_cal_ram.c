...
void plldcc_cal_ram(void) BANKING_CTRL {
	...
	if (lnx_CAL_DONE_LANE == 0) {                        //power-on calibration
		...
		dcc_cnt = cmx_cal_plldcc[PWR + cmx_PLLDCC_CAL_CONT_CUR_LOAD_EN][0];
		...
		PLLDCCCAL_EN = 1;
		dcc_cnt = cmx_cal_plldcc[CONT][0];    //always load cont result
		in_cont_mode = 1;
	}
	...
	do {
		...
		if( lnx_CAL_DONE_LANE == 0 && is_toggle_pat( dir, time_out_cnt - 1 ) ) { cmx_PLLDCC_CAL_PASS = 1;  break; }     //power-on, toggled
	...
	if (lnx_CAL_DONE_LANE == 0) {                        //if power-on calibration
	    cmx_cal_plldcc[PWR][0] = dcc_cnt;   //save to power-on result
	}

	cmx_cal_plldcc[CONT][0] = dcc_cnt;    //no matter power-on or LOAD or continious, need to save to [CONT]
