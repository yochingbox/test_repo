...
void PowerUp_Seq(void) BANKING_CTRL {
	...
	if(phy_mode==SERDES) {
		uint8_t genno = reg_PIN_PHY_GEN_TX_RD_LANE_4_0;
	...
	//reg_MCU_DEBUG8_LANE_7_0 = cmx_ETHERNET_CFG_2_0;
...
void pu_pll_on(void) BANKING_CTRL { // on - *pll_fast_cal, dtx_clk_off*, reset_dtx*
	...
	if(serdes_ring_lane_en && tag_ringpll_ind_en) {
		if(ring_lane_sel) {
            ...
            /* We never turn off LCPLL so there is no need to turn on
			pll_fast_cal();	
			reg_DTX_CLK_OFF = 0;
			delay01(10);
			reg_RESET_DTX	= 0;*/
...
void pu_pll_off(void) BANKING_CTRL { //off - reset_ana*, ana_pu_pll_*,  pll_clk_ready_*, dtx_clk_off*, reset_dtx*
	...
	if(serdes_ring_lane_en && tag_ringpll_ind_en) {
		if(ring_lane_sel) {
            ...
            // If the lane is using LC PLL, it cannot just turn it off because there might be some other lanes using it. 
			//pu_lcgpll_off();
		}
	}
	else 
