...
void clear_xdata(void) BANKING_CTRL {
    ...
    // -- ECC  
    //Step 1 : set these registers  to 1
    ...
    if (mcuid == master_mcu) {
        ...
        MEMORY_CONTROL_4.BT.B2 = 0x7e;

        reg_ROM_PARITY_ERR_CLEAR = 1;
        //MEMORY_CONTROL_0.BT.B0 = 0x84; // ROM_PARITY, PRAM_RTSEL
        //reg_PRAM_ECC_1ERR_CLEAR = 1;
        //reg_PRAM_ECC_2ERR_CLEAR = 1;
        MEMORY_CONTROL_0.BT.B2 = 0x24; // PRAM_ECC        
    }
                                                  
    //step 2 : set these registers to 1
    //NO need :reg_XDATA_ECC_1ERR_ENABLE_LANE = 1;                                      
    //NO need :reg_CACHE_ECC_1ERR_ENABLE_LANE = 1;                                                  
    //No need :reg_IRAM_ECC_1ERR_ENABLE_LANE = 1;                                                  
    //reg_XDATA_ECC_2ERR_ENABLE_LANE = 1;                                                 
    //reg_CACHE_ECC_2ERR_ENABLE_LANE = 1;                                         
    //reg_IRAM_ECC_2ERR_ENABLE_LANE = 1;                                         
    MCU_MEM_REG2_LANE.BT.B1 = 0xe0;
                               
    if (mcuid == master_mcu) {
        //No need : reg_XDATA_ECC_1ERR_ENABLE_CMN = 1;                              
        //reg_XDATA_ECC_2ERR_ENABLE_CMN = 1;                                                      
        //No need : reg_CACHE_ECC_1ERR_ENABLE_CMN = 1;                                            
        //reg_CACHE_ECC_2ERR_ENABLE_CMN = 1;                                                      
        MEMORY_CONTROL_4.BT.B1 = 0xc0;                                            
        //No need : reg_IRAM_ECC_1ERR_ENABLE_CMN = 1;                                      
        reg_IRAM_ECC_2ERR_ENABLE_CMN = 1;                                                     
        //MEMORY_CONTROL_4.BT.B2 = 0x7f;
                                        
        reg_ROM_PARITY_ERR_ENABLE = 1;
        //MEMORY_CONTROL_0.BT.B0 = 0x86;                                      
        //No need : reg_PRAM_ECC_1ERR_ENABLE = 1;                                        
        reg_PRAM_ECC_2ERR_ENABLE = 1;                                                 
        //MEMORY_CONTROL_0.BT.B2 = 0x26;                                     
    }
                                        
    //Step 3 : set these registers  back to 0
    ...
    if (mcuid == master_mcu) {
        ...
        //reg_IRAM_ECC_1ERR_CLEAR_CMN = 0;                                           
        //reg_IRAM_ECC_2ERR_CLEAR_CMN = 0;                                          
        MEMORY_CONTROL_4.BT.B2 = 0x01;

        reg_ROM_PARITY_ERR_CLEAR = 0;
        //MEMORY_CONTROL_0.BT.B0 = 0x82;                                     
        //reg_PRAM_ECC_1ERR_CLEAR = 0;                                        
        //reg_PRAM_ECC_2ERR_CLEAR = 0;                                      
        MEMORY_CONTROL_0.BT.B2 = 0x02;                                      
    }
...
*/
    ...
    if(phy_mode == 4) { //ethernet
        cmx_LOCAL_TX_PRESET_INDEX_3_0 = 5; //local txffe train start
    }   
...
void init_reg(void) BANKING_CTRL {
	...
	reg_DFE_EO_UP_THRE_COARSE_LANE_4_0 = 5; //10;//6;
	reg_DFE_EO_UP_THRE_FINE_LANE_4_0_b0 = 7;// 7 //8
	reg_DFE_EO_UP_THRE_FINE_LANE_4_0_b1 = 0;
	
	reg_DFE_DC_SIGN_XOR_LANE = 1; //to fix the rxdata wrong because DC adaptation diverged
	...
	reg_VDDVCO_VTH_12NM_SEL = 1; // ceproject-1654
	#ifdef _32G_ANA
	...
	if (cmx_TX_TRAIN_MODE == 1) {
        ...
        reg_FFE_PW_SEL_LANE_1_0 = 2; //12nm:2, 16nm:3;
 ...
 * FW needs to config table A first in order to select only one clock source (default LC clock)
 */
...
/*
 *  Since table A is configured to use the CMN LC clock source,
 *  only need to reconfigure if one of the lane uses the CMN ring or Lane ring clock sources
 *  PCIe mode use either CMN LC or CMN ring PLL, other PHY modes use CMN LC or Lane Ring PLL.
 */
void load_rpll_cmn_clk_mux_tableB(void) BANKING_CTRL {
	if (use_ring_pll) {
...
#endif
		{
			if (
			...
			#endif
			#ifdef RPLL_X4
			    || (cmx_LANE2_LCPLL_SEL_7_0 == 1) || (cmx_LANE3_LCPLL_SEL_7_0 == 1)
			#endif
			) {  // Ring PLL + LC PLL
				//reg_CLK_DIRECTION_LCPLL = 1;  // if other lane select CMN LC, don't set this to 0
				//reg_FORCE_CLK_DIRECTION_LCPLL = 1;
				//reg_CLK_BKWD_DIRECTION_LCPLL = 0;
				//reg_CLK_FWD2_DIRECTION_LCPLL = 1;  // if other lane select CMN LC, don't set this to 0
				//reg_CLK_BKWD2_DIRECTION_LCPLL = 0;
				...
				// TODO - always select LCPLL; reg_CLK_DIRECTION_LCPLL = 0;
				//reg_FORCE_CLK_DIRECTION_LCPLL = 1;
				//reg_CLK_BKWD_DIRECTION_LCPLL = 0;
				// TODO - always select LCPLL; reg_CLK_FWD2_DIRECTION_LCPLL = 0;
				//reg_CLK_BKWD2_DIRECTION_LCPLL = 0;
