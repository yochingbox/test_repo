...
static inline void sync_status_lane_en_0(uint8_t lane_en) {
	...
	#ifdef _RPLL_X4
	case MCU_LANE2:
		SYNC_STATUS_LANE2_EN = lane_en;
		break;
	case MCU_LANE3:
		SYNC_STATUS_LANE3_EN = lane_en;
		break;
	#endif
	}
...
void timer0_isr (void) __interrupt (1) __using (2)
{
	...
	//if(lnx_TX_TRAIN_FRAME_DET_TIMER_ENABLE_LANE && do_train &&
	//   train_timer_int && link_train_mode==0 && reg_FRAME_LOCK_LANE==0) {
	//	if(phy_mode<=SAS) {
...
void int1_isr (void) __interrupt (2)  __using (2) //pcie
{
	...
	if(reg_PIN_PU_RX_RD_LANE==0) {
		...
		reg_DTL_CLK_OFF_LANE = 1;         //to prevent L1 freq loop drift
...
void int8_isr (void) __interrupt (17)  __using (2)
{
	...
	if(reg_INT_TXDETRX_EN_CHG_ISR_LANE ) {
		...
		if( reg_PIN_TXDETRX_EN_RD_LANE == 1) {
			...
			default: 
				if(reg_PIN_PU_RX_RD_LANE==0) PHY_NS = ST_P1_TXDETRX;
				else PHY_NS = ST_TXDETRX; 
				break; //P0
			}
	...
	if((PM_CTRL_INTERRUPT_REG1_LANE.BT.B3&0xe1) != 0 ) 
	{
		...
		default:
			if(reg_PIN_PU_PLL_RD_LANE == 0){
                ...
                if(do_train)// if it is still in train mode and power down, we need to revert back the sel bits.
                {
...
void int2_isr (void) __interrupt (9) __using (2) //gen
{
    ...
    if(phy_mode == SERDES  && !lnx_DIS_SPEED_CHANGE_TX_IDLE_LANE) {
        reg_ANA_TX_IDLE_FORCE_LANE = 1; // workaround for Voltron
        reg_ANA_TX_IDLE_LANE = 1; 
    } 
	
    if(reg_PIN_PU_PLL_RD_LANE == 1 && reg_PIN_PU_TX_RD_LANE == 1 && reg_PIN_PU_PLL_RD_LANE == 1 ){
	//if(reg_ANA_RX_PU_DLY2_LANE && reg_ANA_PU_TX_LANE) {
		if(phy_mode == SERDES) {
            PHY_NS = ST_CAL; PHY_STATUS_INT |= SPDCHG_ON;
        }
...
void int5_isr (void) __interrupt (12) __using (2) //txtrain
{
	...
	if(reg_PIN_TX_TRAIN_ENABLE_MUX_RD_LANE && do_train==0) {
		...
		if(phy_mode==SERDES) { 
            ...
            {
                tx_sel_bit = reg_TX_SEL_BITS_LANE; rx_sel_bit = reg_RX_SEL_BITS_LANE; 
		    ...
		    if(reg_PIN_TX_TRAIN_ENABLE_MUX_RD_LANE==0 && reg_PIN_PAPTA_TRAIN_ENABLE_RD_LANE == 1) 
			    ...
			    ((remote_ctrl_h&0x0c)==0) || ((remote_ctrl_h&0x0c)==0x8)  ) ) ) 
				    ...
				    return;								
                            ...
                            {
			        if((remote_ctrl_h&0x0c)==0x4)  
                                { //[11:10] //START
				        if(do_tx_apta!=0) 
                                        {
					        reg_LOCAL_STATUS_FIELD_LANE_15_0 = 0x3f; 
					        lnx_APTA_TERMINATE_REASON_LANE_7_0 = 0x02; //received Received APTA_ADJUST(START) control after enter coeffient adjust stage
				        }
				        else 
                                        {	
					        do_tx_apta = 1; 
			        ...
			        else  if( ((remote_ctrl_h&0x0c)==0x8)|| ((remote_ctrl_h&0x0c)==0xc) ) 
                                { //COMPLETE || TERMINATE
				        do_tx_apta = 0; 
		    ...
		    if( (do_train && reg_PIN_TX_TRAIN_ENABLE_MUX_RD_LANE==1) || (do_tx_apta && reg_PIN_TX_TRAIN_ENABLE_MUX_RD_LANE==0 && reg_PIN_PAPTA_TRAIN_ENABLE_RD_LANE==1)) 
				...
				ce = (uint8_t)((remote_ctrl_h>>4) & 0x03);
			    	if(ce&0x1) ce = 4;
                            ...
                            {
				    if( (remote_ctrl_h&0xe0) == 0) 
                                    { //pattern check
				    }
				    else if( (remote_ctrl_h&0xe0) == 0xe0) 
		    	...
		    	if(ce > 0) 
				    ...
				    //ce--;
				    #ifdef SUPPORT_SAS_TXTRAIN_IF_ERROR_HANDLE
                                    if(((remote_ctrl_l&0x3f) != 0) && (remote_ctrl_l&0x3!=0x03) 
                                                                && (remote_ctrl_l&0xc!=0x0c) && (remote_ctrl_l&0x30!=0x030) )
                                    {
...
ext_int:
        		...
        		{		
        			...
        			if(do_tx_apta && cmx_APTA_LIMITED_STATUS_DIS==0) 
                                {
        				switch(ce) 
                                        { //APTA doesn't have min/max_update combination.
...
ext_int0:
    ...
    //if(TRAIN_SIM_EN == 0)  //  NOT WORKING FOR SIMULATION  //  not sure it needs for silicon
    //{
...
{
	EA = 0;
	EX4 = 4; //reg_INT_ENABLE_ALL_LANE = 0;
	...
	if(reg_INT_TIMER3_ISR_LANE) {
		...
		if(lnx_TX_TRAIN_FRAME_DET_TIMER_ENABLE_LANE && do_train && train_timer_int && link_train_mode==0 && reg_FRAME_LOCK_LANE==0) {
			...
			//reg_TX_TRAIN_FAILED_LANE = 1;
            reg_TX_TRAIN_FAILED_LANE = 1;
            lnx_TX_TRAIN_FRAME_UNLOCK_FAILED_TIMEOUT_INT_LANE = 1;
		}
		#endif
	#else 
		if(reg_RX_INIT_DONE_LANE==0) {
	...
	if( reg_MCU_REMOTE_REQ_ISR ) {
		...
		//((reg_MCU_REMOTE_COMMAND_RD_7_0 == CHECK_MP_PLL_CLK_READY_RING_ON) && reg_ANA_TX_CLK_READY_RPLL_LANE) ||
