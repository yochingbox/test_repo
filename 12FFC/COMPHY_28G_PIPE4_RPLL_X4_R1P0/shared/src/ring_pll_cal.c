...
void ring_pll_cal( void ) BANKING_CTRL {
	...
	#endif
	delay01(100);
        delay01(200); //added for high temp PVT test unlock
	PLL_CAL_EN_RING = 1;
	temp = cmx_REFCLK_FREQ_7_0;
	temp--;
	#ifdef _LANE_RPLL
	reg_RPLL_FBC_CNT_TIMER_LANE_15_0_b0 = (uint8_t)(temp & 0x7);
        reg_RPLL_FBC_CNT_TIMER_LANE_15_0_b1 = (uint8_t)((temp >> 3) & 0xff);
        reg_RPLL_FBC_CNT_TIMER_LANE_15_0_b2 = (uint8_t)(temp >> 11);
        #else // CMN_RPLL
...
adjust: 
	...
	if(timeout) {
		...
		ringpll_load_ram(PWR);
	        ...
	        ringpll_save_ram(PWR);
	}

exit_out:
	ringpll_save_ram(CONT);
...
void ring_pll_cont(void) BANKING_CTRL {
	...
	ringpll_dac_fine = lnx_cal_sllp_dac_fine_ring[CONT][0];
#else  // CMN_RPLL
	//ringpll_dac_fine = cmx_cal_sllp_dac_fine_ring[CONT][rate];
	ringpll_dac_fine = cmx_cal_sllp_dac_fine_ring[CONT][0];
#endif
...
void ringpll_save_ram(uint8_t no) BANKING_CTRL {
	lnx_cal_pll_speed_ring[no][0] = (reg_PLL_SPEED_RING_LANE_4_0 & 0x1f) | 0x00;
	lnx_cal_pll_sllp_dac_coarse_ring[no][0] = (reg_PLL_SLLP_DAC_COARSE_RING_LANE_3_0 & 0x0f) | 0x00;
	lnx_cal_sllp_dac_fine_ring[no][0] = ringpll_dac_fine;
}

void ringpll_load_ram(uint8_t no) BANKING_CTRL {
	reg_PLL_SPEED_RING_LANE_4_0 = lnx_cal_pll_speed_ring[no][0];
	reg_PLL_SLLP_DAC_COARSE_RING_LANE_3_0 = lnx_cal_pll_sllp_dac_coarse_ring[no][0];
	ringpll_dac_fine = lnx_cal_sllp_dac_fine_ring[no][0]; 
	reg_PLL_SLLP_DAC_FINE_RING_LANE_10_8 = (uint8_t)(ringpll_dac_fine>>8); 
	reg_PLL_SLLP_DAC_FINE_RING_LANE_7_0 = (uint8_t)ringpll_dac_fine;
}
