...
void Calibration(void) BANKING_CTRL {
	...
	if(speedtable[genno][spdoft_tx_ck_sel_lane] == 0 || speedtable[genno][spdoft_rx_ck_sel_lane] == 0)
	 	lc_pll_used = 1;
	...
	if(phy_mode==SERDES && SPDCHG_ON_READ) {
		dtx_off = (pre_gen_pll_rate != gen_pll_rate) || (pre_use_ring_pll != use_ring_pll);
/*		if(dtx_off) {
    ...
    if(!lnx_DIS_SPEED_CHANGE_TX_IDLE_LANE){ 
    ...
    if ((!SPDCHG_ON_READ) ||  (!use_ring_pll && (gen_pll_rate != PLL_RATE_SEL))   ) 
    {
        ...
        if(!use_ring_pll) 
        {
            while((reg_MCU_DEBUGF_LANE_7_0 = 1 ) && (cmx_LANE0_LCPLL_RATEREQ_ON || cmx_LANE1_LCPLL_RATEREQ_ON || cmx_LANE2_LCPLL_RATEREQ_ON || cmx_LANE3_LCPLL_RATEREQ_ON) );
            reg_MCU_DEBUGF_LANE_7_0 = 0;

            if (mcuid == MCU_LANE0)
            {
                cmx_LANE0_PLL_RATE_SEL_REQ_7_0 = gen_pll_rate;
                cmx_LANE0_LCPLL_RATEREQ_ON = 1;
                while((reg_MCU_DEBUGF_LANE_7_0 = 2) && (!cmx_LANE0_LCPLL_RATECHG_DONE));
            }
            if (mcuid == MCU_LANE1)
            {
                cmx_LANE1_PLL_RATE_SEL_REQ_7_0 = gen_pll_rate;
                cmx_LANE1_LCPLL_RATEREQ_ON = 1;
                while((reg_MCU_DEBUGF_LANE_7_0 = 2) && (!cmx_LANE1_LCPLL_RATECHG_DONE));
            }
            if (mcuid == MCU_LANE2)
            {
                cmx_LANE2_PLL_RATE_SEL_REQ_7_0 = gen_pll_rate;
                cmx_LANE2_LCPLL_RATEREQ_ON = 1;
                while((reg_MCU_DEBUGF_LANE_7_0 = 2) && (!cmx_LANE2_LCPLL_RATECHG_DONE));
            }
            if (mcuid == MCU_LANE3)
            {
                cmx_LANE3_PLL_RATE_SEL_REQ_7_0 = gen_pll_rate;
                cmx_LANE3_LCPLL_RATEREQ_ON = 1;
                while((reg_MCU_DEBUGF_LANE_7_0 = 2) && (!cmx_LANE3_LCPLL_RATECHG_DONE));
            }
	...
	for(genno = min_gen; genno<=max_gen; genno++)  
	{
		...
		{
			if( mcuid==MCU_LANE0)	{
                cmx_LANE0_VDD_CAL_REQ = 1; 
                while((reg_MCU_DEBUGF_LANE_7_0 = 3) && cmx_LANE0_VDD_CAL_LOCK);
                vdd_cal();
                cmx_LANE0_VDD_CAL_REQ = 0; 
			}
                        
			if( mcuid==MCU_LANE1)	{
                cmx_LANE1_VDD_CAL_REQ = 1; 
                while((reg_MCU_DEBUGF_LANE_7_0 = 3) && cmx_LANE1_VDD_CAL_LOCK);
			    vdd_cal();
                cmx_LANE1_VDD_CAL_REQ = 0; 
			}
			
			if( mcuid==MCU_LANE2)	{
                cmx_LANE2_VDD_CAL_REQ = 1; 
                while((reg_MCU_DEBUGF_LANE_7_0 = 3) && cmx_LANE2_VDD_CAL_LOCK);
			    vdd_cal();
                cmx_LANE2_VDD_CAL_REQ = 0; 
			}

			if( mcuid==MCU_LANE3)	{
                cmx_LANE3_VDD_CAL_REQ = 1; 
                while((reg_MCU_DEBUGF_LANE_7_0 = 3) && cmx_LANE3_VDD_CAL_LOCK);
                vdd_cal();
                cmx_LANE3_VDD_CAL_REQ = 0; 
			}
            reg_MCU_DEBUGF_LANE_7_0 = 0;
		}
		...
		{
			if( mcuid==MCU_LANE0)	{
                cmx_LANE0_TXDETECT_CAL_REQ = 1;
                while((reg_MCU_DEBUGF_LANE_7_0 = 4) && cmx_LANE0_TXDETECT_CAL_LOCK);
			    txdetect_cal();
                cmx_LANE0_TXDETECT_CAL_REQ = 0;
			}

			if( mcuid==MCU_LANE1)	{
                cmx_LANE1_TXDETECT_CAL_REQ = 1;
                while((reg_MCU_DEBUGF_LANE_7_0 = 4) && cmx_LANE1_TXDETECT_CAL_LOCK);
			    txdetect_cal();
                cmx_LANE1_TXDETECT_CAL_REQ = 0;
			}

			if( mcuid==MCU_LANE2)	{
                cmx_LANE2_TXDETECT_CAL_REQ = 1;
                while((reg_MCU_DEBUGF_LANE_7_0 = 4) && cmx_LANE2_TXDETECT_CAL_LOCK);
			    txdetect_cal();
                cmx_LANE2_TXDETECT_CAL_REQ = 0;
			}

			if( mcuid==MCU_LANE3)	{
                cmx_LANE3_TXDETECT_CAL_REQ = 1;
                while((reg_MCU_DEBUGF_LANE_7_0 = 4) && cmx_LANE3_TXDETECT_CAL_LOCK);
			    txdetect_cal();
                cmx_LANE3_TXDETECT_CAL_REQ = 0;
			}
            reg_MCU_DEBUGF_LANE_7_0 = 0;
		}
        ...
        reg_ALIGN90_REF_LANE_5_0 = 0x1e; // ceproject-2053
		align90_comp_cal();
		load_rpll_cmn_clk_mux_tableB();
		rxalign90_cal();
		txdcc_cal();
    ...
    reg_RX_PULSE_SHORTB_LANE = 0; // CEProject-1650
	sampler_cal();
    reg_RX_PULSE_SHORTB_LANE = 1; // CEProject-1650
	squelch_cal();
	...
	//-- rximp
	{
		...
		//rximp lane0
		if(mcuid==MCU_LANE0) {
            cmx_LANE0_RXIMP_CAL_REQ = 1; 
            while((reg_MCU_DEBUGF_LANE_7_0 = 5) && cmx_LANE0_RXIMP_CAL_LOCK);
		    rximp_cal();
            cmx_LANE0_RXIMP_CAL_REQ = 0; 
		}
		
		//rximp lane1
		if(mcuid==MCU_LANE1)	{
            cmx_LANE1_RXIMP_CAL_REQ = 1; 
            while((reg_MCU_DEBUGF_LANE_7_0 = 5) && cmx_LANE1_RXIMP_CAL_LOCK);
		    rximp_cal();
            cmx_LANE1_RXIMP_CAL_REQ = 0; 
		}

		//rximp lane2
		if(mcuid==MCU_LANE2)	{
            cmx_LANE2_RXIMP_CAL_REQ = 1; 
            while((reg_MCU_DEBUGF_LANE_7_0 = 5) && cmx_LANE2_RXIMP_CAL_LOCK);
		    rximp_cal();
            cmx_LANE2_RXIMP_CAL_REQ = 0; 
		}
		//rximp lane3
		if(mcuid==MCU_LANE3 )	{
            cmx_LANE3_RXIMP_CAL_REQ = 1; 
            while((reg_MCU_DEBUGF_LANE_7_0 = 5) && cmx_LANE3_RXIMP_CAL_LOCK);
		    rximp_cal();
            cmx_LANE3_RXIMP_CAL_REQ = 0; 
		}
	...
	{	
		//tximp lane0
		if( mcuid==MCU_LANE0 )	{
            cmx_LANE0_TXIMP_CAL_REQ = 1; 
            while((reg_MCU_DEBUGF_LANE_7_0 = 6) && cmx_LANE0_TXIMP_CAL_LOCK);
		    tximp_cal();
            cmx_LANE0_TXIMP_CAL_REQ = 0; 
        }
               
		//tximp lane1
		if( mcuid==MCU_LANE1 )	{
            cmx_LANE1_TXIMP_CAL_REQ = 1; 
            while((reg_MCU_DEBUGF_LANE_7_0 = 6) && cmx_LANE1_TXIMP_CAL_LOCK);
		    tximp_cal();
            cmx_LANE1_TXIMP_CAL_REQ = 0; 
		}
		
		//tximp lane2
		if( mcuid==MCU_LANE2 ) 	{
            cmx_LANE2_TXIMP_CAL_REQ = 1; 
            while((reg_MCU_DEBUGF_LANE_7_0 = 6) && cmx_LANE2_TXIMP_CAL_LOCK);
		    tximp_cal();
            cmx_LANE2_TXIMP_CAL_REQ = 0; 
		}
		//tximp lane3
		if( mcuid==MCU_LANE3 )	{
            cmx_LANE3_TXIMP_CAL_REQ = 1; 
            while((reg_MCU_DEBUGF_LANE_7_0 = 6) && cmx_LANE3_TXIMP_CAL_LOCK);
		    tximp_cal();
            cmx_LANE3_TXIMP_CAL_REQ = 0; 
		}
...
void all_cal_ext(void) BANKING_CTRL {
	...
	loadspeedtbl_gen();
	if(phy_mode==SERDES) {
		if( (gen_pll_rate == pre_gen_pll_rate) && (pre_use_ring_pll == use_ring_pll)) return;
	}

    if(!use_ring_pll) 
    {

        while(cmx_LANE0_LCPLL_RATEREQ_ON || cmx_LANE1_LCPLL_RATEREQ_ON || cmx_LANE2_LCPLL_RATEREQ_ON || cmx_LANE3_LCPLL_RATEREQ_ON );
        if(gen_pll_rate != PLL_RATE_SEL)
        {
            if (mcuid == MCU_LANE0)
            {
                cmx_LANE0_PLL_RATE_SEL_REQ_7_0 = gen_pll_rate;
                cmx_LANE0_LCPLL_RATEREQ_ON = 1;
                while(!cmx_LANE0_LCPLL_RATECHG_DONE);
            }
	        if (mcuid == MCU_LANE1)
            {
                cmx_LANE1_PLL_RATE_SEL_REQ_7_0 = gen_pll_rate;
                cmx_LANE1_LCPLL_RATEREQ_ON = 1;
                while(!cmx_LANE1_LCPLL_RATECHG_DONE);
            }
	        if (mcuid == MCU_LANE2)
            {
                cmx_LANE2_PLL_RATE_SEL_REQ_7_0 = gen_pll_rate;
                cmx_LANE2_LCPLL_RATEREQ_ON = 1;
                while(!cmx_LANE2_LCPLL_RATECHG_DONE);
            }
	        if (mcuid == MCU_LANE3)
            {
                cmx_LANE3_PLL_RATE_SEL_REQ_7_0 = gen_pll_rate;
                cmx_LANE3_LCPLL_RATEREQ_ON = 1;
                while(!cmx_LANE3_LCPLL_RATECHG_DONE);
            }
        }
    }
...
void clear_normal_mode_cal_en(void) BANKING_CTRL {
   ...
   if(do_train && phy_mode==SERDES) {
        ...
        if(!lnx_TX_TRAIN_END_PATTERN_DIS_LANE) {
	...
	if(reg_PIN_PLL_READY_RX_LANE==0 || reg_PIN_PLL_READY_TX_LANE==0) {
		...
		TX_TRAIN_CTRL_LANE0.VAL &= 0xa01f0002;
	}
