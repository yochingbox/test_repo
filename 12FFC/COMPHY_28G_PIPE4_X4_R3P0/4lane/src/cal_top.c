...
*/
	...
	{ 	
		...
		if(mcuid == master_mcu) reg_RXIMPCAL_EN_COMMON = 1; 
		else while(reg_RXIMPCAL_EN_COMMON==0);	
		...
		if(mcuid == master_mcu) 
			reg_RXIMPCAL_EN_COMMON = 0;
		else while(reg_RXIMPCAL_EN_COMMON);
	}
...
void Cal_Cont(void) BANKING_CTRL {
	...
	do {
		...
		case 6:	
			...
			if( mcuid==master_mcu && reg_ANA_PLL_CLK_READY) {
				if( timeout == 0 ) break;			
		...
		break;	
		case 7:	
			if(mcuid==master_mcu && reg_ANA_PLL_LOCK_RD) {
				//if( timeout2 == 0) break;
...
void mcu_align_0(uint8_t val) BANKING_CTRL {
	...
	PHY_CHECK_LANE = val; 
	while((PHY_CHECK_LANE0_READ!=val||PHY_CHECK_LANE1_READ!=val ||PHY_CHECK_LANE2_READ!=val ||PHY_CHECK_LANE3_READ!=val)){
		if(SYNC_STATUS_LANE0_EN_READ==0 || SYNC_STATUS_LANE1_EN_READ==0 ||
		   SYNC_STATUS_LANE2_EN_READ==0 || SYNC_STATUS_LANE3_EN_READ==0 ) break;	
	}	
}
