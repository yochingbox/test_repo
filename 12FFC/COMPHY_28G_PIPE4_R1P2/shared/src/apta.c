...
uint8_t apta_l_r_error_check(void) BANKING_CTRL {
	uint8_t error_flag, ctrl, status, rm_st_h;
	uint8_t c1, c0, cn1;
	uint8_t g1, g0, gn1;
	
	error_flag = 0;
	ctrl = reg_LOCAL_CTRL_FIELD_LANE_15_0 & 0x3f;	
	status = remote_status_l;
	rm_st_h = remote_status_h;

	#ifdef _28G_X1_R1P2
	//This is known bug for R1P1. SAS Wrapper doesn't want to fix this.
	if((status&0x3f) == 0x3f) {
		apta_sim_state = 0;
		return 0;
	}
	...
	case 0x0c00: //APTA_TERM:
		...
		break;
	default:	
		if((rm_st_h&0x0c)!= 0) error_flag = 1;
...
//          
	...
	if( reg_PIN_PAPTA_TRAIN_ENABLE_RD_LANE && reg_PIN_LOCAL_CTRL_FIELD_READY_RD_LANE ) { //Xu: When Remote enable LOCAL_CTRL_FIELD_READY=1, Remote have to wait LOCAL_CTRL_VALID toggle.
		...
		//while(remote_status_in==0 && !timeout2); //get ready?
		//timeout_stop2;
		//if(timeout2==1) 
		//{
...
cmd_if_exit:	
		EA = 0;
		if(	cmd_if_on > 0 && PHY_MCU_REMOTE_REQ_LANE) {
