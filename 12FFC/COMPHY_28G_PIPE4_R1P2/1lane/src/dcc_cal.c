...
#ifdef _28G_X2_B0
#define TXDCC_PDIV_CNT_MAX				 cmx_TX_PDIV_DCC_MAX_7_0
#define TXDCC_PDIV_CNT_MIN				 cmx_TX_PDIV_DCC_MIN_7_0
#else
#define TXDCC_PDIV_CNT_MAX				 63
#define TXDCC_PDIV_CNT_MIN				 0
#endif	
...
//main loop  
  do {
    ...
    if( lnx_CAL_DONE_LANE == 0 && is_toggle_pat( dir, time_out_cnt - 1 ))  { lnx_TXDCC_CAL_PASS_LANE = 1;  break; }     //power-on, toggled
    if( lnx_CAL_DONE_LANE == 0 && time_out_cnt >= TXDCC_LOOP_MAX && cmx_TXDCC_CAL_TIMEOUT_DIS == 0) break;     //power-on, reached maximum rounds
  ...
  if (lnx_CAL_DONE_LANE == 0) {                        //if power-on calibration (including ext_en)
    lnx_cal_txdcc[PWR][tx_pll_rate] = dcc_cnt;      //save to power-on result
    lnx_cal_txdcc_hg[tx_pll_rate] = (reg_TXDCC_HG_LANE & 0x01) | 0x00;       //save the HG setting for current rate, will be loaded by continious mode
  }
...
//main loop  
  do {
    ...
    if (int_dn == 1) {
      if (dcc_cnt <= TXDCC_PDIV_CNT_MIN) {dcc_cnt = TXDCC_PDIV_CNT_MIN;}
      else              {dcc_cnt--;}
    }
    else if (int_up == 1) {
      dir |= 1;
      if (dcc_cnt >= TXDCC_PDIV_CNT_MAX) {dcc_cnt = TXDCC_PDIV_CNT_MAX;}
      else               {dcc_cnt++;}
    }
    dir<<=1;
    dir = dir & 0x3f;
...
//main loop  
  do {
    ...
    if( lnx_CAL_DONE_LANE == 1 && time_out_cnt > 1) break;                   //continious mode, has ran 1 round
    if( lnx_CAL_DONE_LANE == 0 && is_toggle_pat( dir, time_out_cnt - 1 ))  { lnx_RXDCC_DLL_CAL_PASS_LANE = 1;  break; }     //power-on, toggled
    if( lnx_CAL_DONE_LANE == 0 && time_out_cnt >= RXDCC_DLL_LOOP_MAX && cmx_RXDCC_DLL_CAL_TIMEOUT_DIS == 0) break; //power-on, reached maximum rounds
  ...
  if (lnx_CAL_DONE_LANE == 0) {                            //if power-on calibration
    ...
    lnx_cal_rxdcc_dll_hg[rx_pll_rate] = (reg_RXDCC_HG_DLLCLK_LANE & 0x01) | 0x00; //save the HG setting for current rate, will be loaded by continious mode
  }
...
void rxdcc_data_cal(void) BANKING_CTRL {
	...
	if (lnx_CAL_DONE_LANE == 0) {                        //power-on calibration
		...
		if (ext_en) {                                     //use ext value for power-on, bypass calibration
			dcc_cnt = lnx_cal_rxdcc_data[PWR][gen];
			RXDCC_HG_DATACLK = lnx_cal_rxdcc_data_hg[gen];          //load the HG setting.
		}
		else {                                          //use normal power-on calibration
		...
		//either load power-on, or previous cont cal result for target speed
		dcc_cnt = lnx_cal_rxdcc_data[PWR + cmx_RXDCC_DATA_CAL_CONT_CUR_LOAD_EN][gen];
		rxdcc_data_step_cnt = RXDCC_DATA_CAL_STEP_SIZE;     //when speed change, restore the fracition code to middle
		RXDCC_HG_DATACLK = lnx_cal_rxdcc_data_hg[gen];          //load the HG setting.
	}
	else if (cmx_RXDCC_DATA_CAL_CONT_EN && RX_SPEED_DIV < 4 ) {  //not power-on. Continious mode, only performed when SPEED_DIV = 0
		RXDCC_CAL_EN = 1;
		TRXDCCIN_SEL = 1;
		dcc_cnt = lnx_cal_rxdcc_data[CONT][gen];    //always load cont result
		RXDCC_HG_DATACLK = lnx_cal_rxdcc_data_hg[gen];          //load the HG setting.
		in_cont_mode = 1;                                  //when continious mode, use fraction code
	}
	else {                                               //Continious mode, but disabled, or SPEED_DIV != 0
	...
	do {
		...
		time_out_cnt ++;
		if( RXDCC_CAL_EN == 0 ) break;                                      //either LOAD, or EXT_EN in power-on
		if( lnx_CAL_DONE_LANE == 0 && RX_SPEED_DIV > 3 ) break;                  //power-on, but SPEED_DIV = 1xx, skip and keep using init code (0x20)
		if( lnx_CAL_DONE_LANE == 1 && time_out_cnt > 1) break;                   //continious mode, has ran 1 round
		if( lnx_CAL_DONE_LANE == 0 && is_toggle_pat( dir, time_out_cnt - 1 ))  { lnx_RXDCC_DATA_CAL_PASS_LANE = 1;  break; }     //power-on, toggled
		if( lnx_CAL_DONE_LANE == 0 && time_out_cnt >= RXDCC_DATA_LOOP_MAX && cmx_RXDCC_DATA_CAL_TIMEOUT_DIS == 0) break; //power-on, reached maximum rounds
	...
	//save calibration result
	if (lnx_CAL_DONE_LANE == 0) {                            //if power-on calibration
		lnx_cal_rxdcc_data[PWR][gen] = dcc_cnt;       //save to power-on result
		lnx_cal_rxdcc_data_hg[gen] = (RXDCC_HG_DATACLK & 0x01) | 0x0; //save the HG setting for current rate, will be loaded by continious mode
	}

	lnx_cal_rxdcc_data[CONT][gen] = dcc_cnt;     //no matter power-on or LOAD or continious, need to save to [CONT]
	//for next continious mode to read
	RXDCC_CAL_EN = 0;
	lnx_RXDCC_DATA_CAL_DONE_LANE = 1;
...
void rxdcc_eom_cal(void) BANKING_CTRL {
	...
	if (lnx_CAL_DONE_LANE == 0) {                        //power-on calibration
		...
		if (ext_en){                                     //use ext value for power-on, bypass calibration
			dcc_cnt = lnx_cal_rxdcc_eom[PWR][gen];
			RXDCC_HG_EOMCLK = lnx_cal_rxdcc_eom_hg[gen];          //load the HG setting.
		}
		else {                                          //use normal power-on calibration
			lnx_RXDCC_EOM_CAL_PASS_LANE = 0; // clear pass flag for normal power-on cal
		...
		//either load power-on, or previous cont cal result for target speed
		dcc_cnt = lnx_cal_rxdcc_eom[PWR + cmx_RXDCC_EOM_CAL_CONT_CUR_LOAD_EN][gen];
		rxdcc_eom_step_cnt = RXDCC_EOM_CAL_STEP_SIZE;     //when speed change, restore the fracition code to middle
		RXDCC_HG_EOMCLK = lnx_cal_rxdcc_eom_hg[gen];          //load the HG setting.
	}
	else if (cmx_RXDCC_EOM_CAL_CONT_EN && EOM_CLK_EN == 1 && RX_SPEED_DIV < 4 ) {  //not power-on. Continious mode, only performed when EOM_CLK_EN == 1 and RX_SPEED_DIV[2]==0
		RXDCC_CAL_EN = 1;
		TRXDCCIN_SEL = 2; // Fixed from 0 in ROM code function
		dcc_cnt = lnx_cal_rxdcc_eom[CONT][gen];    //always load cont result
		RXDCC_HG_EOMCLK = lnx_cal_rxdcc_eom_hg[gen];    //load the HG setting.
		in_cont_mode = 1;                                  //when continious mode, use fraction code
	}
	else {                                               //Continious mode, but disabled, or SPEED_DIV != 0
	...
	do {
		...
		if( lnx_CAL_DONE_LANE == 0 && RX_SPEED_DIV > 3 ) break;                  //power-on, but SPEED_DIV = 1xx, skip and keep using init code (0x20)
		if( lnx_CAL_DONE_LANE == 1 && time_out_cnt > 1) break;                   //continious mode, has ran 1 round
		if( lnx_CAL_DONE_LANE == 0 && is_num_toggle_pat( dir, time_out_cnt - 1, 2 ))  { lnx_RXDCC_EOM_CAL_PASS_LANE = 1;  break; }     //power-on, toggled
		if( lnx_CAL_DONE_LANE == 0 && time_out_cnt >= RXDCC_EOM_LOOP_MAX && cmx_RXDCC_EOM_CAL_TIMEOUT_DIS == 0) break; //power-on, reached maximum rounds
	...
	//save calibration result
	if (lnx_CAL_DONE_LANE == 0) {                            //if power-on calibration
		lnx_cal_rxdcc_eom[PWR][gen] = dcc_cnt;       //save to power-on result
		lnx_cal_rxdcc_eom_hg[gen] = (RXDCC_HG_EOMCLK & 0x01) | 0x0; //save the HG setting for current rate, will be loaded by continious mode
	}
	lnx_cal_rxdcc_eom[CONT][gen] = dcc_cnt;     //no matter power-on or LOAD or continuous, need to save to [CONT]
	//for next continuous mode to read
