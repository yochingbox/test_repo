...
static void update_ssc_sq_thr(void) ;
static void sata_speedchange(void) ;
...
void SpeedChange(){
  ...
  if((new_gen_tx != gen_tx || new_gen_rx != gen_rx /*|| ( pwrsq_on && phy_mode!=SERDES )*/ ) /*&& phy_mode!=SERDES*/ ){
	...
	if(sata_on) { //SATA must support 1.5G/3G/6G only!! 
		sata_speedchange();	
	}	
	else 
	#endif		
  ...
  if(!sata_pll_lock || !sata_on) {
	delay01(5); //TXCLK need more time 0.5sec even in bypassed delay 
	if(use_ring_pll) check_ring_pll_lock_0();
	else check_pll_lock(); //For Hayabusa simulation fast but need pll lock
  }
...
static void sata_speedchange(void) {
	...
	//assume RINGPLL only or LCPLL only
		//gen_pll_rate = 1;
		//tx_pll_rate = 1;
		//use_ring_pll = 0;		
	
		LINK_TRAIN_MODE0.BT.B0 = 0; //tx_em_po
		LINK_TRAIN_MODE0.BT.B1 = 0; //tx_em_pre, peak
		...
		UPHY14_TRX_ANAREG_TOP_128.BT.B0 = 0x6c; //res1/cap1
		UPHY14_TRX_ANAREG_TOP_128.BT.B0 = 0x0f; //res2/cap2_o
		UPHY14_TRX_ANAREG_BOT_0.BT.B0 = 0x0f; //res2/cap2_e
		
		if(gen==0) {
			reg_TX_SPEED_DIV_LANE_2_0 = 4;  
			SPD_CTRL_TX_LANE_REG1_LANE.BT.B0 = 0x04; //reg_TXSPEED_DIV_LANE_2_0
			UPHY14_TRX_ANAREG_TOP_138.BT.B0 = 0x7d;//reg_DTL_CLK_SPEEDUP_LANE_2_0=3,reg_RX_SPEED_DIV_LANE_2_0 = 7;
			reg_PU_F1P_S_O_LANE = 0; 	
			reg_PU_F1P_D_O_LANE	= 1; 
			reg_PU_F1P_D_E_LANE	= 0; 
			reg_RX_SELMUFF_LANE_2_0 = 5;
			reg_DFE_TAP_SETTLE_SCALE_LANE_1_0 = 3;
			reg_FFE_DATA_RATE_LANE_3_0 = 2;
			dfe_dis = 1;	
			reg_DFE_EN_LANE = 0;  reg_DFE_DIS_LANE = 1;			
		}
		else if(gen==1) {
			reg_TX_SPEED_DIV_LANE_2_0 = 3; 
			SPD_CTRL_TX_LANE_REG1_LANE.BT.B0 = 0x03; //reg_TXSPEED_DIV_LANE_2_0
			UPHY14_TRX_ANAREG_TOP_138.BT.B0 = 0x35;//reg_DTL_CLK_SPEEDUP_LANE_2_0=1,reg_RX_SPEED_DIV_LANE_2_0 = 5;
			reg_PU_F1P_S_O_LANE = 0; 	
			reg_PU_F1P_D_O_LANE	= 1; //0; 
			reg_PU_F1P_D_E_LANE	= 1; 
			reg_RX_SELMUFF_LANE_2_0 = 5;
			reg_DFE_TAP_SETTLE_SCALE_LANE_1_0 = 3;
			reg_FFE_DATA_RATE_LANE_3_0 = 3;			
			dfe_dis = 1;	
			reg_DFE_EN_LANE = 0;  reg_DFE_DIS_LANE = 1;			

		}
		else if(gen==2) {
			reg_TX_SPEED_DIV_LANE_2_0 = 2; 
			SPD_CTRL_TX_LANE_REG1_LANE.BT.B0 = 0x02; //reg_TXSPEED_DIV_LANE_2_0
			UPHY14_TRX_ANAREG_TOP_138.BT.B0 = 0x35;//reg_DTL_CLK_SPEEDUP_LANE_2_0=1,reg_RX_SPEED_DIV_LANE_2_0 = 5;
			reg_PU_F1P_S_O_LANE = 1; 	
			reg_PU_F1P_D_O_LANE	= 1; 
			reg_PU_F1P_D_E_LANE	= 1; 
			reg_RX_SELMUFF_LANE_2_0 = 6;
			reg_DFE_TAP_SETTLE_SCALE_LANE_1_0 = 2;
			reg_FFE_DATA_RATE_LANE_3_0 = 4;			
			dfe_dis = 0;	
			reg_DFE_EN_LANE = 1;  reg_DFE_DIS_LANE = 0;
		
		}

	//to fix hardware Rx Metastability in SAS 3G
	if(lnx_RX3G_FIX_EN_LANE) {

	  if(gen==1) { //3G	
		reg_ANA_RSVD9_LANE_7_0 = reg_ANA_RSVD9_LANE_7_0 | 0x88;//06242020 Hui Z added this ANA_RSVD9_LANE[3]=1 for only at 2.5G/3G/3.125Gmode. But Script cannot support. Move to FW hard cord.
		reg_ANA_RSVD9_LANE_7_0 = reg_ANA_RSVD9_LANE_7_0 & 0xfe;
		
		UPHY14_TRX_ANAREG_TOP_138.BT.B0 = 0x35;	//reg_DTL_CLK_SPEEDUP_LANE_2_0 = 1,reg_RX_SPEED_DIV_LANE_2_0 = 5;
		reg_PU_F1P_D_O_LANE = 1;
	  } 
	  else {
		val = (reg_RX_SPEED_DIV_LANE_2_0&0x02)? 0x80: 0;
		val |=  reg_RX_SPEED_DIV_LANE_2_0&0x01;
		reg_ANA_RSVD9_LANE_7_0 = (reg_ANA_RSVD9_LANE_7_0 & 0x76) | val;		
	  }
	}
	else { //default: no fix v1.1.2b

	  if(gen==1) { //3G	
		UPHY14_TRX_ANAREG_TOP_138.BT.B0 = 0x59;//reg_DTL_CLK_SPEEDUP_LANE_2_0 = 2, reg_RX_SPEED_DIV_LANE_2_0 = 6;
		reg_PU_F1P_D_O_LANE = 0;
	  } 
	...
	//reg_MCU_DEBUG1_LANE_7_0 = 0x68;
	//reg_MCU_DEBUG1_LANE_7_0 = reg_ANA_RSVD9_LANE_7_0;	
	...
	//load_cal
    reg_TXDCCCAL_PDIV_CNT_LANE_5_0 = lnx_cal_txdcc_pdiv[PWR][1];  
	...
	if(!sata_pll_lock) { reg_DTX_CLK_OFF = 0; reg_DTX_CLK_OFF_RING = 0; }
	reg_DTL_CLK_OFF_LANE= 0; 
	reg_DFE_CLK_OFF_LANE = 0; 	
	delay01(5);
	if(!sata_pll_lock) { reg_RESET_DTX	= 0; reg_RESET_DTX_RING = 0; }
...
*/
	...
	if(lnx_RX3G_FIX_EN_LANE) {
	#ifdef _PCIE_USB_BUILD
		if(gen==0) { //2.5G
	#elif _SATA_SERDES_BUILD
		#ifdef _SERDES_BUILD
		if(reg_PIN_PHY_GEN_RX_RD_LANE_3_0==12 || reg_PIN_PHY_GEN_RX_RD_LANE_3_0==1) { //2.5G, 3.125G
		#else //SAS
		if(gen==1){ //3G	
		#endif
	#endif	
			reg_ANA_RSVD9_LANE_7_0 = reg_ANA_RSVD9_LANE_7_0 | 0x88;//06242020 Hui Z added this ANA_RSVD9_LANE[3]=1 for only at 2.5G/3G/3.125Gmode. But Script cannot support. Move to FW hard code.
			reg_ANA_RSVD9_LANE_7_0 = reg_ANA_RSVD9_LANE_7_0 & 0xfe;			
			reg_RX_SPEED_DIV_LANE_2_0 = 5;
			reg_DTL_CLK_SPEEDUP_LANE_2_0 = 1;
			reg_PU_F1P_D_O_LANE = 1;
		}
		else {
			val = (reg_RX_SPEED_DIV_LANE_2_0&0x02)? 0x80: 0;
		...
		#endif
	#endif	
			reg_RX_SPEED_DIV_LANE_2_0 = 6;
			reg_DTL_CLK_SPEEDUP_LANE_2_0 = 2;
			reg_PU_F1P_D_O_LANE = 0;
	  	} 
	...
	reg_MCU_DEBUG1_LANE_7_0 = reg_ANA_RSVD9_LANE_7_0;	
//Hui's comment: 
//This is to fix titania2 PCIE gen1 rx sampler bug we found recently. Originally we only use one sampler to sample both data and edge information, which create errors because of one sampler configuration. Now we change to 5G mode configuration by enabling additional divide by 2. So we are using two samplers separately, one for data and the other for edge at 2.5G/3G/3.125G mode.
//ANA_RSVD9[3]=1 only at 2.5G/3G/3.125G. ANA_RSVD9[3]=0 for the rest of rates. the new additional divide by 2 should be disabled(ANA_RSVD9[3]=0). 
	
	DFE_TEST_1.BT.B3 = 0; //clear train indicators

	//move to after rx_init
	//if(DFE_TEST_4.BT.B3 & (0x01<<gen))
	//	reg_EN_DFE_FLOATING_LANE = 1;
	//else 
	//	reg_EN_DFE_FLOATING_LANE = 0;
	//if(TRAIN_CONTROL_13.BT.B0 & (0x01<<gen))
