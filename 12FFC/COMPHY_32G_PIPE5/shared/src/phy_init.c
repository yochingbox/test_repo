...
*/
	...
	if(pcie_usb_mode)
	{
		...
		tx_adapt_gn1_en = 1;
		tx_adapt_g1_en = 1;
		reg_TRX_TRAIN_TIMER_LANE_15_0 = 23;
		#endif
	}
	else {
		#ifdef _SATA_SERDES_BUILD
...
void init_irq(void) BANKING_CTRL {
	...
	reg_INT7_REMOTE_STATUS_FIELD_VALID_MUX_INT_EN_LANE = 1;
	#endif

	if(phy_mode != PCIE || reg_PHY_ISOLATE_MODE) {
		//int7
		//reg_INT7_REMOTE_CTRL_VALID_INT_EN_LANE = 1;
		reg_INT7_REMOTE_CTRL_FIELD_VALID_MUX_INT_EN_LANE = 1;
...
void init_reg(void) BANKING_CTRL {
	...
	if ((phy_mode == PCIE) && (reg_PIN_PHY_GEN_TX_RD_LANE_3_0 >= 4)) {
	//if (1) {
	dfe_f0_res_sel = 2;
	dfe_res_f1 = 2;
	dfe_f234_res_sel = 0;
	...
	reg_DFE_EO_UP_THRE_FINE_LANE_4_0 = 7;// 7 //8
	...
	// reference clock dependent; set up for unified calibration sample clock
	reg_TRX_DIG_CAL2M_DIV_BOT_LANE_7_0 = cmx_REFCLK_FREQ_7_0/2; //0x9c
	reg_TRX_DIG_CAL2M_DIV_TOP_LANE_7_0 = cmx_REFCLK_FREQ_7_0/2; //0x9c

	reg_DLL_VDDA_CAL_SAMPLE_PULSE_DIV_LANE_7_0 = 0x14; //0x0f
	reg_ALIGN90_CAL_SAMPLE_PULSE_DIV_LANE_7_0 = 0x14; //0x04
	...
	reg_ALIGN90_CAL_VAL_MAX_LSB_LANE_5_0 = 0x31;
...
void reset_pwr_reg(void) BANKING_CTRL {
	...
	if(phy_mode != PCIE) {
		...
		reg_INT7_REMOTE_CTRL_FIELD_VALID_MUX_INT_EN_LANE = 0;
...
#ifdef SUPPORT_EXT_MEM
	reg_TIMER3_LO_CNT_LANE_15_0 = u16mul_ram(cmx_REFCLK_FREQ_7_0, lnx_TX_TRAIN_FRAME_DET_TIMER_LANE_7_0+1);
#else
