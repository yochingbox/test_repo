...
void clear_normal_mode_cal_en(void) BANKING_CTRL {
	...
	if(reg_PIN_PLL_READY_RX_LANE==0 || reg_PIN_PLL_READY_TX_LANE==0) {
		...
		turn_off_cont_cal(); //unified cont_cal stop	
	}
...
// Fix Vale SOC simulation failure
...
//			// For the unified cal IP, the analog PU signals take longer time
//			while (reg_ANA_PU_PLL==0 || reg_ANA_PU_TX_LANE==0);
//			reg_PIN_PLL_READY_RX_LANE = 0;
//			PHY_STATUS = ST_P1;
//		}
...
void turn_off_cont_cal(void) BANKING_CTRL
{
	...
	uint8_t dcc_start, dll_start, align90_start;

	dcc_start = 0; dll_start = 0; align90_start = 0;
	...
	if (dcc_start) {
		while(!reg_DCC_TOP_CONT_DONE_LANE);
		// save results
		lnx_cal_rxdcc_dll[PWR][rate] = (reg_RX_DLL_DCC_CAL_RESULT_RD_LANE_5_0 & 0x3f) | 0x00;
		lnx_cal_rxdcc_dll_hg[rate] = (reg_RXDCC_HG_DLLCLK_LANE & 0x01) | 0x00;
	...
	if (dll_start) {
		while(!reg_DLL_TOP_CONT_DONE_LANE);
		// save results
		lnx_cal_vdda_dll_sel[PWR][rate] = (reg_DLL_VDDA_CAL_RESULT_RD_LANE_5_0 & 0x3f) | 0x00;
		//if( !cmx_EOM_DLL_CAL_EXT_EN ) {
			lnx_cal_vdda_dll_eom_sel[PWR][rate] = (reg_EOM_DLL_VDDA_CAL_RESULT_RD_LANE_5_0 & 0x3f) | 0x00;
		//}
	}
	if (align90_start) {
		while(!reg_ALIGN90_TOP_CONT_DONE_LANE);	
