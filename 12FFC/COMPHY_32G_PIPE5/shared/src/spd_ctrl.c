...
void SpeedChange(){
...
#ifdef SUPPORT_EXT_MEM
				if(reg_ANA_PLL_CLK_READY==0) pll_clk_ready_1_ram();
#else
...
#ifdef SUPPORT_EXT_MEM
				#ifdef _LANE_RPLL
				if(reg_ANA_TX_CLK_READY_RPLL_LANE==0) pll_clk_ready_ring_1_ram();
				#else // CMN_RPLL
				if(reg_ANA_PLL_CLK_READY_RING==0) pll_clk_ready_ring_1_ram();
				#endif
#else
				#ifdef _LANE_RPLL
	...
	// restart cont_cal; make sure PCIe gen5 cont cal run
	Unified_Cal_Cont();
	...
	if (reg_MCU_DEBUG8_LANE_7_0 != 0)
		delay01(10 * reg_MCU_DEBUG8_LANE_7_0);
...
void sata_speedchange(void) {
	...
	reg_TX_DCC_CAL_POST_DIV_RESULT_EXT_LANE_5_0 = lnx_cal_txdcc_pdiv[PWR + cmx_TXDCC_PDIV_CAL_CONT_CUR_LOAD_EN][tx_pll_rate_x];  
...
void loadspeedtbl_gen(void) {
	...
	rx_pll_rate_x				= speedtable[gen][spdoft_pll_rate_sel_rx]; 
	if(rx_pll_rate_x==2 && phy_mode==PCIE) { rx_pll_rate = 0; /*rx_pll_rate_x = gen_pll_rate;*/ }
	else if(rx_pll_rate_x>1 || phy_mode==SERDES) { rx_pll_rate = 0; rx_pll_rate_x = 0; }
...
////ANA_RSVD9[3]=1 only at 2.5G/3G/3.125G. ANA_RSVD9[3]=0 for the rest of rates. the new additional divide by 2 should be disabled(ANA_RSVD9[3]=0). 
	...
	if(gen==1){ //3G
		...
		reg_ANA_RSVD9_LANE_7_0 = reg_ANA_RSVD9_LANE_7_0 & 0xfd;
		//reg_RX_SPEED_DIV_LANE_2_0 = 5;
		//reg_DTL_CLK_SPEEDUP_LANE_2_0 = 1;
		//reg_PU_F1P_D_O_LANE = 1;
	}
	else {
		temp = (reg_RX_SPEED_DIV_LANE_2_0&0x02)? 0x80: 0;
		temp |=  (reg_RX_SPEED_DIV_LANE_2_0&0x01) << 1;
		reg_ANA_RSVD9_LANE_7_0 = (reg_ANA_RSVD9_LANE_7_0 & 0x75) | temp;
	}
...
void Unified_Cal_Cont(void) {
	...
	if (cmx_RXDCC_DATA_CAL_CONT_EN && (reg_DCC_TOP_CONT_START_LANE == 0))
		dcc_cal_cont();

	if (cmx_RXDLL_CAL_CONT_EN && (reg_DLL_TOP_CONT_START_LANE == 0))
		dll_vdda_cal_cont();

	if (cmx_ALIGN90_CAL_CONT_EN && (reg_ALIGN90_TOP_CONT_START_LANE == 0))
		align90_cal_cont();
}
