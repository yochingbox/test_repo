...
#ifdef _28G_X2_DFE      
	SAMPLER_CAL_SM_SAVE = (uint8_t*)&CAL_OFST_REG0; SAMPLER_CAL_SM_SAVE+=sw; //sm
#else
	SAMPLER_CAL_SAVE = (uint8_t*)&CAL_SAVE_DATA4_LANE; SAMPLER_CAL_SAVE+=sw;  //2c  
...
__code uint8_t samper_cal_sel_tb[] = { 0, 0, 1, 1, 3, 3, 4, 4, 2, 5 }; //matched order to TC's DFE register
__code uint8_t sampler_f1_pol_tb[] = { 1, 0, 1, 0, 1, 0, 1, 0, 0, 0 };
...
void Edge_Sampler_Update_ram(void) BANKING_CTRL {
	...
	if( tag_edge_sampler_adj == 0 ) return;
	step_size = tag_edge_sampler_adj;

	// ---
	sampler_sel_ram(OFST_F1P_D_E);
	f1p_e = get_sampler_ram();
	...
	sampler_sel_ram(OFST_EDGE_E);
	edge_e = get_sampler_ram();
	set_sampler_ram(edge_e + diff_e);
	...
	// --- 
	sampler_sel_ram(OFST_F1P_D_O);
	f1p_o = get_sampler_ram();
	diff_o = f1p_o - f1p_o_old;
	if(step_size==1) diff_o >>= 1;
	else if(step_size==3) diff_o <<= 1;

	sampler_sel_ram(OFST_EDGE_O);
	edge_o = get_sampler_ram();
	set_sampler_ram(edge_o + diff_o);
...
void  save_sampler_edge_ram(void) BANKING_CTRL{
	sampler_sel_ram(OFST_F1P_D_E);
	f1p_e_old = get_sampler_ram();
	sampler_sel_ram(OFST_F1P_D_O);
	f1p_o_old = get_sampler_ram();
}
