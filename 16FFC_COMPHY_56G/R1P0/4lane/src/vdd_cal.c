...
void vdd_cal_calen(uint8_t sel, uint8_t val ) BANKING_CTRL {
	...
	case SELLV_RXEOMCLK:  reg_CALEN_VDDA_RXEOMCLK_LANE = val;  break;
	case SELLV_RXDATACLK: reg_CALEN_VDDA_RXDATACLK_LANE = val;  break;
	case SELLV_RXSAMPLER: reg_CALEN_VDDA_RXSAMPLER_LANE = val;  break;
	}	
}
...
void vdd_cal_set_pass(uint8_t sel, uint8_t val ) BANKING_CTRL {
	...
	case SELLV_RXEOMCLK:  lnx_SELLV_RXSAMPLER_PASS_LANE = val;  break;
...
bool vdd_cal_op (uint8_t sel ) BANKING_CTRL
{
	...
	while (op_done == 0) {
		...
		done_ptn &= 0x0f;
		if ((done_ptn == 0xa) || (done_ptn == 0x5) || (cont_cal_on) || (run_cnt >= tag_MAX_RUN)) {op_done = 1;}
