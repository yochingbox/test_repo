...
void OFST_D_S_init(uint8_t dat)
{
...
void OFST_EDGE_init(uint8_t dat){
	...
	for(i=0; i<14; i++) {
		...
		for (ntimes=0; ntimes < n_programmable; ntimes++) {
			...
			for( offset=-ofst_bnd; offset<=ofst_bnd; offset++ ) { //80% sweep
				...
				{
					offset = offset + 1;
					delay(T_6us); // 250ns, 500ns, 1us and 2us programmable
				}else
					delay(T_12us); // 250ns, 500ns, 1us and 2us programmable
				set_sampler(offset);
				//delay(T_16us); // 250ns, 500ns, 1us and 2us programmable
				if(VOFF_POS == 1 || offset == ofst_bnd) break;
				/*if (i<12)
					offset = offset + 1;*/
			}
