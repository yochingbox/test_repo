...
//main loop  
  do {
    ...
    if( cmx_CAL_DONE == 1 && time_out_cnt > 1) break;                   //continious mode, has ran 1 round
    if( cmx_CAL_DONE == 0 && is_toggle_pat( dir, time_out_cnt - 1 ))  { lnx_RX_CLK_CAL_PASS_LANE = 1;  break; }     //power-on, toggled
...
//main loop  
  do {
    ...
    if( RXDCC_CAL_EN == 0 ) break;                                      //either LOAD, or EXT_EN in power-on
    if( cmx_CAL_DONE == 1 && time_out_cnt > 1) break;                   //continious mode, has ran 1 round
    if( cmx_CAL_DONE == 0 && is_toggle_pat( dir, time_out_cnt - 1 ))  { lnx_RX_EOM_CAL_PASS_LANE = 1;  break; }     //power-on, toggled
...
void rx_clk_cal_init(void) BANKING_CTRL {
	...
	if (reg_RX_HALFRATE_EN_LANE == 1)
	{
		...
		reg_RX_ALIGN90_DE_CAL_SETTING_LANE_10_3 = (0x144 & 0x7f8);
		reg_RX_ALIGN90_DE_CAL_SETTING_LANE_12_11 = (0x144 & 0x1800);
	}
}
#if 0
