...
void pll_clk_ready_ring_1(void) BANKING_CTRL {
	...
	delay01(5);
	reg_ANA_TX_CLK_READY_PRE1_RPLL_LANE = 1;
	delay01(5);
	reg_ANA_TX_CLK_READY_RPLL_LANE = 1;
	#else // CMN_RPLL
...
void check_pll_clk_ready(void) BANKING_CTRL {
	do {
		#ifdef _LANE_RPLL
		if(ring_pll_enabled && reg_ANA_TX_CLK_READY_RPLL_LANE) break;
		#else // CMN_RPLL
...
void check_pll_clk_ready_pcie(void) BANKING_CTRL {
	do {
		#ifdef _LANE_RPLL
		if(use_ring_pll && reg_ANA_TX_CLK_READY_RPLL_LANE) break;
		#else // CMN_RPLL
