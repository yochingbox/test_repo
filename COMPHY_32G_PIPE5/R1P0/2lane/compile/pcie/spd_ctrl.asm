...
;	../../shared/src/spd_ctrl.c:205: load_cal_data_all();
	...
	lcall	__sdcc_banked_call
00150$:
;	../../shared/src/spd_ctrl.c:210: if( phy_mode==PCIE ) { //only pcie need align because of one mac
	mov	dptr,#(_SYSTEM + 0x0002)
	movx	a,@dptr
	anl	a,#0x07
	mov	r2,a
	cjne	r2,#0x03,00157$
;	../../shared/src/spd_ctrl.c:211: txlane_align();
	mov	r0,#_txlane_align
	mov	r1,#(_txlane_align >> 8)
	mov	r2,#(_txlane_align >> 16)
	lcall	__sdcc_banked_call
;	../../shared/src/spd_ctrl.c:212: if(use_ring_pll) pre_ringpll_rate = gen_pll_rate;
	jnb	_use_ring_pll,00154$
	mov	dptr,#_gen_pll_rate
	movx	a,@dptr
	mov	r2,a
	mov	dptr,#_pre_ringpll_rate
	...
	sjmp	00157$
00154$:
;	../../shared/src/spd_ctrl.c:213: else pre_lcpll_rate = gen_pll_rate;
	mov	dptr,#_gen_pll_rate
	movx	a,@dptr
	mov	r2,a
	mov	dptr,#_pre_lcpll_rate
...
;	../../shared/src/spd_ctrl.c:460: gen = gen_tx;
	...
	inc	r0
	mov	@r0,ar2
;	../../shared/src/spd_ctrl.c:472: if( gen_tx < 2) {
	cjne	r2,#0x02,00211$
00211$:
	jnc	00102$
;	../../shared/src/spd_ctrl.c:473: reg_TX_SEL_BITS_LANE = 0; reg_RX_SEL_BITS_LANE = 0;
	...
	anl	a,#0x7f
	movx	@dptr,a
;	../../shared/src/spd_ctrl.c:475: reg_TX_DEEMPH_FM_REG_LANE = 0;
	...
	sjmp	00103$
00102$:
;	../../shared/src/spd_ctrl.c:479: reg_TX_SEL_BITS_LANE = 1; reg_RX_SEL_BITS_LANE = 1;
	mov	dptr,#(_TX_SYSTEM_LANE + 0x0003)
	movx	a,@dptr
	orl	a,#0x80
	movx	@dptr,a
	...
	orl	a,#0x80
	movx	@dptr,a
;	../../shared/src/spd_ctrl.c:481: reg_TX_DEEMPH_FM_REG_LANE = 1;
	mov	dptr,#_INPUT_TX_PIN_REG1_LANE
	movx	a,@dptr
	orl	a,#0x08
	movx	@dptr,a
00103$:
;	../../shared/src/spd_ctrl.c:486: if(mcuid==master_mcu && !use_ring_pll && phy_mode==PCIE) {
	mov	dptr,#_MCU_CONTROL_LANE
	movx	a,@dptr
	mov	r4,a
	mov	dptr,#_MCU_CONFIG
	...
	mov	a,r4
	cjne	a,ar5,00108$
	jb	_use_ring_pll,00108$
	mov	dptr,#(_SYSTEM + 0x0002)
	movx	a,@dptr
	anl	a,#0x07
	mov	r4,a
	cjne	r4,#0x03,00108$
;	../../shared/src/spd_ctrl.c:488: if (gen_tx == 4)
	cjne	r2,#0x04,00105$
;	../../shared/src/spd_ctrl.c:489: reg_LCPLLCLK_DIV2_SEL = 1;
	mov	dptr,#_UPHY14_CMN_ANAREG_TOP_207
	movx	a,@dptr
	orl	a,#0x02
	movx	@dptr,a
	sjmp	00108$
00105$:
;	../../shared/src/spd_ctrl.c:491: reg_LCPLLCLK_DIV2_SEL = 0;
	mov	dptr,#_UPHY14_CMN_ANAREG_TOP_207
	movx	a,@dptr
	anl	a,#0xfd
	movx	@dptr,a
