...
;--------------------------------------------------------
	...
	.globl _sign_abs_up
	.globl _sign_abs_dn
	.globl _is_num_toggle_pat
	.globl _is_toggle_pat
	.globl _set_test_pattern
	.globl _recover_test_pattern
	.globl _read_tsen
	.globl _u16div
	.globl _u16mul
	.globl _short_delay
	.globl _short_delay_0
	.globl _max
	.globl _min
	.globl _clamp
	.globl _clamp_u8
...
;--------------------------------------------------------
	.area BANK2   (CODE)
;------------------------------------------------------------
;Allocation info for local variables in function 'delay01'
;------------------------------------------------------------
;t                         Allocated to registers r2 r3 
;a                         Allocated to registers r2 r3 
;i                         Allocated to registers r4 r5 
;------------------------------------------------------------
;	../../shared/src/misc_rom.c:25: void delay01(int16_t t) BANKING_CTRL{
;	-----------------------------------------
;	 function delay01
;	-----------------------------------------
_delay01:
	...
	mov	r2,dpl
	mov	r3,dph
;	../../shared/src/misc_rom.c:29: if (t < 3) {
	clr	c
	mov	a,r2
	subb	a,#0x03
	mov	a,r3
	...
	subb	a,#0x80
	jnc	00104$
;	../../shared/src/misc_rom.c:31: return;
	ljmp	00126$
00104$:
;	../../shared/src/misc_rom.c:32: } else if (t < 11) {
	clr	c
	mov	a,r2
	subb	a,#0x0B
	mov	a,r3
	...
	ljmp	00105$
00142$:
;	../../shared/src/misc_rom.c:33: for (i = 3; i < t; i++) {
	mov	r4,#0x03
	mov	r5,#0x00
00122$:
	clr	c
	...
	ljmp	00125$
00143$:
;	../../shared/src/misc_rom.c:61: __endasm;
...
;	../../shared/src/misc_rom.c:33: for (i = 3; i < t; i++) {
	...
	ljmp	00122$
00125$:
;	../../shared/src/misc_rom.c:63: return;
	ljmp	00126$
00105$:
;	../../shared/src/misc_rom.c:65: a = u16mul(mcuclk_mc, t); //elapse time 675nSec
	mov	dptr,#_mcuclk_mc
	movx	a,@dptr
	mov	r4,a
	mov	r5,#0x00
	...
	dec	sp
	dec	sp
;	../../shared/src/misc_rom.c:66: a += 4; a >>= 3; // divide by 8
	mov	a,#0x04
	add	a,r2
	mov	r2,a
	clr	a
...
00144$:
	mov	r3,a
;	../../shared/src/misc_rom.c:67: if (a <= 25) return;
	clr	c
	mov	a,#0x19
	subb	a,r2
	mov	a,#(0x00 ^ 0x80)
...
;	../../shared/src/misc_rom.c:68: a = -a;
	...
	subb	a,r3
	mov	r3,a
;	../../shared/src/misc_rom.c:70: if (BYPASS_DELAY == 0) {
	mov	dptr,#(_CONTROL_CONFIG0 + 0x0002)
	movx	a,@dptr
	anl	a,#0x1c
	jnz	00120$
;	../../shared/src/misc_rom.c:71: TMR0 = a + 25;  //a+compensation
	mov	a,#0x19
	add	a,r2
	mov	_TMR0,a
	clr	a
	addc	a,r3
	mov	(_TMR0 >> 8),a
;	../../shared/src/misc_rom.c:74: TR0 = 1;
	setb	_TR0
;	../../shared/src/misc_rom.c:77: while (TF0 != 1) {};
00108$:
	jnb	_TF0,00108$
;	../../shared/src/misc_rom.c:80: TR0 = 0;
	clr	_TR0
;	../../shared/src/misc_rom.c:81: TF0 = 0;
	clr	_TF0
	sjmp	00126$
00120$:
;	../../shared/src/misc_rom.c:83: if(( PHY_STATUS == ST_POSEQ && BYPASS_POWERON_DELAY ) ||
	...
	jb	acc.5,00126$
00118$:
;	../../shared/src/misc_rom.c:84: ( BYPASS_DELAY>=4 )) {
	mov	dptr,#(_CONTROL_CONFIG0 + 0x0002)
	movx	a,@dptr
	rr	a
	rr	a
	...
	subb	a,#0x84
	jnc	00126$
;	../../shared/src/misc_rom.c:87: a >>= BYPASS_DELAY;
	mov	dptr,#(_CONTROL_CONFIG0 + 0x0002)
	movx	a,@dptr
	rr	a
	rr	a
...
00153$:
	djnz	b,00152$
;	../../shared/src/misc_rom.c:89: TR0 = 1;
	setb	_TR0
;	../../shared/src/misc_rom.c:90: while (TF0 != 1) {};
00111$:
	jnb	_TF0,00111$
;	../../shared/src/misc_rom.c:91: TR0 = 0;
	clr	_TR0
;	../../shared/src/misc_rom.c:92: TF0 = 0;
	clr	_TF0
00126$:
	ljmp	__sdcc_banked_ret
;------------------------------------------------------------
;Allocation info for local variables in function 'sign_abs_up'
;------------------------------------------------------------
;dcc_cnt                   Allocated to registers r2 
;result                    Allocated to registers r3 
;------------------------------------------------------------
;	../../shared/src/misc_rom.c:133: uint8_t sign_abs_up(uint8_t dcc_cnt) BANKING_CTRL {
;	-----------------------------------------
;	 function sign_abs_up
;	-----------------------------------------
_sign_abs_up:
	mov	r2,dpl
;	../../shared/src/misc_rom.c:136: if (dcc_cnt == 63) {  //sign = 1, abs = 1_1111
	cjne	r2,#0x3F,00108$
;	../../shared/src/misc_rom.c:137: result = 63;
	mov	r3,#0x3F
	sjmp	00109$
00108$:
;	../../shared/src/misc_rom.c:139: else if (dcc_cnt == 0) { //sign = 0, abs = 0_0000
	mov	a,r2
	jnz	00105$
;	../../shared/src/misc_rom.c:140: result = 32;       //sign = 1, abs = 0_0000
	mov	r3,#0x20
	sjmp	00109$
00105$:
;	../../shared/src/misc_rom.c:142: else if (dcc_cnt >= 32) { //sign = 1, abs >= 0_0000
	cjne	r2,#0x20,00118$
00118$:
	jc	00102$
;	../../shared/src/misc_rom.c:143: result = dcc_cnt + 1;
	mov	a,r2
	inc	a
	mov	r3,a
	sjmp	00109$
00102$:
;	../../shared/src/misc_rom.c:146: result = dcc_cnt - 1;
	mov	a,r2
	dec	a
	mov	r3,a
00109$:
;	../../shared/src/misc_rom.c:149: return result;
	mov	dpl,r3
	ljmp	__sdcc_banked_ret
;------------------------------------------------------------
;Allocation info for local variables in function 'sign_abs_dn'
;------------------------------------------------------------
;dcc_cnt                   Allocated to registers r2 
;result                    Allocated to registers r3 
;------------------------------------------------------------
;	../../shared/src/misc_rom.c:152: uint8_t sign_abs_dn(uint8_t dcc_cnt) BANKING_CTRL {
;	-----------------------------------------
;	 function sign_abs_dn
;	-----------------------------------------
_sign_abs_dn:
	mov	r2,dpl
;	../../shared/src/misc_rom.c:155: if (dcc_cnt == 31) {  //sign = 0, abs = 1_1111
	cjne	r2,#0x1F,00108$
;	../../shared/src/misc_rom.c:156: result = 31;
	mov	r3,#0x1F
	sjmp	00109$
00108$:
;	../../shared/src/misc_rom.c:158: else if (dcc_cnt == 32) { //sign = 1, abs = 0_0000
	cjne	r2,#0x20,00105$
;	../../shared/src/misc_rom.c:159: result = 0;       //sign = 0, abs = 0_0000
	mov	r3,#0x00
	sjmp	00109$
00105$:
;	../../shared/src/misc_rom.c:161: else if (dcc_cnt > 32) { //sign = 1, abs > 0_0000
	mov	a,#0x20
	cjne	a,ar2,00119$
00119$:
	jnc	00102$
;	../../shared/src/misc_rom.c:162: result = dcc_cnt - 1;
	mov	a,r2
	dec	a
	mov	r3,a
	sjmp	00109$
00102$:
;	../../shared/src/misc_rom.c:165: result = dcc_cnt + 1;
	mov	a,r2
	inc	a
	mov	r3,a
00109$:
;	../../shared/src/misc_rom.c:168: return result;
	mov	dpl,r3
	ljmp	__sdcc_banked_ret
;------------------------------------------------------------
;Allocation info for local variables in function 'is_num_toggle_pat'
;------------------------------------------------------------
;output_cnt                Allocated to stack - offset -4
;target_toggles            Allocated to stack - offset -5
;val                       Allocated to registers r2 
;toggle_cnt                Allocated to registers r4 
;prev_state                Allocated to registers b0 
;new_state                 Allocated to registers b1 
;mask                      Allocated to registers r3 
;------------------------------------------------------------
;	../../shared/src/misc_rom.c:195: BIT is_num_toggle_pat( uint8_t val, uint8_t output_cnt, uint8_t target_toggles  ) BANKING_CTRL {
;	-----------------------------------------
;	 function is_num_toggle_pat
;	-----------------------------------------
_is_num_toggle_pat:
	push	_bp
	mov	_bp,sp
;	../../shared/src/misc_rom.c:197: BIT prev_state = ( val & 0x01 );	// [0] state
	mov	a,dpl
	mov	r2,a
	rrc	a
	mov	b0,c
;	../../shared/src/misc_rom.c:198: BIT new_state = ( val & 0x02 );		// [1] state
	mov	a,r2
	mov	c,acc[1]
	mov	b1,c
;	../../shared/src/misc_rom.c:199: uint8_t mask = 0x02;	// Current bit mask
	mov	r3,#0x02
;	../../shared/src/misc_rom.c:201: while ( mask < 0x40 && output_cnt > 1 ) {
	mov	r4,#0x00
	mov	a,_bp
	add	a,#0xfc
	mov	r0,a
...
00116$:
	jnc	00106$
;	../../shared/src/misc_rom.c:202: if ( new_state != prev_state ) {
	mov	c,b1
	jb	b0,00118$
	cpl	c
00118$:
	jc	00102$
;	../../shared/src/misc_rom.c:203: ++toggle_cnt;
	inc	r4
;	../../shared/src/misc_rom.c:204: prev_state = new_state;
	mov	c,b1
	mov	b0,c
00102$:
;	../../shared/src/misc_rom.c:206: mask <<= 1;
	mov	a,r3
	add	a,r3
	mov	r3,a
;	../../shared/src/misc_rom.c:207: --output_cnt;
	dec	r5
;	../../shared/src/misc_rom.c:208: new_state = val & mask;
	setb	c
	mov	a,r3
	anl	a,r2
	jnz	00120$
...
00120$:
	...
	sjmp	00104$
00106$:
;	../../shared/src/misc_rom.c:210: return toggle_cnt >= target_toggles;
	mov	a,_bp
	add	a,#0xfb
	mov	r0,a
	clr	c
	...
	ljmp	__sdcc_banked_ret
;------------------------------------------------------------
;Allocation info for local variables in function 'is_toggle_pat'
;------------------------------------------------------------
;output_cnt                Allocated to stack - offset -4
;val                       Allocated to registers r2 
;------------------------------------------------------------
;	../../shared/src/misc_rom.c:213: BIT is_toggle_pat( uint8_t val, uint8_t output_cnt  ) BANKING_CTRL {
;	-----------------------------------------
;	 function is_toggle_pat
;	-----------------------------------------
_is_toggle_pat:
	push	_bp
	mov	_bp,sp
	mov	r2,dpl
;	../../shared/src/misc_rom.c:214: return is_num_toggle_pat( val, output_cnt, 3 );
	mov	a,#0x03
	push	acc
	mov	a,_bp
	add	a,#0xfc
	...
	ljmp	__sdcc_banked_ret
;------------------------------------------------------------
;Allocation info for local variables in function 'set_test_pattern'
;------------------------------------------------------------
;pt_4t_en                  Allocated to registers r2 
;------------------------------------------------------------
;	../../shared/src/misc_rom.c:217: void set_test_pattern(uint8_t pt_4t_en) BANKING_CTRL {
;	-----------------------------------------
;	 function set_test_pattern
;	-----------------------------------------
_set_test_pattern:
	mov	r2,dpl
;	../../shared/src/misc_rom.c:218: save_local_ana_tx2rx_lpbk_en_lane  =  reg_LOCAL_ANA_TX2RX_LPBK_EN_LANE;
	mov	dptr,#_UPHY14_TRX_ANAREG_TOP_145
	movx	a,@dptr
	rl	a
	rl	a
	anl	a,#0x01
	mov	dptr,#_save_local_ana_tx2rx_lpbk_en_lane
	movx	@dptr,a
;	../../shared/src/misc_rom.c:219: save_pt_tx_pattern_sel_lane_5_0_b0 =  reg_PT_TX_PATTERN_SEL_LANE_5_0_b0;
	mov	dptr,#(_PT_CONTROL0 + 0x0002)
	movx	a,@dptr
	rl	a
	rl	a
	anl	a,#0x03
	mov	dptr,#_save_pt_tx_pattern_sel_lane_5_0_b0
	movx	@dptr,a
;	../../shared/src/misc_rom.c:220: save_pt_tx_pattern_sel_lane_5_0_b1 =  reg_PT_TX_PATTERN_SEL_LANE_5_0_b1;
	mov	dptr,#(_PT_CONTROL0 + 0x0003)
	movx	a,@dptr
	anl	a,#0x0f
	mov	dptr,#_save_pt_tx_pattern_sel_lane_5_0_b1
	movx	@dptr,a
;	../../shared/src/misc_rom.c:221: save_pt_en_lane 				   =  reg_PT_EN_LANE;
	mov	dptr,#(_PT_CONTROL0 + 0x0003)
	movx	a,@dptr
	rl	a
	anl	a,#0x01
	mov	dptr,#_save_pt_en_lane
	movx	@dptr,a
;	../../shared/src/misc_rom.c:222: save_pt_tx_clk_en_lane             =  reg_PT_TX_CLK_EN_LANE;
	mov	dptr,#(_CLKGEN_TX_LANE_REG1_LANE + 0x0002)
	movx	a,@dptr
	rl	a
	rl	a
	anl	a,#0x01
	mov	r3,a
	mov	dptr,#_save_pt_tx_clk_en_lane
	movx	@dptr,a
;	../../shared/src/misc_rom.c:224: reg_LOCAL_ANA_TX2RX_LPBK_EN_LANE  = 1;
	mov	dptr,#_UPHY14_TRX_ANAREG_TOP_145
	movx	a,@dptr
	orl	a,#0x40
	movx	@dptr,a
;	../../shared/src/misc_rom.c:225: if( pt_4t_en ) {
	mov	a,r2
	jz	00102$
;	../../shared/src/misc_rom.c:226: reg_PT_TX_PATTERN_SEL_LANE_5_0_b0 = 0x03; // 4T pattern = 0x0b
	mov	dptr,#(_PT_CONTROL0 + 0x0002)
	movx	a,@dptr
	orl	a,#0xc0
	movx	@dptr,a
;	../../shared/src/misc_rom.c:227: reg_PT_TX_PATTERN_SEL_LANE_5_0_b1 = 0x02;
	mov	dptr,#(_PT_CONTROL0 + 0x0003)
	movx	a,@dptr
	anl	a,#0xf0
	orl	a,#0x02
	...
	sjmp	00103$
00102$:
;	../../shared/src/misc_rom.c:230: reg_PT_TX_PATTERN_SEL_LANE_5_0_b0 = 0x00; //PRBS7:0x10
	mov	dptr,#(_PT_CONTROL0 + 0x0002)
	movx	a,@dptr
	anl	a,#0x3f
	movx	@dptr,a
;	../../shared/src/misc_rom.c:231: reg_PT_TX_PATTERN_SEL_LANE_5_0_b1 = 0x04;
	mov	dptr,#(_PT_CONTROL0 + 0x0003)
	movx	a,@dptr
	anl	a,#0xf0
	orl	a,#0x04
	movx	@dptr,a
00103$:
;	../../shared/src/misc_rom.c:234: reg_PT_EN_LANE = 1;
	mov	dptr,#(_PT_CONTROL0 + 0x0003)
	movx	a,@dptr
	orl	a,#0x80
	movx	@dptr,a
;	../../shared/src/misc_rom.c:235: reg_PT_TX_CLK_EN_LANE = 1;
	mov	dptr,#(_CLKGEN_TX_LANE_REG1_LANE + 0x0002)
	movx	a,@dptr
	orl	a,#0x40
	movx	@dptr,a
	ljmp	__sdcc_banked_ret
;------------------------------------------------------------
;Allocation info for local variables in function 'recover_test_pattern'
;------------------------------------------------------------
;------------------------------------------------------------
;	../../shared/src/misc_rom.c:238: void recover_test_pattern(void) BANKING_CTRL {
;	-----------------------------------------
;	 function recover_test_pattern
;	-----------------------------------------
_recover_test_pattern:
;	../../shared/src/misc_rom.c:239: reg_PT_TX_PATTERN_SEL_LANE_5_0_b0 = save_pt_tx_pattern_sel_lane_5_0_b0;
	mov	dptr,#_save_pt_tx_pattern_sel_lane_5_0_b0
	movx	a,@dptr
	mov	dptr,#(_PT_CONTROL0 + 0x0002)
	rr	a
	...
	orl	a,b
	movx	@dptr,a
;	../../shared/src/misc_rom.c:240: reg_PT_TX_PATTERN_SEL_LANE_5_0_b1 = save_pt_tx_pattern_sel_lane_5_0_b1;
	mov	dptr,#_save_pt_tx_pattern_sel_lane_5_0_b1
	movx	a,@dptr
	mov	dptr,#(_PT_CONTROL0 + 0x0003)
	anl	a,#0x0f
	...
	orl	a,b
	movx	@dptr,a
;	../../shared/src/misc_rom.c:241: reg_PT_EN_LANE                    = save_pt_en_lane;
	mov	dptr,#_save_pt_en_lane
	movx	a,@dptr
	mov	dptr,#(_PT_CONTROL0 + 0x0003)
	rrc	a
	...
	mov	acc.7,c
	movx	@dptr,a
;	../../shared/src/misc_rom.c:242: reg_PT_TX_CLK_EN_LANE             = save_pt_tx_clk_en_lane;
	mov	dptr,#_save_pt_tx_clk_en_lane
	movx	a,@dptr
	mov	dptr,#(_CLKGEN_TX_LANE_REG1_LANE + 0x0002)
	rrc	a
	...
	mov	acc.6,c
	movx	@dptr,a
;	../../shared/src/misc_rom.c:243: reg_LOCAL_ANA_TX2RX_LPBK_EN_LANE  = save_local_ana_tx2rx_lpbk_en_lane;
	mov	dptr,#_save_local_ana_tx2rx_lpbk_en_lane
	movx	a,@dptr
	mov	r2,a
	mov	dptr,#_UPHY14_TRX_ANAREG_TOP_145
	...
	ljmp	__sdcc_banked_ret
;------------------------------------------------------------
;Allocation info for local variables in function 'read_tsen'
;------------------------------------------------------------
;dat                       Allocated to registers r2 r3 
;------------------------------------------------------------
;	../../shared/src/misc_rom.c:256: int16_t read_tsen(void) BANKING_CTRL {
;	-----------------------------------------
;	 function read_tsen
;	-----------------------------------------
_read_tsen:
;	../../shared/src/misc_rom.c:261: if(SIMENV_ON==0) {
	mov	dptr,#(_CONTROL_CONFIG0 + 0x0002)
	movx	a,@dptr
	anl	a,#0x1c
	jnz	00105$
	...
	movx	a,@dptr
	jb	acc.1,00105$
;	../../shared/src/misc_rom.c:262: while(reg_TSEN_ADC_RDY==0);
00101$:
	mov	dptr,#(_ANA_TSEN_CONTROL + 0x0001)
	movx	a,@dptr
	jnb	acc.2,00101$
00105$:
;	../../shared/src/misc_rom.c:265: reg_TSEN_ADC_RD_REQ = 1;
	mov	dptr,#(_ANA_TSEN_CONTROL + 0x0001)
	movx	a,@dptr
	orl	a,#0x08
	movx	@dptr,a
;	../../shared/src/misc_rom.c:267: if( reg_TSEN_ADC_DATA_9_0_b1 & 0x02 ) {
	mov	dptr,#(_ANA_TSEN_CONTROL + 0x0001)
	movx	a,@dptr
	anl	a,#0x03
	mov	r2,a
	jnb	acc.1,00107$
;	../../shared/src/misc_rom.c:268: dat = 0xfc00 | (reg_TSEN_ADC_DATA_9_0_b1<<8) | reg_TSEN_ADC_DATA_9_0_b0;
	mov	dptr,#(_ANA_TSEN_CONTROL + 0x0001)
	movx	a,@dptr
	anl	a,#0x03
	mov	r3,a
	...
	sjmp	00108$
00107$:
;	../../shared/src/misc_rom.c:271: dat = (reg_TSEN_ADC_DATA_9_0_b1<<8) | reg_TSEN_ADC_DATA_9_0_b0;
	mov	dptr,#(_ANA_TSEN_CONTROL + 0x0001)
	movx	a,@dptr
	anl	a,#0x03
	mov	r5,a
	...
	mov	ar3,r5
00108$:
;	../../shared/src/misc_rom.c:274: if(dat==0&& reg_TSEN_ADC_RDY==0) dat = -234; //return 0xfef0; //2f0=18'
	mov	a,r2
	orl	a,r3
	jnz	00110$
	mov	dptr,#(_ANA_TSEN_CONTROL + 0x0001)
	...
	mov	r3,#0xFF
00110$:
;	../../shared/src/misc_rom.c:282: return dat;
	mov	dpl,r2
	mov	dph,r3
	ljmp	__sdcc_banked_ret
;------------------------------------------------------------
;Allocation info for local variables in function 'u16div'
;------------------------------------------------------------
...
;------------------------------------------------------------
;	../../shared/src/misc_rom.c:285: uint16_t u16div(uint16_t x, uint16_t y) BANKING_CTRL
;	-----------------------------------------
;	 function u16div
;	-----------------------------------------
_u16div:
	...
	mov	r2,dpl
	mov	r3,dph
;	../../shared/src/misc_rom.c:287: uint16_t reste = 0;
	mov	r4,#0x00
	mov	r5,#0x00
;	../../shared/src/misc_rom.c:291: do
	mov	r6,#0x10
00105$:
;	../../shared/src/misc_rom.c:293: c = ( x >> 15 ) & 1;
	mov	a,r3
	rlc	a
	mov	b0,c
;	../../shared/src/misc_rom.c:294: x <<= 1;
	mov	a,r3
	xch	a,r2
	add	a,acc
	xch	a,r2
	rlc	a
	mov	r3,a
;	../../shared/src/misc_rom.c:295: reste <<= 1;
	mov	a,r5
	xch	a,r4
	add	a,acc
	xch	a,r4
	rlc	a
	mov	r5,a
;	../../shared/src/misc_rom.c:296: if (c)
	jnb	b0,00102$
;	../../shared/src/misc_rom.c:297: reste |= 1;
	orl	ar4,#0x01
00102$:
;	../../shared/src/misc_rom.c:299: if (reste >= y)
	mov	a,_bp
	add	a,#0xfb
	mov	r0,a
	clr	c
	...
	subb	a,@r0
	jc	00106$
;	../../shared/src/misc_rom.c:301: reste -= y;
	mov	a,_bp
	add	a,#0xfb
	mov	r0,a
	mov	a,r4
	...
	subb	a,@r0
	mov	r5,a
;	../../shared/src/misc_rom.c:303: x |= 1;
	orl	ar2,#0x01
00106$:
;	../../shared/src/misc_rom.c:306: while (--count);
	djnz	r6,00105$
;	../../shared/src/misc_rom.c:307: return x;
	mov	dpl,r2
	mov	dph,r3
	pop	_bp
	ljmp	__sdcc_banked_ret
;------------------------------------------------------------
;Allocation info for local variables in function 'u16mul'
;------------------------------------------------------------
;b                         Allocated to stack - offset -5
;a                         Allocated to registers r2 r3 
;x                         Allocated to stack - offset 1
;y                         Allocated to stack - offset 3
;t                         Allocated to stack - offset 5
;------------------------------------------------------------
;	../../shared/src/misc_rom.c:317: uint16_t u16mul (uint16_t a, uint16_t b) BANKING_CTRL
;	-----------------------------------------
;	 function u16mul
;	-----------------------------------------
_u16mul:
	...
	mov	r2,dpl
	mov	r3,dph
;	../../shared/src/misc_rom.c:322: x.val = a;
	mov	a,_bp
	inc	a
	mov	r0,a
	mov	@r0,ar2
	inc	r0
	mov	@r0,ar3
;	../../shared/src/misc_rom.c:323: y.val = b;
	mov	a,_bp
	add	a,#0x03
	mov	r0,a
	mov	a,_bp
	...
	mov	a,@r1
	mov	@r0,a
;	../../shared/src/misc_rom.c:325: t.val = x.s.lo * y.s.lo;
	mov	a,_bp
	add	a,#0x05
	mov	r0,a
	mov	r1,_bp
	...
	inc	r0
	mov	@r0,b
;	../../shared/src/misc_rom.c:326: t.s.hi += (x.s.lo * y.s.hi) + (x.s.hi * y.s.lo);
	mov	a,_bp
	add	a,#0x05
	inc	a
	mov	r0,a
	...
	add	a,r5
	mov	@r0,a
;	../../shared/src/misc_rom.c:328: return t.val;
	mov	a,_bp
	add	a,#0x05
	mov	r0,a
	mov	dpl,@r0
	...
	ljmp	__sdcc_banked_ret
;------------------------------------------------------------
;Allocation info for local variables in function 'short_delay'
;------------------------------------------------------------
;------------------------------------------------------------
;	../../shared/src/misc_rom.c:331: void short_delay(void) BANKING_CTRL { //100nS @400MHz
;	-----------------------------------------
;	 function short_delay
;	-----------------------------------------
_short_delay:
;	../../shared/src/misc_rom.c:340: __endasm;
	...
	ljmp	__sdcc_banked_ret
;------------------------------------------------------------
;Allocation info for local variables in function 'short_delay_0'
;------------------------------------------------------------
;------------------------------------------------------------
;	../../shared/src/misc_rom.c:342: void short_delay_0(void) BANKING_CTRL{
;	-----------------------------------------
;	 function short_delay_0
;	-----------------------------------------
_short_delay_0:
;	../../shared/src/misc_rom.c:344: }
	ljmp	__sdcc_banked_ret
;------------------------------------------------------------
...
;------------------------------------------------------------
;	../../shared/src/misc_rom.c:346: int8_t max(int8_t a, int8_t b) BANKING_CTRL{
;	-----------------------------------------
;	 function max
;	-----------------------------------------
_max:
	...
	mov	_bp,sp
	mov	r2,dpl
;	../../shared/src/misc_rom.c:347: return a>b?a:b;
	mov	a,_bp
	add	a,#0xfc
	mov	r0,a
	clr	c
...
;a                         Allocated to registers r2 
;------------------------------------------------------------
;	../../shared/src/misc_rom.c:350: int8_t min(int8_t a, int8_t b) BANKING_CTRL{
;	-----------------------------------------
;	 function min
;	-----------------------------------------
_min:
	...
	mov	_bp,sp
	mov	r2,dpl
;	../../shared/src/misc_rom.c:351: return a<b?a:b;
	mov	a,_bp
	add	a,#0xfc
	mov	r0,a
	clr	c
...
;------------------------------------------------------------
;ub                        Allocated to stack - offset -4
;db                        Allocated to stack - offset -5
;a                         Allocated to registers r2 
;------------------------------------------------------------
;	../../shared/src/misc_rom.c:354: int8_t clamp(int8_t a, int8_t ub, int8_t db) BANKING_CTRL{
;	-----------------------------------------
;	 function clamp
;	-----------------------------------------
_clamp:
	...
	mov	_bp,sp
	mov	r2,dpl
;	../../shared/src/misc_rom.c:355: return a<db?db:a>ub?ub:a;
	mov	a,_bp
	add	a,#0xfb
	mov	r0,a
	clr	c
...
00104$:
	...
	ljmp	__sdcc_banked_ret
;------------------------------------------------------------
;Allocation info for local variables in function 'clamp_u8'
;------------------------------------------------------------
;ub                        Allocated to stack - offset -4
;db                        Allocated to stack - offset -5
;a                         Allocated to registers r2 
;------------------------------------------------------------
;	../../shared/src/misc_rom.c:358: uint8_t clamp_u8(uint8_t a, uint8_t ub, uint8_t db) BANKING_CTRL{
;	-----------------------------------------
;	 function clamp_u8
;	-----------------------------------------
_clamp_u8:
	...
	mov	_bp,sp
	mov	r2,dpl
;	../../shared/src/misc_rom.c:361: return a<db?db:a>ub?ub:a;
	mov	a,_bp
	add	a,#0xfb
	mov	r0,a
	clr	c
...
;------------------------------------------------------------
;ub                        Allocated to stack - offset -5
;db                        Allocated to stack - offset -7
;a                         Allocated to registers r2 r3 
;------------------------------------------------------------
;	../../shared/src/misc_rom.c:364: int16_t clamp16(int16_t a, int16_t ub, int16_t db) BANKING_CTRL{
;	-----------------------------------------
;	 function clamp16
;	-----------------------------------------
_clamp16:
	...
	mov	r2,dpl
	mov	r3,dph
;	../../shared/src/misc_rom.c:365: return a<db?db:a>ub?ub:a;
	mov	a,_bp
	add	a,#0xf9
	mov	r0,a
	clr	c
...
;a                         Allocated to registers r2 
;------------------------------------------------------------
;	../../shared/src/misc_rom.c:368: int8_t abs(int8_t a) BANKING_CTRL{
;	-----------------------------------------
;	 function abs
;	-----------------------------------------
_abs:
;	../../shared/src/misc_rom.c:369: return a>=0?a:-a;
	mov	a,dpl
	mov	r2,a
	rlc	a
	cpl	c
...
;a                         Allocated to registers r2 r3 
;------------------------------------------------------------
;	../../shared/src/misc_rom.c:372: int16_t abs16(int16_t a) BANKING_CTRL{
;	-----------------------------------------
;	 function abs16
;	-----------------------------------------
_abs16:
	mov	r2,dpl
;	../../shared/src/misc_rom.c:373: return a>=0?a:-a;
	mov	a,dph
	mov	r3,a
	rlc	a
	cpl	c
...
;------------------------------------------------------------
;src                       Allocated to stack - offset -6
;n                         Allocated to stack - offset -8
;des                       Allocated to registers r2 r3 r4 
;------------------------------------------------------------
;	../../shared/src/misc_rom.c:386: void memcpy(uint8_t *des, uint8_t *src, int n) BANKING_CTRL{
;	-----------------------------------------
;	 function memcpy
;	-----------------------------------------
_memcpy:
	...
	mov	r3,dph
	mov	r4,b
;	../../shared/src/misc_rom.c:388: DMASEL = DMA0; 
	mov	_DMASEL,#0x00
;	../../shared/src/misc_rom.c:389: DMAM0 = 0x66; //01100110
	mov	_DMAM0,#0x66
;	../../shared/src/misc_rom.c:390: DMAM1 = 0x1f;
	mov	_DMAM1,#0x1F
;	../../shared/src/misc_rom.c:391: DMAS2 = 0; DMAT2 = 0; DMAC2 = 0;
;	../../shared/src/misc_rom.c:392: DMASA = (uint16_t)src;
	clr	a
	mov	_DMAS2,a
	mov	_DMAT2,a
	mov	_DMAC2,a
	...
	inc	r0
	mov	(_DMASA >> 8),@r0
;	../../shared/src/misc_rom.c:393: DMATA = (uint16_t)des;
	mov	_DMATA,r2
	mov	(_DMATA >> 8),r3
;	../../shared/src/misc_rom.c:394: DMAC = n - 1;
	mov	a,_bp
	add	a,#0xf8
	mov	r0,a
	mov	a,@r0
	...
	addc	a,#0xff
	mov	(_DMAC >> 8),a
;	../../shared/src/misc_rom.c:395: DMASEL = 0x80; 
	mov	_DMASEL,#0x80
;	../../shared/src/misc_rom.c:396: while(DMASEL&0x80);
00101$:
	mov	a,_DMASEL
	jb	acc.7,00101$
	pop	_bp
	ljmp	__sdcc_banked_ret
;------------------------------------------------------------
;Allocation info for local variables in function 'memcpy_prom_to_xdat'
;------------------------------------------------------------
;src                       Allocated to stack - offset -7
;n                         Allocated to stack - offset -9
;des                       Allocated to registers r2 r3 r4 r5 
;------------------------------------------------------------
;	../../shared/src/misc_rom.c:400: void memcpy_prom_to_xdat(uint32_t des, uint32_t src, uint16_t n) BANKING_CTRL{
;	-----------------------------------------
;	 function memcpy_prom_to_xdat
;	-----------------------------------------
_memcpy_prom_to_xdat:
	push	_bp
	mov	_bp,sp
	mov	r2,dpl
	mov	r3,dph
	mov	r4,b
	mov	r5,a
;	../../shared/src/misc_rom.c:402: DMASEL = DMA0; 
	mov	_DMASEL,#0x00
;	../../shared/src/misc_rom.c:403: DMAM0 = 0x62; //01100010
	mov	_DMAM0,#0x62
;	../../shared/src/misc_rom.c:404: DMAM1 = 0x1f;
	mov	_DMAM1,#0x1F
;	../../shared/src/misc_rom.c:405: DMAT2 = 0; DMAC2 = 0;
;	../../shared/src/misc_rom.c:406: DMAS2 = (uint8_t)(src>>16);
	clr	a
	mov	_DMAT2,a
	mov	_DMAC2,a
	mov	a,_bp
	...
	inc	r0
	mov	_DMAS2,@r0
;	../../shared/src/misc_rom.c:407: DMASA = (uint16_t)src;
	mov	a,_bp
	add	a,#0xf9
	mov	r0,a
	mov	_DMASA,@r0
	inc	r0
	mov	(_DMASA >> 8),@r0
;	../../shared/src/misc_rom.c:408: DMATA = (uint16_t)des;
	mov	_DMATA,r2
	mov	(_DMATA >> 8),r3
;	../../shared/src/misc_rom.c:409: DMAC = n - 1;
	mov	a,_bp
	add	a,#0xf7
	mov	r0,a
	mov	a,@r0
	...
	addc	a,#0xff
	mov	(_DMAC >> 8),a
;	../../shared/src/misc_rom.c:410: DMASEL = 0x80; 
	mov	_DMASEL,#0x80
;	../../shared/src/misc_rom.c:411: while(DMASEL&0x80);
00101$:
	mov	a,_DMASEL
	jb	acc.7,00101$
	pop	_bp
	ljmp	__sdcc_banked_ret
;------------------------------------------------------------
;Allocation info for local variables in function 'memset0'
;------------------------------------------------------------
;n                         Allocated to stack - offset -5
;dest                      Allocated to registers r2 r3 r4 
;------------------------------------------------------------
;	../../shared/src/misc_rom.c:415: void memset0(uint8_t *dest, int n) BANKING_CTRL{
;	-----------------------------------------
;	 function memset0
;	-----------------------------------------
_memset0:
	...
	mov	r3,dph
	mov	r4,b
;	../../shared/src/misc_rom.c:417: DMASEL = DMA1; 
	mov	_DMASEL,#0x01
;	../../shared/src/misc_rom.c:418: DMAM0 = 0x64; //10100100
	mov	_DMAM0,#0x64
;	../../shared/src/misc_rom.c:419: DMAM1 = 0x1f;
	mov	_DMAM1,#0x1F
;	../../shared/src/misc_rom.c:420: DMAS2 = 0; DMAT2 = 0; DMAC2 = 0;
	mov	_DMAS2,#0x00
	mov	_DMAT2,#0x00
	mov	_DMAC2,#0x00
;	../../shared/src/misc_rom.c:421: DMASA = (uint16_t)&DFE_CONTROL_2;
	mov	_DMASA,#_DFE_CONTROL_2
	mov	(_DMASA >> 8),#(_DFE_CONTROL_2 >> 8)
;	../../shared/src/misc_rom.c:422: DMATA = (uint16_t)dest;
	mov	_DMATA,r2
	mov	(_DMATA >> 8),r3
;	../../shared/src/misc_rom.c:423: DMAC = n - 1;
	mov	a,_bp
	add	a,#0xfb
	mov	r0,a
	mov	a,@r0
	...
	addc	a,#0xff
	mov	(_DMAC >> 8),a
;	../../shared/src/misc_rom.c:424: DMASEL = 0x81; 
	mov	_DMASEL,#0x81
;	../../shared/src/misc_rom.c:425: while(DMASEL&0x80);
00101$:
	mov	a,_DMASEL
	jb	acc.7,00101$
	pop	_bp
	ljmp	__sdcc_banked_ret
;------------------------------------------------------------
;Allocation info for local variables in function 'conv_thermo_2_bi'
;------------------------------------------------------------
;val                       Allocated to registers r2 
;index                     Allocated to registers r3 
;------------------------------------------------------------
;	../../shared/src/misc_rom.c:428: uint8_t conv_thermo_2_bi(uint8_t val) BANKING_CTRL{
;	-----------------------------------------
;	 function conv_thermo_2_bi
;	-----------------------------------------
_conv_thermo_2_bi:
	mov	r2,dpl
;	../../shared/src/misc_rom.c:431: while(val!=0) {
	mov	r3,#0x00
00101$:
	mov	a,r2
	jz	00103$
;	../../shared/src/misc_rom.c:432: val >>= 1; index++;
	mov	a,r2
	clr	c
	rrc	a
	mov	r2,a
	...
	sjmp	00101$
00103$:
;	../../shared/src/misc_rom.c:434: return index;
	mov	dpl,r3
	ljmp	__sdcc_banked_ret
	.area CSEG    (CODE)
	.area BANK2   (CODE)
