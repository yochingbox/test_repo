...
;--------------------------------------------------------
	.globl _rx_ffe_set_r_pcie_tb
	.globl _rx_ffe_set_r_tb
	.globl _rx_ffe_set_c_tb
	.globl _TF2
	.globl _TI
	.globl _RI
	.globl _EX12
	...
	.globl _rx_train
	.globl _rx_train_dummy
	.globl _rx_ffe_train
	.globl _set_train
	.globl _RX_FFE_Comparision
	.globl _ffe_res2_adjust
;--------------------------------------------------------
...
;--------------------------------------------------------
	.area CSEG    (CODE)
;------------------------------------------------------------
;Allocation info for local variables in function 'rx_train'
;------------------------------------------------------------
;------------------------------------------------------------
;	../../shared/src/rx_train.c:36: void rx_train()
;	-----------------------------------------
;	 function rx_train
;	-----------------------------------------
_rx_train:
	...
	ar0 = 0x00
	ar1 = 0x01
;	../../shared/src/rx_train.c:38: PHY_STATUS = ST_RXTRAIN;
	mov	dptr,#_MCU_STATUS0_LANE
	mov	a,#0x2A
	movx	@dptr,a
;	../../shared/src/rx_train.c:44: rx.ffe_index2 = opt.rx_ffe_index;
	mov	dptr,#(_opt + 0x0024)
	movx	a,@dptr
	mov	r2,a
	mov	dptr,#(_rx + 0x0002)
	movx	@dptr,a
;	../../shared/src/rx_train.c:54: ctrl_trx_ffe_updated = 0;
	clr	_ctrl_trx_ffe_updated
;	../../shared/src/rx_train.c:93: if(opt.excellent_eo && opt.saturated==0) { // if excellent EO and not saturated
	...
	jz	00105$
00104$:
;	../../shared/src/rx_train.c:97: opt2train();
	lcall	_opt2train
;	../../shared/src/rx_train.c:98: if(lnx_BYPASS_CTLE_TRAIN_LANE==0)
	mov	dptr,#_DFE_CONTROL_10
	movx	a,@dptr
	jb	acc.7,00102$
;	../../shared/src/rx_train.c:99: rx_ffe_train();
	lcall	_rx_ffe_train
00102$:
;	../../shared/src/rx_train.c:100: opt2train();
	lcall	_opt2train
00105$:
;	../../shared/src/rx_train.c:120: if(lnx_ADJUST_FFE_R2_LANE)	ffe_res2_adjust();
	mov	dptr,#(_DFE_CONTROL_7 + 0x0002)
	movx	a,@dptr
	jnb	acc.4,00108$
	lcall	_ffe_res2_adjust
00108$:
;	../../shared/src/rx_train.c:127: dfe_cdr_phase_opt();
	lcall	_dfe_cdr_phase_opt
;	../../shared/src/rx_train.c:131: ctrl_cdr_phase_on = lnx_CDRPHASE_OPT_EN_LANE && ( ctrl_rx_only_mode || rx.blind_training || tag_CDR_PHASE_LAST_ALWAYS );
...
;	../../shared/src/rx_train.c:132: adapt_data_en = tag_TRAIN_USE_D; //0
...
;	../../shared/src/rx_train.c:133: adapt_slicer_en = tag_TRAIN_USE_S; //1		
	mov	dptr,#_DFE_CONTROL_3
	movx	a,@dptr
	rl	a
	anl	a,#0x01
...
;	../../shared/src/rx_train.c:134: ph_control_mode = tag_TRAIN_PH_CONTROL_MODE; 
	mov	dptr,#_DFE_CONTROL_3
	movx	a,@dptr
	swap	a
	anl	a,#0x03
	...
	movx	@dptr,a
;	../../shared/src/rx_train.c:135: set_ph_mode();
	mov	r0,#_set_ph_mode
	mov	r1,#(_set_ph_mode >> 8)
	mov	r2,#(_set_ph_mode >> 16)
	lcall	__sdcc_banked_call
;	../../shared/src/rx_train.c:142: dfe_cdr_phase_opt();  // do the DFE and CDR phase optimization
	lcall	_dfe_cdr_phase_opt
;	../../shared/src/rx_train.c:143: update_opt_trx_ffe();	
	lcall	_update_opt_trx_ffe
;	../../shared/src/rx_train.c:146: ctrl_rx_train_done = 1; // RX training is done after going through all stages
	setb	_ctrl_rx_train_done
;	../../shared/src/rx_train.c:148: ctrl_trx_ffe_updated = (opt.rx_ffe_index != rx.ffe_index2);
	mov	dptr,#(_opt + 0x0024)
	movx	a,@dptr
	mov	r2,a
	mov	dptr,#(_rx + 0x0002)
...
00138$:
	mov  b0,c
	cpl	c
	mov	_ctrl_trx_ffe_updated,c
	ret
;------------------------------------------------------------
;Allocation info for local variables in function 'rx_train_dummy'
;------------------------------------------------------------
;------------------------------------------------------------
;	../../shared/src/rx_train.c:156: void rx_train_dummy(void) {
;	-----------------------------------------
;	 function rx_train_dummy
;	-----------------------------------------
_rx_train_dummy:
;	../../shared/src/rx_train.c:159: PHY_STATUS = ST_RXTRAIN;
	mov	dptr,#_MCU_STATUS0_LANE
	mov	a,#0x2A
	movx	@dptr,a
;	../../shared/src/rx_train.c:215: ctrl_rx_train_done = 1; // RX training is done after going through all stages
	setb	_ctrl_rx_train_done
	ret
;------------------------------------------------------------
;Allocation info for local variables in function 'rx_ffe_train'
;------------------------------------------------------------
;------------------------------------------------------------
;	../../shared/src/rx_train.c:241: void rx_ffe_train(void)
;	-----------------------------------------
;	 function rx_ffe_train
;	-----------------------------------------
_rx_ffe_train:
;	../../shared/src/rx_train.c:247: ctrl_cdr_phase_on = 0; 
	clr	_ctrl_cdr_phase_on
;	../../shared/src/rx_train.c:283: set_train(); // train the FFEs within the set
	ljmp	_set_train
;------------------------------------------------------------
;Allocation info for local variables in function 'set_train'
;------------------------------------------------------------
;i                         Allocated to registers r3 
;size_ffe_set              Allocated to registers r2 
;------------------------------------------------------------
;	../../shared/src/rx_train.c:293: void set_train(void)
;	-----------------------------------------
;	 function set_train
;	-----------------------------------------
_set_train:
;	../../shared/src/rx_train.c:300: if( tx_status_pcie_mode ) 
	jnb	_tx_status_pcie_mode,00102$
;	../../shared/src/rx_train.c:301: size_ffe_set = 6; //5; //11;
	mov	r2,#0x06
	sjmp	00118$
00102$:
;	../../shared/src/rx_train.c:303: size_ffe_set = 16;
	mov	r2,#0x10
;	../../shared/src/rx_train.c:305: while (i < size_ffe_set) //32 default, 16 fast when gain_train_with_C = 1//rx.size_ffe_set) // while the index is not the last one
00118$:
	mov	r3,#0x00
00109$:
	mov	a,r3
...
00121$:
	...
	ret
00122$:
;	../../shared/src/rx_train.c:317: if( tx_status_pcie_mode ) 
	jnb	_tx_status_pcie_mode,00105$
;	../../shared/src/rx_train.c:318: train_r = rx_ffe_set_r_pcie_tb[i];
	mov	a,r3
	add	a,r3
	mov	r4,a
	mov	dptr,#_rx_ffe_set_r_pcie_tb
	movc	a,@a+dptr
	xch	a,r4
	inc	dptr
	movc	a,@a+dptr
	...
	sjmp	00106$
00105$:
;	../../shared/src/rx_train.c:320: train_r = rx_ffe_set_r_tb[i];
	mov	a,r3
	add	a,r3
	mov	r4,a
	mov	dptr,#_rx_ffe_set_r_tb
	movc	a,@a+dptr
	xch	a,r4
	inc	dptr
	movc	a,@a+dptr
	...
	movx	@dptr,a
00106$:
;	../../shared/src/rx_train.c:322: if (tag_GAIN_TRAIN_WITH_C==0)  //1
	mov	dptr,#(_DFE_CONTROL_1 + 0x0002)
	movx	a,@dptr
	jb	acc.0,00108$
;	../../shared/src/rx_train.c:323: train_c = rx_ffe_set_c_tb[i]; // update C		
	mov	a,r3
	add	a,r3
	mov	r4,a
	mov	dptr,#_rx_ffe_set_c_tb
	movc	a,@a+dptr
	xch	a,r4
	inc	dptr
	movc	a,@a+dptr
	...
	movx	@dptr,a
00108$:
;	../../shared/src/rx_train.c:326: Set_Rx_FFE();
	push	ar2
	push	ar3
	lcall	_Set_Rx_FFE
;	../../shared/src/rx_train.c:342: dfe_cdr_phase_opt(); // adapt DFE
	lcall	_dfe_cdr_phase_opt
;	../../shared/src/rx_train.c:343: RX_FFE_Comparision(); // compare the training RX FFE with the global optimal one
	lcall	_RX_FFE_Comparision
	pop	ar3
	pop	ar2
;	../../shared/src/rx_train.c:349: i++;	
	inc	r3
	ljmp	00109$
;------------------------------------------------------------
;Allocation info for local variables in function 'RX_FFE_Comparision'
;------------------------------------------------------------
;------------------------------------------------------------
;	../../shared/src/rx_train.c:404: void RX_FFE_Comparision()
;	-----------------------------------------
;	 function RX_FFE_Comparision
;	-----------------------------------------
_RX_FFE_Comparision:
;	../../shared/src/rx_train.c:415: rx.ffe_updated = ((train.boost < opt.boost && train_c== opt_c) && train.level > 0)|| 
...
00116$:
	mov	a,r2
	jnz	00112$
;	../../shared/src/rx_train.c:416: (opt.level == 3 && train.level >= opt.level && train_c <opt_c) || 
...
00113$:
	mov	a,r2
	jnz	00109$
;	../../shared/src/rx_train.c:417: (train_c ==opt_c && train_c <tag_RX_C_SAT_THRES && train.level == 3 && 
	mov	dptr,#_train_c
	movx	a,@dptr
	mov	r2,a
	mov	dptr,#_opt_c
...
00133$:
	mov	a,r2
	jz	00128$
;	../../shared/src/rx_train.c:418: ((tag_DFE_SAT_PREFER_HIGH_DC && train_r>opt_r) || //1
	mov	dptr,#_train_r
	movx	a,@dptr
	mov	r2,a
	mov	dptr,#_opt_r
...
00142$:
	mov	a,r2
;	../../shared/src/rx_train.c:419: (!tag_DFE_SAT_PREFER_HIGH_DC && train_r<opt_r)) ) ||
	jnz	00138$
	mov	r2,a
	sjmp	00139$
00138$:
...
00110$:
	mov	a,r2
	jnz	00106$
;	../../shared/src/rx_train.c:420: (train.level > opt.level && tag_RX_LEVEL_PROTECTION_IN_MIN_BOOST);//1
	mov	dptr,#(_train + 0x0023)
	movx	a,@dptr
	mov	r2,a
	mov	dptr,#(_opt + 0x0023)
...
00106$:
	mov	r2,#0x01
00107$:
;	../../shared/src/rx_train.c:469: if (rx.ffe_updated)
	mov	dptr,#(_rx + 0x0001)
	mov	a,r2
	movx	@dptr,a
	movx	a,@dptr
	jz	00103$
;	../../shared/src/rx_train.c:471: update_opt_trx_ffe();
	ljmp	_update_opt_trx_ffe
00103$:
	ret
;------------------------------------------------------------
;Allocation info for local variables in function 'ffe_res2_adjust'
;------------------------------------------------------------
;boostlevel                Allocated to registers r2 r3 
;------------------------------------------------------------
;	../../shared/src/rx_train.c:535: void ffe_res2_adjust(void) {
;	-----------------------------------------
;	 function ffe_res2_adjust
;	-----------------------------------------
_ffe_res2_adjust:
;	../../shared/src/rx_train.c:539: boostlevel = train.sumFtap - sumf_boost_target_c;
	...
	subb	a,r5
	mov	r3,a
;	../../shared/src/rx_train.c:545: if( reg_FFE_RES1_SEL_LANE_3_0 == 0xB) {		
...
00122$:
	ret
00123$:
;	../../shared/src/rx_train.c:546: if(boostlevel  > FFE_R2_LEVEL1) { //40
	clr	c
	mov	a,#0x28
	subb	a,r2
	mov	a,#(0x00 ^ 0x80)
	...
	subb	a,b
	jnc	00111$
;	../../shared/src/rx_train.c:547: reg_FFE_RES2_SEL_E_LANE_3_0 = 4;
	mov	dptr,#_UPHY14_TRX_ANAREG_BOT_0
	movx	a,@dptr
	anl	a,#0x0f
	orl	a,#0x40
	movx	@dptr,a
;	../../shared/src/rx_train.c:548: reg_FFE_RES2_SEL_O_LANE_3_0 = 4;
	...
	ljmp	00112$
00111$:
;	../../shared/src/rx_train.c:550: else if(boostlevel  > FFE_R2_LEVEL2) { //30
	clr	c
	mov	a,#0x1E
	subb	a,r2
	mov	a,#(0x00 ^ 0x80)
	...
	subb	a,b
	jnc	00108$
;	../../shared/src/rx_train.c:551: reg_FFE_RES2_SEL_E_LANE_3_0 = 3;
	mov	dptr,#_UPHY14_TRX_ANAREG_BOT_0
	movx	a,@dptr
	anl	a,#0x0f
	orl	a,#0x30
	movx	@dptr,a
;	../../shared/src/rx_train.c:552: reg_FFE_RES2_SEL_O_LANE_3_0 = 3;
	mov	dptr,#_UPHY14_TRX_ANAREG_TOP_129
	movx	a,@dptr
	anl	a,#0x0f
	orl	a,#0x30
	...
	sjmp	00112$
00108$:
;	../../shared/src/rx_train.c:554: else if(boostlevel  > FFE_R2_LEVEL3) { //20
	clr	c
	mov	a,#0x14
	subb	a,r2
	mov	a,#(0x00 ^ 0x80)
	...
	subb	a,b
	jnc	00105$
;	../../shared/src/rx_train.c:555: reg_FFE_RES2_SEL_E_LANE_3_0 = 2;
	mov	dptr,#_UPHY14_TRX_ANAREG_BOT_0
	movx	a,@dptr
	anl	a,#0x0f
	orl	a,#0x20
	movx	@dptr,a
;	../../shared/src/rx_train.c:556: reg_FFE_RES2_SEL_O_LANE_3_0 = 2;
	mov	dptr,#_UPHY14_TRX_ANAREG_TOP_129
	movx	a,@dptr
	anl	a,#0x0f
	orl	a,#0x20
	...
	sjmp	00112$
00105$:
;	../../shared/src/rx_train.c:558: else if(boostlevel  > FFE_R2_LEVEL4) {
	clr	c
	mov	a,#0x0A
	subb	a,r2
	mov	a,#(0x00 ^ 0x80)
	...
	subb	a,b
	jnc	00102$
;	../../shared/src/rx_train.c:559: reg_FFE_RES2_SEL_E_LANE_3_0 = 1;
	mov	dptr,#_UPHY14_TRX_ANAREG_BOT_0
	movx	a,@dptr
	anl	a,#0x0f
	orl	a,#0x10
	movx	@dptr,a
;	../../shared/src/rx_train.c:560: reg_FFE_RES2_SEL_O_LANE_3_0 = 1;
	mov	dptr,#_UPHY14_TRX_ANAREG_TOP_129
	movx	a,@dptr
	anl	a,#0x0f
	orl	a,#0x10
	...
	sjmp	00112$
00102$:
;	../../shared/src/rx_train.c:563: reg_FFE_RES2_SEL_E_LANE_3_0 = 0;
	mov	dptr,#_UPHY14_TRX_ANAREG_BOT_0
	movx	a,@dptr
	anl	a,#0x0f
	movx	@dptr,a
;	../../shared/src/rx_train.c:564: reg_FFE_RES2_SEL_O_LANE_3_0 = 0;	
	mov	dptr,#_UPHY14_TRX_ANAREG_TOP_129
	movx	a,@dptr
	anl	a,#0x0f
	movx	@dptr,a
00112$:
;	../../shared/src/rx_train.c:567: r2_adj_on = 1;
	mov	dptr,#_r2_adj_on
	mov	a,#0x01
	movx	@dptr,a
	ret
