...
;--------------------------------------------------------
	...
	.globl _sync_status_lane_en
	.globl _sync_status_lane
	.globl _master_sync_request
	.globl _master_sync_clean
	.globl _slave_sync_wait
	.globl _check_master_phy_status
;--------------------------------------------------------
...
;--------------------------------------------------------
	.area BANK1   (CODE)
;------------------------------------------------------------
;Allocation info for local variables in function 'sync_status_lane_en'
;------------------------------------------------------------
;lane_en                   Allocated to registers r2 
;------------------------------------------------------------
;	../../shared/src/mcu_sync.c:16: void sync_status_lane_en(uint8_t lane_en) BANKING_CTRL {
;	-----------------------------------------
;	 function sync_status_lane_en
;	-----------------------------------------
_sync_status_lane_en:
	ar2 = 0x02
	ar3 = 0x03
	ar4 = 0x04
	ar5 = 0x05
	...
	ar1 = 0x01
	mov	r2,dpl
;	../../shared/src/mcu_sync.c:17: SYNC_STATUS_LANE_EN = lane_en;
	mov	dptr,#(_MCU_STATUS1_LANE + 0x0002)
	mov	a,r2
	movx	@dptr,a
;	../../shared/src/mcu_sync.c:18: if (mcuid == MCU_LANE0)
	mov	dptr,#_MCU_CONTROL_LANE
	movx	a,@dptr
	jnz	00102$
;	../../shared/src/mcu_sync.c:19: SYNC_STATUS_LANE0_EN = lane_en;
	mov	dptr,#(_MCU_INFO_12 + 0x0002)
	mov	a,r2
	movx	@dptr,a
	sjmp	00104$
00102$:
;	../../shared/src/mcu_sync.c:21: SYNC_STATUS_LANE1_EN = lane_en;
...
;------------------------------------------------------------
;Allocation info for local variables in function 'sync_status_lane'
;------------------------------------------------------------
;status                    Allocated to registers r2 
;------------------------------------------------------------
;	../../shared/src/mcu_sync.c:24: void sync_status_lane(uint8_t status) BANKING_CTRL {
;	-----------------------------------------
;	 function sync_status_lane
;	-----------------------------------------
_sync_status_lane:
	mov	r2,dpl
;	../../shared/src/mcu_sync.c:25: SYNC_STATUS_LANE = status;
	mov	dptr,#_MCU_STATUS1_LANE
	mov	a,r2
	movx	@dptr,a
;	../../shared/src/mcu_sync.c:26: if (mcuid == MCU_LANE0)
	mov	dptr,#_MCU_CONTROL_LANE
	movx	a,@dptr
	jnz	00102$
;	../../shared/src/mcu_sync.c:27: SYNC_STATUS_LANE0 = status;
	mov	dptr,#_MCU_INFO_12
	mov	a,r2
	movx	@dptr,a
	sjmp	00104$
00102$:
;	../../shared/src/mcu_sync.c:29: SYNC_STATUS_LANE1 = status;
	mov	dptr,#_MCU_INFO_13
	mov	a,r2
	movx	@dptr,a
00104$:
	ljmp	__sdcc_banked_ret
;------------------------------------------------------------
;Allocation info for local variables in function 'master_sync_request'
;------------------------------------------------------------
;------------------------------------------------------------
;	../../shared/src/mcu_sync.c:32: void master_sync_request(void) BANKING_CTRL	{ 
;	-----------------------------------------
;	 function master_sync_request
;	-----------------------------------------
_master_sync_request:
;	../../shared/src/mcu_sync.c:33: if(serdes_ring_lane_en==0) { 
	mov	dptr,#_serdes_ring_lane_en
	movx	a,@dptr
	mov	r2,a
	jnz	00110$
;	../../shared/src/mcu_sync.c:43: sync_status_lane(WAIT);
	mov	dpl,#0x01
	mov	r0,#_sync_status_lane
	mov	r1,#(_sync_status_lane >> 8)
	mov	r2,#(_sync_status_lane >> 16)
	lcall	__sdcc_banked_call
;	../../shared/src/mcu_sync.c:45: while (cmx_MCU_STATUS1_LANE0_31_0_b0 && cmx_MCU_STATUS1_LANE1_31_0_b0) {
00105$:
	mov	dptr,#_MCU_INFO_12
	movx	a,@dptr
	jz	00110$
	...
	movx	a,@dptr
	jz	00110$
;	../../shared/src/mcu_sync.c:46: if (cmx_LANE_DIS_LANE0_7_0==0 || cmx_LANE_DIS_LANE1_7_0==0) break;
...
00110$:
	ljmp	__sdcc_banked_ret
;------------------------------------------------------------
;Allocation info for local variables in function 'master_sync_clean'
;------------------------------------------------------------
;------------------------------------------------------------
;	../../shared/src/mcu_sync.c:51: void master_sync_clean(void) BANKING_CTRL {
;	-----------------------------------------
;	 function master_sync_clean
;	-----------------------------------------
_master_sync_clean:
;	../../shared/src/mcu_sync.c:52: if(serdes_ring_lane_en==0) { 
	mov	dptr,#_serdes_ring_lane_en
	movx	a,@dptr
	mov	r2,a
;	../../shared/src/mcu_sync.c:62: sync_status_lane(IDLE);
	jnz	00110$
	mov	dpl,a
	mov	r0,#_sync_status_lane
	mov	r1,#(_sync_status_lane >> 8)
	mov	r2,#(_sync_status_lane >> 16)
	lcall	__sdcc_banked_call
;	../../shared/src/mcu_sync.c:64: while (cmx_MCU_STATUS1_LANE0_31_0_b0 || cmx_MCU_STATUS1_LANE1_31_0_b0) {
00105$:
	mov	dptr,#_MCU_INFO_12
	movx	a,@dptr
	jnz	00106$
	...
	jz	00110$
00106$:
;	../../shared/src/mcu_sync.c:65: if(cmx_LANE_DIS_LANE0_7_0==0 || cmx_LANE_DIS_LANE1_7_0==0) break;
	mov	dptr,#(_MCU_CONFIG + 0x0001)
	movx	a,@dptr
	jz	00110$
	mov	dptr,#(_MCU_CONFIG + 0x0002)
...
00110$:
	ljmp	__sdcc_banked_ret
;------------------------------------------------------------
;Allocation info for local variables in function 'slave_sync_wait'
;------------------------------------------------------------
;------------------------------------------------------------
;	../../shared/src/mcu_sync.c:70: void slave_sync_wait(void) BANKING_CTRL {
;	-----------------------------------------
;	 function slave_sync_wait
;	-----------------------------------------
_slave_sync_wait:
;	../../shared/src/mcu_sync.c:71: if(serdes_ring_lane_en==0) {	
	mov	dptr,#_serdes_ring_lane_en
	movx	a,@dptr
	mov	r2,a
	jnz	00115$
;	../../shared/src/mcu_sync.c:73: while(SYNC_REQEST_LANE_READ!=WAIT){
00104$:
	mov	dptr,#_SYNC_INFO
	movx	a,@dptr
	mov	r2,a
	...
	sjmp	00106$
00124$:
;	../../shared/src/mcu_sync.c:74: if(cmx_LANE_DIS_LANE0_7_0==0 || cmx_LANE_DIS_LANE1_7_0==0) break;
	mov	dptr,#(_MCU_CONFIG + 0x0001)
	movx	a,@dptr
	jz	00106$
	mov	dptr,#(_MCU_CONFIG + 0x0002)
	...
	jnz	00104$
00106$:
;	../../shared/src/mcu_sync.c:77: sync_status_lane(ACK);
	mov	dpl,#0x01
	mov	r0,#_sync_status_lane
	mov	r1,#(_sync_status_lane >> 8)
	mov	r2,#(_sync_status_lane >> 16)
	lcall	__sdcc_banked_call
;	../../shared/src/mcu_sync.c:88: while(SYNC_REQEST_LANE_READ!=IDLE){
00110$:
	mov	dptr,#_SYNC_INFO
	movx	a,@dptr
	jz	00112$
;	../../shared/src/mcu_sync.c:89: if(cmx_LANE_DIS_LANE0_7_0==0 || cmx_LANE_DIS_LANE1_7_0==0) break;
	mov	dptr,#(_MCU_CONFIG + 0x0001)
	movx	a,@dptr
	jz	00112$
	mov	dptr,#(_MCU_CONFIG + 0x0002)
	...
	jnz	00110$
00112$:
;	../../shared/src/mcu_sync.c:92: sync_status_lane(IDLE);
	mov	dpl,#0x00
	mov	r0,#_sync_status_lane
	mov	r1,#(_sync_status_lane >> 8)
	mov	r2,#(_sync_status_lane >> 16)
...
00115$:
	ljmp	__sdcc_banked_ret
;------------------------------------------------------------
;Allocation info for local variables in function 'check_master_phy_status'
;------------------------------------------------------------
;cmd                       Allocated to registers r2 
;------------------------------------------------------------
;	../../shared/src/mcu_sync.c:126: void check_master_phy_status(uint8_t cmd) BANKING_CTRL {
;	-----------------------------------------
;	 function check_master_phy_status
;	-----------------------------------------
_check_master_phy_status:
	mov	r2,dpl
;	../../shared/src/mcu_sync.c:128: if((reg_PHY_CONFIG_1_0&0x03)==0x3) {
	mov	dptr,#_SYSTEM
	movx	a,@dptr
	rl	a
	rl	a
...
00123$:
	ljmp	00109$
00124$:
;	../../shared/src/mcu_sync.c:131: sync_check = 0;
	clr	_sync_check
;	../../shared/src/mcu_sync.c:132: do {
00101$:
;	../../shared/src/mcu_sync.c:133: reg_MCU_LOCAL_COMMAND_7_0 = cmd;
	mov	dptr,#_MCU_SYNC1
	mov	a,r2
	movx	@dptr,a
;	../../shared/src/mcu_sync.c:134: reg_MCU_LOCAL_REQ = 0;		
;	../../shared/src/mcu_sync.c:135: reg_MCU_LOCAL_REQ = 1;		
	mov	dptr,#(_MCU_SYNC1 + 0x0001)
	movx	a,@dptr
	anl	a,#0xfe
	movx	@dptr,a
	movx	a,@dptr
	orl	a,#0x01
	movx	@dptr,a
;	../../shared/src/mcu_sync.c:137: delay01(10);	
	mov	dptr,#0x000A
	push	ar2
	mov	r0,#_delay01
	mov	r1,#(_delay01 >> 8)
	...
	lcall	__sdcc_banked_call
	pop	ar2
;	../../shared/src/mcu_sync.c:139: sync_check = (reg_MCU_REMOTE_ACK_RD==1 && reg_MCU_REMOTE_STATUS_RD_7_0==cmd);				
	mov	dptr,#(_MCU_SYNC2 + 0x0003)
	movx	a,@dptr
	anl	a,#0x01
	mov	r3,a
	cjne	r3,#0x01,00111$
	mov	dptr,#(_MCU_SYNC2 + 0x0002)
	movx	a,@dptr
	mov	r3,a
	cjne	a,ar2,00127$
	sjmp	00112$
...
00113$:
	mov	c,b0
	mov	_sync_check,c
;	../../shared/src/mcu_sync.c:141: } while(!sync_check);				
;	../../shared/src/mcu_sync.c:144: sync_check = 0;
	jbc	_sync_check,00128$
	sjmp	00101$
00128$:
;	../../shared/src/mcu_sync.c:145: do {
00104$:
;	../../shared/src/mcu_sync.c:146: reg_MCU_LOCAL_COMMAND_7_0 = CHECK_CLEAR;
	mov	dptr,#_MCU_SYNC1
	clr	a
	movx	@dptr,a
;	../../shared/src/mcu_sync.c:147: reg_MCU_LOCAL_REQ = 0;		
;	../../shared/src/mcu_sync.c:148: reg_MCU_LOCAL_REQ = 1;		
	mov	dptr,#(_MCU_SYNC1 + 0x0001)
	movx	a,@dptr
	anl	a,#0xfe
	movx	@dptr,a
	...
	orl	a,#0x01
	movx	@dptr,a
;	../../shared/src/mcu_sync.c:150: delay01(10);	
	mov	dptr,#0x000A
	mov	r0,#_delay01
	mov	r1,#(_delay01 >> 8)
	mov	r2,#(_delay01 >> 16)
	lcall	__sdcc_banked_call
;	../../shared/src/mcu_sync.c:152: sync_check = (reg_MCU_REMOTE_ACK_RD==0 && reg_MCU_REMOTE_STATUS_RD_7_0==CHECK_CLEAR);				
	mov	dptr,#(_MCU_SYNC2 + 0x0003)
	movx	a,@dptr
	anl	a,#0x01
	mov	r2,a
...
00116$:
	mov	c,b0
;	../../shared/src/mcu_sync.c:154: } while(!sync_check);		
	mov	_sync_check,c
	jnc	00104$
00109$:
	ljmp	__sdcc_banked_ret
