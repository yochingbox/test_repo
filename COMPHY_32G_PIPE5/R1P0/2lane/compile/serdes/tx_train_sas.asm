...
;--------------------------------------------------------
	.area CSEG    (CODE)
;------------------------------------------------------------
;Allocation info for local variables in function 'tx_train_control'
;------------------------------------------------------------
;------------------------------------------------------------
;	../../shared/src/tx_train_sas.c:36: void tx_train_control(void)
;	-----------------------------------------
;	 function tx_train_control
;	-----------------------------------------
_tx_train_control:
	ar2 = 0x02
	ar3 = 0x03
	...
	ar0 = 0x00
	ar1 = 0x01
;	../../shared/src/tx_train_sas.c:39: if ( (tx.num_tx_loop >= tag_TX_MAX_LOOP) || //1
	mov	dptr,#(_tx + 0x0002)
	movx	a,@dptr
	mov	r2,a
	cjne	r2,#0x01,00111$
00111$:
	jnc	00101$
;	../../shared/src/tx_train_sas.c:40: ( (train.eo >= tag_TX_EO_BYPASS_THRES) && train.saturated==0) || //tag_TX_EO_BYPASS_THRES=60 //15
...
00113$:
	...
	jz	00101$
00106$:
;	../../shared/src/tx_train_sas.c:41: (train.level >= 5))
...
00116$:
	jc	00102$
00101$:
;	../../shared/src/tx_train_sas.c:44: ctrl_tx_train_done = 1;
	setb	_ctrl_tx_train_done
;	../../shared/src/tx_train_sas.c:45: tx.num_tx_loop = 0;
	mov	dptr,#(_tx + 0x0002)
	clr	a
	movx	@dptr,a
	sjmp	00103$
00102$:
;	../../shared/src/tx_train_sas.c:48: ctrl_tx_train_done = 0;
	clr	_ctrl_tx_train_done
;	../../shared/src/tx_train_sas.c:49: tx.num_tx_loop ++; // update counter of Number of TX loop by adding one
	inc	r2
	mov	dptr,#(_tx + 0x0002)
	mov	a,r2
	movx	@dptr,a
00103$:
;	../../shared/src/tx_train_sas.c:51: tx.blind_training = 0;//(tag_BLIND_TRAIN_EN && !opt.eye_check_pass); //tag_BLIND_TRAIN_EN=0
	mov	dptr,#_tx
	clr	a
	movx	@dptr,a
	ret
;------------------------------------------------------------
;Allocation info for local variables in function 'tx_ffe_train'
;------------------------------------------------------------
;------------------------------------------------------------
;	../../shared/src/tx_train_sas.c:69: void tx_ffe_train(void)
;	-----------------------------------------
;	 function tx_ffe_train
;	-----------------------------------------
_tx_ffe_train:
;	../../shared/src/tx_train_sas.c:76: if( (train.level < 5) &&
	mov	dptr,#(_train + 0x0023)
	movx	a,@dptr
	mov	r2,a
	cjne	r2,#0x05,00118$
00118$:
	jnc	00111$
;	../../shared/src/tx_train_sas.c:77: ((train.eo < tag_TX_EO_BYPASS_THRES) || train.saturated))
	mov	dptr,#(_train + 0x001f)
	movx	a,@dptr
	mov	r2,a
	cjne	r2,#0x3C,00120$
00120$:
	...
	jz	00111$
00107$:
;	../../shared/src/tx_train_sas.c:82: if(tx_adapt_g0_en)      tx_g0_train();     //tag_TX_ADAPT_G0_EN=1
	mov	dptr,#(_TRAIN_CONTROL_2 + 0x0002)
	movx	a,@dptr
	jnb	acc.7,00102$
	lcall	_tx_g0_train
00102$:
;	../../shared/src/tx_train_sas.c:83: if(tx_adapt_gn1_en)     tx_gn1_train();    //tag_TX_ADAPT_GN1_EN=1
	mov	dptr,#(_TRAIN_CONTROL_2 + 0x0002)
	movx	a,@dptr
	jnb	acc.6,00104$
	lcall	_tx_gn1_train
00104$:
;	../../shared/src/tx_train_sas.c:84: if(tx_adapt_g1_en)      tx_g1_train();     //tag_TX_ADAPT_G1_EN=0
	mov	dptr,#(_TRAIN_CONTROL_2 + 0x0002)
	movx	a,@dptr
	jnb	acc.5,00111$
	ljmp	_tx_g1_train
