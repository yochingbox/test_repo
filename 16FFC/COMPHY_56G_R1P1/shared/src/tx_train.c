...
void tx_ffe_train(void)
{
	...
	#endif 
	{
				...
				((train.eo < tag_TX_EO_BYPASS_THRES) || train.saturated))
		{
			...
			//TODO if(tx_adapt_gn2_en)     tx_gn2_train();     //tag_TX_ADAPT_GN2_EN=0
		}
	}
}
...
void tx_reset(uint8_t index) {
	...
	req_local_ctrl(index, 0, 0, 0, 0); //TODO
	/*drv_TXTRAIN_CTRL_PRESET = index;
	set_remote_tx();*/
}
	...
	if (update > 0 && tx.g0_is_max==0)
	{
		while(cnt-->0){
			if(update > 0 &&  tx.g0_is_max==0){
				...
				/*drv_TXTRAIN_CTRL_PRESET = CE_NORMAL;
				drv_TXTRAIN_CTRL_C1 = 0;
				drv_TXTRAIN_CTRL_C0 = 1;
				drv_TXTRAIN_CTRL_CN1 = 0;
				drv_TXTRAIN_CTRL_CN2 = 0;
				set_remote_tx();*/
...
{            
	...
	if (update > 0 && tx.g1_is_max==0)
	{
		...
		while(cnt-->0) {
			...
			//g1_request = -1;
            //g0_request  = -tag_TX_TRAIN_P2P_HOLD;
				req_local_ctrl(CE_NORMAL, 2, (tag_TX_TRAIN_P2P_HOLD ? 2 : 0), 0, 0);		
...
{             
	...
	if (update > 0 && tx.gn2_is_max==0)
	{
		while(cnt-->0){		
			...
			//gn1_request = 1;
            //g0_request  = tag_TX_TRAIN_P2P_HOLD;
				req_local_ctrl(CE_NORMAL, 0, tag_TX_TRAIN_P2P_HOLD, 0, 1);		
		...
		while(cnt-->0) 
		{
			if(update <0 && tx.gn2_is_min==0)
			{
				//gn2_request = -1;
				//g0_request  = -tag_TX_TRAIN_P2P_HOLD;
				req_local_ctrl(CE_NORMAL, 0, (tag_TX_TRAIN_P2P_HOLD ? 2 : 0), 0, 2);		
