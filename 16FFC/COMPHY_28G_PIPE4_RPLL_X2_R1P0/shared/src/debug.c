...
#ifdef USE_BANKING
#if !defined _28G_X2 && !defined _28G_X4
...
void debug_print(void) BANKING_CTRL{
	...
	Printf(" fn1=%d", train.fn1);
}

#if _DBG_TRAIN_SAVE
void debug_print_train_result(void) BANKING_CTRL{
	Printf("\r\ntxffe(%d,%d,%d-,",reg_TX_EM_PEAK_CTRL_LANE_3_0, reg_TX_EM_PO_CTRL_LANE_3_0, reg_TX_EM_PRE_CTRL_LANE_3_0);
	Printf("%d,%d,%d),",reg_ANA_TX_EM_PEAK_CTRL_RD_LANE_3_0_b1*4+reg_ANA_TX_EM_PEAK_CTRL_RD_LANE_3_0_b0, reg_ANA_TX_EM_POST_CTRL_RD_LANE_3_0, reg_ANA_TX_EM_PRE_CTRL_RD_LANE_3_0);
	Printf("\r\nffe(%d,%d,%d-%d,%d,%d),",reg_FFE_RES1_SEL_LANE_3_0,reg_FFE_RES2_SEL_E_LANE_3_0, reg_FFE_RES2_SEL_O_LANE_3_0,reg_FFE_CAP1_SEL_LANE_3_0,reg_FFE_CAP2_SEL_E_LANE_3_0,reg_FFE_CAP2_SEL_O_LANE_3_0);
	Printf("dfe_res(%d,%d,%d,%d)",ctrl_step_res_dfe,dfe_f1_res_sel, dfe_f234_res_sel,dfe_f567_res_sel);
	Printf("ph(%d-%d,%d,%d)",cdr_align90,reg_ALIGN90_GM_LANE_2_0, reg_ALIGN90_DAC_LANE_5_0,reg_ALIGN90_DUMMY_CLK_LANE);
}
#endif
...
void print_osft_f1p(void) BANKING_CTRL {
	...
	Printf(" ofst(p/n_D/S_e,p/n_D/S_o,ee,eo)=", dat);
	for(i=0; i<10; i++) {
		sampler_sel(i);
		dat = get_sampler();
		Printf("%d,", dat);
