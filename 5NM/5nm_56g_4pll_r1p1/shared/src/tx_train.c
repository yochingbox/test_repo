...
#define _DEBUG1 _DBG_TRXTRAIN_LVL1
...
void tx_gx_adjust(int8_t x) {
	...
	train_cds = CDS_TRX_TRAIN; //cds_call = CDS_EN_PHASETRAIN2;
...
void tx_train(void)
{
	...
	if (!(TRAIN_STEP & TX_TRAIN_ST) || !do_train) return;
	if (opt.excellent_eo && (opt.saturated==0)) {
		#if _DEBUG
		DBG_Printf("\r\n  (tx_train) - excellent! no need");
		#endif
		return;
	}		
	if(ctrl_tx_train_done) return;
	if(cmx_TRAIN_SIM_EN && phy_mode!=PCIE) return;
	...
	while (1)
	{
		#ifdef _PCIE_BUILD
		{
			if( reg_EYE_OPEN_EN_RD_LANE ) {
				...
				{
					train_cds = CDS_PCIE_TRAIN; //cds_call = CDS_EN_DEFAULTPCIE;
					dfe_adaptation();					
				}					
				...
				if(!cmx_TRAIN_SIM_EN) {
					train_cds = CDS_PCIE_TRAIN_F0B; //cds_call = CDS_EN_DEFAULTPCIE_F0B;
					tx_train_PCIe();		
					train_cds = CDS_PCIE_TRAIN; //cds_call = CDS_EN_DEFAULTPCIE;
				}
				else {
		    		    	req_local_ctrl(CE_NORMAL, 0, (tag_TX_TRAIN_P2P_HOLD?2:0), 2, 0);
...
void tx_train_dummy(void) {
	...
	if (!(TRAIN_STEP & TX_TRAIN_DONE) || !do_train) return;
	...
	if(cmx_TRAIN_SIM_EN) {
	       ...
	       req_local_ctrl(CE_NORMAL, 2, 0, 0, 0); //g1--
	       req_local_ctrl(CE_NORMAL, 0, 1, 0, 0); //g0++
	...
	#if _DEBUG
        DBG_Printf("\r\n ++(tx_train_dummy):eye_pass:%d ctrl_num_big_loop:%d",opt.eye_check_pass, ctrl_num_big_loop);
	#endif
	ctrl_num_big_loop++;
	if (opt.eye_check_pass || ctrl_num_big_loop >tag_MAX_BIG_LOOP ) ctrl_trx_train_done = 1;
	else traintb_inx = ST_FIRST_PHASETRAIN;//ST_RSWEEP;
...
void tx_train_control(void)
{
	...
	if ( (tx.num_tx_loop >= (tx_pam2_en?1:tag_TX_MAX_LOOP)) || //1
...
void tx_ffe_train(void)
{
	...
	#endif 
	{
		...
		{
			#if (tag_TX_SWEEP_PRESET_EN)  //tag_TX_SWEEP_PRESET_EN=0
				tx_preset_sweep(); 
			#endif
			if (tx_pam2_en){
				// break sum 63 by g0 to let min/max only mean one fir's own min/max
			...
			if(tag_TX_ADAPT_GN2_EN && tx_pam2_en==0)     tx_gx_adjust(-2);  //tag_TX_ADAPT_GN2_EN=1
...
{ /* find arg: /\<arg\>\|\<th\>\|\<g\> */
	...
	else if (opt.level == train.level) // intra-level compare
	{
		...
		{			
			...
			tx.become_worse = (tag_TX_STOP_IF_WORSE && train.f0t < (opt.f0t - tag_TX_STOP_THRES) || train.f0t <= tag_TX_EO_STOP_THRES );
		}
...
{           
	int8_t cnt = 16;
	#if _DEBUG1
    DBG_Printf("\r\n (update_g0) update= %d  min/max(%d %d)",update, tx.g0_is_min, tx.g0_is_max);
	#endif
	...
	if (update > 0 && tx.g0_is_max==0)
	{
		while(cnt-->0){
			#if _DEBUG1
			DBG_Printf("\r\n update=+%d, tx.g0_is_max=%d, valid=%d",update, tx.g0_is_max,drv_TXTRAIN_STATUS_VALID);
			#endif
			if(update > 0 &&  tx.g0_is_max==0){
				req_local_ctrl(CE_NORMAL, 0, 1, 0, 0);
		...
		{
			#if _DEBUG1
			DBG_Printf("\r\n update=%d, tx.g0_is_min=%d, valid=%d", update, tx.g0_is_min, drv_TXTRAIN_STATUS_VALID);
			#endif
...
{       
	...
	int8_t cnt = 16;
	#if _DEBUG1
	DBG_Printf("\r\n  (update_gn1) update= %d min/max(%d %d)",update, tx.gn1_is_min, tx.gn1_is_max);
	#endif
	...
	if (update > 0 && tx.gn1_is_max==0)
	{
		while(cnt-->0){		
			#if _DEBUG1
			DBG_Printf("\r\n update=+%d tx.gn1_is_max=%d valid=%d",update, tx.gn1_is_max,drv_TXTRAIN_STATUS_VALID);
			#endif
			if(update >0 && tx.gn1_is_max==0){
				...
				if(tag_TX_TRAIN_P2P_HOLD) {
					req_local_ctrl(CE_NORMAL, 0, 1, 0, 0);
				   	if(tx.g0_is_max) {
					    req_local_ctrl(CE_NORMAL, 0, 0, 2, 0); //revert
					    break;	
					}
				}
		...
		{
			#if _DEBUG1
			DBG_Printf("\r\n update=%d tx.gn1_is_min=%d valid=%d", update, tx.gn1_is_min, drv_TXTRAIN_STATUS_VALID);
			#endif
			if(update <0 && tx.gn1_is_min==0)
			{
				...
				if(tx.gn1_is_min) {
					if(pam2_en) req_local_ctrl(CE_NORMAL, 0, 0, 1, 0); //revert
					break;	
				}
				if(tag_TX_TRAIN_P2P_HOLD)  {
					req_local_ctrl(CE_NORMAL, 0, 2, 0, 0);		
					if(tx.g0_is_min) {
						if(pam2_en) {
						req_local_ctrl(CE_NORMAL, 0, 1, 0, 0);
						req_local_ctrl(CE_NORMAL, 0, 0, 1, 0); //revert
						}
				    		break;	
					}
				}
...
{            
	...
	int8_t cnt = 16;
 	#if _DEBUG1
	DBG_Printf("\r\n (update_g1) update= %d min max(%d %d)",update, tx.g1_is_min, tx.g1_is_max);
	#endif
	...
	if (update > 0 && tx.g1_is_max==0)
	{
		while(cnt-->0){
			#if _DEBUG1
			DBG_Printf("\r\n update=+%d tx.g1_is_max=%d, valid=%d",update, tx.g1_is_max, drv_TXTRAIN_STATUS_VALID);
			#endif
			if(update >0 && tx.g1_is_max==0){
				...
				if(tag_TX_TRAIN_P2P_HOLD) {
					req_local_ctrl(CE_NORMAL, 0, 1, 0, 0);	
				   	if(tx.g0_is_max) {
						req_local_ctrl(CE_NORMAL, 2, 0, 0, 0); //revert
					}
				}
		...
		{
			#if _DEBUG1
			DBG_Printf("\r\n update=%d tx.g1_is_min=%d valid=%d",update, tx.g1_is_min, drv_TXTRAIN_STATUS_VALID);
			#endif
			if(update <0 && tx.g1_is_min==0)
			{
				if(tag_TX_TRAIN_P2P_HOLD) {
					req_local_ctrl(CE_NORMAL, 0, 2, 0, 0);
					if(tx.g0_is_min) break;		
				}		
				req_local_ctrl(CE_NORMAL, 2, 0, 0, 0);
				if(tx.g1_is_min) {
					req_local_ctrl(CE_NORMAL, 0, 1, 0, 0); //revert
				    	break;	
				}
...
{             
	...
	int8_t cnt = 16;
	#if _DEBUG1
	DBG_Printf("\r\n  (update_gn2) update= %d min max(%d %d)",update, tx.gn2_is_min, tx.gn2_is_max);
	#endif
	...
	if (update > 0 && tx.gn2_is_max==0)
	{
		while(cnt-->0){		
			#if _DEBUG1
			DBG_Printf("\r\n update=%d tx.gn2_is_max=%d valid=%d",update, tx.gn2_is_max,drv_TXTRAIN_STATUS_VALID);
			#endif
			if(update > 0 && tx.gn2_is_max==0){

				if(tag_TX_TRAIN_P2P_HOLD) {
					req_local_ctrl(CE_NORMAL, 0, 2, 0, 0);
					success = drv_TXTRAIN_STATUS_C0==1;
					if(tx.g0_is_min) break;
				}
				req_local_ctrl(CE_NORMAL, 0, 0, 0, 1);

				if(tag_TX_TRAIN_P2P_HOLD && (drv_TXTRAIN_STATUS_VALID==0 || drv_TXTRAIN_STATUS_CN2!=1)) {
					req_local_ctrl(CE_NORMAL, 0, 1, 0, 0);
					break; 				
				}
		...
		while(cnt-->0) {
			...
			#if _DEBUG1
			DBG_Printf("\r\n update=%d tx.gn2_is_min=%d valid=%d",update, tx.gn2_is_min, drv_TXTRAIN_STATUS_VALID);
			#endif

			if(update < 0 && tx.gn2_is_min==0){
				req_local_ctrl(CE_NORMAL, 0, 0, 0, 2);
				if(tx.gn2_is_min) break;
