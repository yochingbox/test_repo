...
void dc_vref_adj() BANKING_CTRL
{
    ...
    for (j=0; j<40; ++j) {  // Can transfer as much as 40 DC codes. 40 is large enough
        ...
        for (i=0; i<NUM_PATH*NUM_DS; ++i) {
            ...
            if(reg_DFE_ADAPT_SPLR_EN_LANE_3_0==0)
                continue;
            dfe_fsm(0, 0);  // adapt_lpnum = 0
        }
        // Adaptation, adapt_lpnum=4, pol_lpnum=0xFF
        reg_DFE_ADAPT_SPLR_EN_LANE_3_0 = 0xF;
        reg_DFE_ADAPT_SPLR_EN_LANE_3_0 &= ~reg_CDS_ADAPT_SPLR_DIS_LANE_3_0;
        dfe_fsm(0x4FF, Updatetrack);
    }
