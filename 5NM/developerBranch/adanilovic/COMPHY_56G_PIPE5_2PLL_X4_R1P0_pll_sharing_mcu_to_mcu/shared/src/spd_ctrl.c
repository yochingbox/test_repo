...
void SpeedChange(void) BANKING_CTRL{
    ...
    pll_drv_pll_spd_chg_reset_pll_ts();
    pll_drv_pll_spd_chg_reset_pll_rs();
    ...
    pll_drv_pll_spd_chg_clr_reset_pll_rs();
		
	if(phy_mode != SERDES){
		if(pll_sel != RS_ON) {
            pll_drv_pll_spd_chg_pll_cal_tx();
        }

		if(pll_sel != TS_ON) {
            pll_drv_pll_spd_chg_pll_cal_rx();
        }
