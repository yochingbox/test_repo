    ...
    for (dfe_cnt=0;dfe_cnt<dfe_max_cnt;dfe_cnt++)
	{
		...
		cds_save(0x00);//Savf0a);
			...
			if(lnx_CDS_CALL_FORCE_EN_LANE) {
				drv_cdr_dfe_scheme(CDS_TB.VAL);
			}
			else 
			#endif
			{
				drv_cdr_dfe_scheme(ctrl_cdr_phase_on ? cds_table[CDS_EN_PHASETRAIN] : cds_table[cds_call]);
			}
		}
...
if((lnx_CLI_CMD_LANE_7_0 == 0xF8 || lnx_CLI_CMD_LANE_7_0 == 0xE6) && lnx_CLI_START_LANE == 1)
{
		...
		if(rate_mode==QR){
			...
			sum_f0xp1 += combine_f0(cds56.s.f0x);	
		}
	...
	#if _DEBUG
	DBG_Printf("\r\n [%06lx], %d =", (uint32_t)cds_table[cds_call], (reg_RX_ALIGN90_CAL_SETTING_LANE_7_0&0x7f));
	//DBG_Printf(" %d, %d, %d ", ctrl_max_dfe_adapt_shift, dfe_max_cnt, half_shift );
    DBG_Printf(" train.f: %d %d %d %d %d %d %d %d %d, eo:%d, eye_ck_p:%d", train.f0a, train.f0, train.f1, train.f2, train.f3, train.f4, train.f5, train.f6, train.f7, train.eo, train.eye_check_pass);
    DBG_Printf(" (r,c):(%d,%d), dfe_res:%d, ",train.r,train.c,ctrl_step_res_dfe);
	
    DBG_Printf(" saturated:%d, sumFtap:%d, boost:%d lvl:%d, phase:%d(%d), esm:%d(%d), cal_ph=%d", train.saturated,train.sumFtap,train.boost, train.level, train.phase_offset_data, (reg_RX_ALIGN90_CAL_SETTING_LANE_7_0&0x7f), train.phase_offset_esm, (reg_RX_EOM_PI_CAL_RESULT_EXT_LANE_10_8<<8+reg_RX_EOM_PI_CAL_RESULT_EXT_LANE_7_0), lnx_CAL_PHASE_LANE_7_0);
	#endif
...
static void DFE_Final_Calculation( void ) {
	...
	case HR:
		...
		f3_res = dfe_res_f5to8_sumFtap[dfe_f34_res_sel];
		f4_res = dfe_res_f5to8_sumFtap[dfe_f567_res_sel];
		f5_res = dfe_res_f9to14_sumFtap[dfe_f8to10_res_sel];
		f6_res = dfe_res_f9to14_sumFtap[dfe_f8to10_res_sel];
		f7_res = dfe_res_f9to14_sumFtap[dfe_f8to10_res_sel];
	...
	case FR:
		...
		f1_res = dfe_res_f3to4_sumFtap[dfe_f34_res_sel];
		f2_res = dfe_res_f5to8_sumFtap[dfe_f567_res_sel];
		f3_res = dfe_res_f9to14_sumFtap[dfe_f8to10_res_sel];
...
//	else
//	{
//		//train.f0a_max = (max(f0a_max_data, f0a_max_slicer), + half_shift) >> ctrl_max_dfe_adapt_shift;
//		train.f0a_max = (uint8_t)(max(f0a_max_data, f0a_max_slicer));
//	}
//#endif
	train.f0_data = (uint8_t)((((combine_f0(cds56.s.f0k) + (f0d_factor>>1))/f0d_factor) + half_shift) >> ctrl_max_dfe_adapt_shift);
	...
	//	train.fn2 = 0;
	//} else 
	{ // normal behavior (fn2 measurement)
			...
			for (p_idx = 0; p_idx < NUM_PATH; p_idx++) {
				if (en_p[rate_mode][p_idx]) {
					for (tmb_idx = 0; tmb_idx < NUM_TMB; tmb_idx++){
						...
						switch(tap_id) {
						case 1:
							sum += dfe_2c[p_idx].f1[ds_idx][tmb_idx];
							break;
						case 2:
	...
	for (p_idx = 0; p_idx < NUM_PATH; p_idx++) {
		if (en_p[rate_mode][p_idx]) {
			for (tmb_idx = 0; tmb_idx < NUM_TMB; tmb_idx++){
				switch(tap_id) {
					case 3:
						sum += dfe_2c[p_idx].f3[tmb_idx];
						break;
					case 4:
						sum += dfe_2c[p_idx].f4[tmb_idx];
						break;
				}
			}
		}
	...
	for (p_idx = 0; p_idx < NUM_PATH; p_idx++) {
		if (en_p[rate_mode][p_idx]) {
			for (ml_idx = 0; ml_idx < NUM_ML; ml_idx++){
				switch(tap_id) {
					case 5:
						sum += dfe_2c[p_idx].f5[ml_idx] << ml_idx;
						break;
					case 6:
						sum += dfe_2c[p_idx].f6[ml_idx] << ml_idx;
						break;
					case 7:
						sum += dfe_2c[p_idx].f7[ml_idx] << ml_idx;
						break;
					case 8:
						sum += dfe_2c[p_idx].f8[ml_idx] << ml_idx;
						break;
					case 9:
						sum += dfe_2c[p_idx].f9[ml_idx] << ml_idx;
						break;
					case 10:
...
static void combine_vref_f(const dfe_2c_t *dfe_2c, int16_t *tmb)  {
	...
	if (en_p[rate_mode][0]) {//even
		...
		tmb[0] += dfe_2c->vref_top_e;
		tmb[2] += dfe_2c->vref_bot_e;
	}
	if (en_p[rate_mode][1]) {//odd
		tmb[0] += dfe_2c->vref_top_o;
		tmb[1] += dfe_2c->vref_mid_o;
		tmb[2] += dfe_2c->vref_bot_o;
	}
}
...
static int16_t combine_ml_tap_f(const dfe_2c_t *dfe_2c, uint8_t tap_id)  {
	...
	if (en_p[rate_mode][0])	{
		switch(tap_id) {
		case 5:
			sum += dfe_2c->f5_msb_e << 1; sum += dfe_2c->f5_lsb_e;
			break;                        
		case 6:                           
			sum += dfe_2c->f6_msb_e << 1; sum += dfe_2c->f6_lsb_e;
			break;                        
		case 7:                           
			sum += dfe_2c->f7_msb_e << 1; sum += dfe_2c->f7_lsb_e;
			break;                        
		case 8:                           
			sum += dfe_2c->f8_msb_e << 1; sum += dfe_2c->f8_lsb_e;
			break;                        
		case 9:                           
			sum += dfe_2c->f9_msb_e << 1; sum += dfe_2c->f9_lsb_e;
			break;                        
		case 10:                          
			sum += dfe_2c->f10_msb_e << 1; sum += dfe_2c->f10_lsb_e;
			break;
		}
	}
	if (en_p[rate_mode][1]) {
		switch(tap_id) {
		case 5:
			sum += dfe_2c->f5_msb_o << 1; sum += dfe_2c->f5_lsb_o;
			break;                        
		case 6:                           
			sum += dfe_2c->f6_msb_o << 1; sum += dfe_2c->f6_lsb_o;
			break;                        
		case 7:                           
			sum += dfe_2c->f7_msb_o << 1; sum += dfe_2c->f7_lsb_o;
			break;                        
		case 8:                           
			sum += dfe_2c->f8_msb_o << 1; sum += dfe_2c->f8_lsb_o;
			break;                        
		case 9:                           
			sum += dfe_2c->f9_msb_o << 1; sum += dfe_2c->f9_lsb_o;
			break;                        
		case 10:                          
			sum += dfe_2c->f10_msb_o << 1; sum += dfe_2c->f10_lsb_o;
			break;
		}
	}
...
static void cds_calculation(void) { 
	...
	#ifdef _56G_5NM
	combine_vref(cds56.s.dfe_2c, sum_vref);
	#else
	...
	for (idx = 1; idx < 8; idx++) {
		...
		if (rate_idx < 3) {
			// Sampler Taps f[1:2]
			#ifdef _56G_5NM
			sum_f[idx] += combine_splr_tap(cds56.s.dfe_2c, rate_idx);
			#else
			...
			#ifdef _56G_5NM
			sum_f[idx] += combine_tmb_tap(cds56.s.dfe_2c, rate_idx);
			#else
			...
			#ifdef _56G_5NM
			sum_f[idx] += combine_ml_tap(cds56.s.dfe_2c, rate_idx);
			#else
