...
void rx_sq_ofst_init(void) BANKING_CTRL {
	...
	// TODO - from 112G_A code; The below line caused the DONE bit not assert in simulation
	//reg_SQ_CAL_INDV_EXT_EN_LANE = 0;

	reg_RX_SQ_CAL_EN_LANE = 1;
	reg_RX_SQ_OFFSET_CAL_EN_LANE = 1;
	reg_RX_SQ_THRESH_CAL_EN_LANE = 0;

	//reg_SQ_CAL_INDV_EXT_EN_LANE = 1;
	reg_SQ_CAL_RESULT_EXT_LANE_5_0 = 0x20;
	reg_RX_SQ_OFFSET_LANE_4_0 = 0x10;
}
...
void rx_sq_ofst_cal(void) BANKING_CTRL {
	...
	reg_RX_SQ_OFFSETCAL_SEL_LANE_1_0 = 1;
	reg_RX_SQ_TOP_START_LANE = 1;
	/*timeout = 0;
	timeout_start((uint16_t)TIMEOUT_SQ_OFST_CAL);*/
	delay(T_p5us);
	//while((reg_MCU_DEBUGA_LANE_7_0=0x8A)&&!reg_RX_SQ_TOP_DONE_LANE);
	wait_for(reg_RX_SQ_TOP_DONE_LANE,0);
	if(reg_SQ_CAL_TIMEOUT_RD_LANE)
		lnx_SQ_OFST_CAL_PASS_LANE = 0;
	reg_RX_SQ_TOP_START_LANE = 0;

	result = reg_SQ_CAL_RESULT_RD_LANE_5_0;
	ofst1 = rx_sq_ofst_cal_convert_result(result);

	reg_RX_SQ_OFFSETCAL_SEL_LANE_1_0 = 2;
	reg_RX_SQ_TOP_START_LANE = 1;
	/*timeout = 0;
	timeout_start((uint16_t)TIMEOUT_SQ_OFST_CAL);
	while((reg_MCU_DEBUGA_LANE_7_0=0x8B)&&!reg_RX_SQ_TOP_DONE_LANE && !timeout);
	if(timeout)*/
	delay(T_p5us);
	//while((reg_MCU_DEBUGA_LANE_7_0=0x8C)&&!reg_RX_SQ_TOP_DONE_LANE);
	wait_for(reg_RX_SQ_TOP_DONE_LANE,0);
	if(reg_SQ_CAL_TIMEOUT_RD_LANE)
		lnx_SQ_OFST_CAL_PASS_LANE = 0;
	reg_RX_SQ_TOP_START_LANE = 0;
	...
	result = reg_SQ_CAL_RESULT_RD_LANE_5_0;
	ofst2 = rx_sq_ofst_cal_convert_result(result);

	reg_RX_SQ_OFFSET_LANE_4_0 = rx_sq_ofset_cal_get_abs_difference(ofst1, ofst2);
...
void rx_sq_thresh_init(void) BANKING_CTRL {
	reg_RX_SQ_CAL_EN_LANE = 1;
	reg_RX_SQ_OFFSET_CAL_EN_LANE = 0;
	reg_RX_SQ_THRESH_CAL_EN_LANE = 1;
	reg_RX_SQ_OFFSETCAL_SEL_LANE_1_0 = 3;  // pw:based on new pseudo code
	// TODO - from 112G_A code; The below line caused the DONE bit not assert in simulation
	//reg_SQ_CAL_INDV_EXT_EN_LANE = 0;
}
...
void rx_sq_thresh_cal(void) BANKING_CTRL {
	...
	case 0xa:case 0xb:case 0x12:case 0x17:
		reg_RX_SQ_REFTHR_LANE_4_0 = 0x05;break;
	default:reg_RX_SQ_REFTHR_LANE_4_0 = 0x07;break;
	}*/
	reg_RX_SQ_REFTHR_LANE_4_0 = 0x07;
	reg_SQ_CAL_RESULT_EXT_LANE_5_0 = 0x20;
	reg_RX_SQ_TOP_START_LANE = 1;
	/*timeout = 0;
	timeout_start((uint16_t)TIMEOUT_SQ_THRESH_CAL);
	while((reg_MCU_DEBUGA_LANE_7_0=0x8D)&&!reg_RX_SQ_TOP_DONE_LANE && !timeout);
	if(timeout)*/
	delay(T_p5us);
	//while((reg_MCU_DEBUGA_LANE_7_0=0x8E)&&!reg_RX_SQ_TOP_DONE_LANE);
	wait_for(reg_RX_SQ_TOP_DONE_LANE,0);
	...
	//reg_RX_SQ_REFTHR_LANE_4_0 = 0x17;
	reg_RX_SQ_REFTHR_LANE_4_0 |= 0x10;
	reg_RX_SQ_TOP_START_LANE = 1;
	/*timeout = 0;
	timeout_start((uint16_t)TIMEOUT_SQ_THRESH_CAL);
	while((reg_MCU_DEBUGA_LANE_7_0=0x8F)&&!reg_RX_SQ_TOP_DONE_LANE && !timeout);
	if(timeout)*/
	delay(T_p5us);
	//while((reg_MCU_DEBUGA_LANE_7_0=0x90)&&!reg_RX_SQ_TOP_DONE_LANE);
	wait_for(reg_RX_SQ_TOP_DONE_LANE,0);
	if(reg_SQ_CAL_TIMEOUT_RD_LANE)
		lnx_SQ_THRESH_CAL_PASS_LANE = 0;
	reg_RX_SQ_TOP_START_LANE = 0;
	...
	lnx_SQ_THRESH_CAL_DONE_LANE = 1;
	reg_RX_SQ_THRESH_CAL_EN_LANE = 0; // pw: based on new pseodu code
	reg_RX_SQ_CAL_EN_LANE = 0;
}
