...
Serdes mode sft_rst has new scheduling*/
	...
	if (reg_INT_PU_TX_CHG_RISING_ISR_LANE || reg_INT_PU_TX_CHG_FALLING_ISR_LANE) {    //Separate RISE/FALL polling to avoid stuck
		...
		//pu_tx_down function could be escalated to higher soft reset level to complete full OFF/ON cycle.
		//Use reg_pin_pll_ready as an indicator to determine if ON sequence is completed or not 
	...
	if (reg_INT_PU_RX_CHG_RISING_ISR_LANE || reg_INT_PU_RX_CHG_FALLING_ISR_LANE) {    //Separate RISE/FALL polling to avoid stuck
		...
		//pu_rx_down function could be escalated to higher soft reset level to complete full OFF/ON cycle.
		//Use reg_pin_pll_ready as an indicator to determine if ON sequence is completed or not 
			
			if(!reg_PIN_PLL_READY_RX_LANE){
				pu_rx_up();
			}
...
void reset_core_tx_down(void) BANKING_CTRL {
	...
	if(reg_RX_SFT_RST_CHG_FALLING_ISR_LANE == 1) {
		...
		wait_for((reg_PIN_PU_RX_RD_LANE && !reg_PIN_RESET_CORE_RX_RD_LANE) || (reg_PIN_PU_TX_RD_LANE && !reg_PIN_RESET_CORE_TX_RD_LANE), 1);   //TRX SFT_RST wakeup needs to wait at least one PU and RESET_CORE level to be asserted
		
		reg_MCU_DEBUGD_LANE_7_0 = timeout;
		
		//timeout then exit the power up 
		if(timeout) {
			return;
		}
	...
	//Only TX release
		...
		wait_for(reg_PIN_PU_TX_RD_LANE && !reg_PIN_RESET_CORE_TX_RD_LANE, 100); //SFT_RST wakeup needs to wait PU & RESET_CORE signals	
...
void reset_core_rx_down(void) BANKING_CTRL {
	...
	//Only RX release
		...
		wait_for(reg_PIN_PU_RX_RD_LANE && !reg_PIN_RESET_CORE_RX_RD_LANE, 100); //SFT_RST wakeup needs to wait PU & RESET_CORE signals	
...
//Mini scheduler TX only portion. TX subset of the general machine above


void Mini_Serdes_Power_Speed_Management(void) BANKING_CTRL {   //PM/SPD/RST function for Mini scheduler. TX activities only during RX training
	...
	if (reg_TX_SFT_RST_CHG_RISING_ISR_LANE || reg_TX_SFT_RST_CHG_FALLING_ISR_LANE) {    //Separate RISE/FALL polling to avoid stuck
		...
		if(reg_TX_SFT_RST_CHG_RISING_ISR_LANE && reg_TX_SFT_RST_CHG_FALLING_ISR_LANE) {	
			...
			reset_core_tx_only_up();

			reset_core_tx_only_down();

			reg_MCU_DEBUG0_LANE_7_0 = 0x24;
		}
		//Case 2 Only SFT_RST rising
		else if(reg_TX_SFT_RST_CHG_RISING_ISR_LANE) {	
			delay(Tus);
