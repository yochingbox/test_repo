...
void delay(int16_t t) {
	...
	else if( ( mcuid != MCU_CMN && PHY_STATUS == ST_POSEQ && BYPASS_POWERON_DELAY ) ||
		   ( BYPASS_DELAY>=4 )) {
			//bypass delay
			return;
	}
...
void tsen_on(void) BANKING_CTRL {
	...
	//T_128us overflow the counter, replace with two 64us delays
	//https://ceproject.marvell.com/index.html#/task/2242
	delay(T_64us);
	delay(T_64us);
	//delay(T_128us); //T_100us; Zubir requested to increase delay
...
/*
 * Module is_pause() waits for the PAUSE_LANE signal to toggle before exit.
 */
void is_pause(void) BANKING_CTRL {
	if (lnx_TRX_TRAIN_DEBUG_EN_LANE) {
