...
int8_t set_TXFFE_tap(uint8_t tap, int8_t dir)
{
    ...
    if(tap == 1) // co
    {
		if(tag_TX_TRAIN_P2P_HOLD == 0)
        {
            if(dir > 0) // increase c0
				req_local_ctrl(CE_NORMAL, ST_HOLD, ST_INC, ST_HOLD, ST_HOLD);	
            else    
				req_local_ctrl(CE_NORMAL, ST_HOLD, ST_DEC, ST_HOLD, ST_HOLD);	
    ...
    else if(tap == 0) //c1
    {
        ...
        {
            // increase c1
			req_local_ctrl(CE_NORMAL, ST_INC, ST_HOLD, ST_HOLD, ST_HOLD);
            ...
            if((update_flag == 1) && (tag_TX_TRAIN_P2P_HOLD == 1))
            {
                ...
                {
                    // revert back c1
			        req_local_ctrl(CE_NORMAL, ST_DEC, ST_HOLD, ST_HOLD, ST_HOLD);
                    update_flag = (drv_TXTRAIN_STATUS_VALID == 1) & (drv_TXTRAIN_STATUS_C1 == 1);
                    if(update_flag == 0)
    ...
    else if(tap == 2) //cm1
    {
        ...
        {
            // increase cm1
			req_local_ctrl(CE_NORMAL, ST_HOLD, ST_HOLD, ST_INC, ST_HOLD);	
            ...
            if((update_flag == 1) && (tag_TX_TRAIN_P2P_HOLD == 1))
            {
                ...
                {
                    // revert back to cm1
			        req_local_ctrl(CE_NORMAL, ST_HOLD, ST_HOLD, ST_DEC, ST_HOLD);
                    update_flag = (drv_TXTRAIN_STATUS_VALID == 1) & (drv_TXTRAIN_STATUS_CN1 == 1);
                    if(update_flag == 0)
                        return -1; // critical error
                    else return 0;
    ...
    else if(tap == 3) //cm2
    {
        ...
        {
            // decrease cm2
			req_local_ctrl(CE_NORMAL, ST_HOLD, ST_HOLD, ST_HOLD, ST_DEC);	
            ...
            if((update_flag == 1) && (tag_TX_TRAIN_P2P_HOLD == 1))
            {
                ...
                {
                    // revert back to cm2
			        req_local_ctrl(CE_NORMAL, ST_HOLD, ST_HOLD, ST_HOLD, ST_INC);	
                    update_flag = (drv_TXTRAIN_STATUS_VALID == 1) & (drv_TXTRAIN_STATUS_CN2 == 1);
                    if(update_flag == 0)
                        return -1; // critical error
                    else return 0;
...
//		   		fail_flag = true;
			...
			conv_LMS_FFE_value = reg_RX_DTL_LMS_FFE_PST1_COE_LANE_5_0;
...
//		    	fail_flag = true;
			...
			conv_LMS_FFE_value = reg_RX_DTL_LMS_FFE_PRE1_COE_LANE_5_0;
...
//		    	fail_flag = true;
			...
			conv_LMS_FFE_value = reg_RX_DTL_LMS_FFE_PRE2_COE_LANE_4_0;
