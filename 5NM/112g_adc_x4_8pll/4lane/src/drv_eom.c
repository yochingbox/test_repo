...
void drv_eom_set_recon_flt(void) BANKING_CTRL {
    ...
    for(i=0;i<=eom_max_idx;i++){
        acc = 0;
        for(j=0;j<=i;j++){
            mult = (int32_t)delta_h[j] * ffe[i-j];  // S16.12 * S8.6 = S23.18
            acc += mult;                            // S28.18
        }

        //acc_out = (long long)acc * gain;          // S28.18 * U11.6 = S39.24
        //32-bit multiplication isn't supported
        //split 32x16 to two 16x16 multiplications
        //AABBCCDD*MM = (AABB0000)*MM + CCDD*MM = (AABB*MM)<<16 + CCDD*MM
        acc_sign = (acc >> 31) & 0x1;
        if(acc_sign)
            acc = -acc;
        ...
        //(AABBCCDD*MM)>>16 = AABB*MM + (CCDD*MM)>>16, S39.24 -> S23.8
        acc_li_mult = acc_li_mult >> 16;
        acc_out = acc_hi_mult + acc_li_mult;    // S23.8

        acc_out = acc_out / 3;                  // S22.8
        acc_out >>= 1;                          // S21.7
...
int16_t drv_eom_chest(void) BANKING_CTRL {
    ...
    chest_val = chest_val >> (4 + reg_CHEST_LEN_LANE_3_0); //16 MSB = S16.12
