...
#ifdef DRV_PWR_V0    //7nm V0 initial version
  reg_RX_GB_FIFO_RD_CLK_EN_LANE = 0;
  reg_RX_GB_FIFO_WR_CLK_EN_LANE = 0;
#else       //5nm V1,2 and subsequent ones
...
//OFF
    ...
    reg_RX_GB_FIFO_WR_CLK_EN_LANE = 0;
    reg_RX_GB_FIFO_RD_CLK_EN_LANE = 0;
  #else   //5nm V1, V2 and subsequent ones
    reg_RX_GB_ADCCLK_EN_LANE = 0;
    reg_RX_GB_DATA_CLK_EN_LANE = 0;
  #endif
  reg_RX_GB_RST_LANE = 1;
...
//Save and restore for command interface
    uint8_t phy_gen_rx; 
    bool phy_gen_rx_fm_reg, reset_core_rx, reset_core_rx_fm_reg;
    bool pu_rx_fm_reg;
    bool pu_rx;
    phy_gen_rx = reg_PHY_GEN_RX_LANE_5_0;
    phy_gen_rx_fm_reg = reg_PHY_GEN_RX_FM_REG_LANE;
    reset_core_rx = reg_RESET_CORE_RX_LANE;
    reset_core_rx_fm_reg = reg_RESET_CORE_RX_FM_REG_LANE;
    pu_rx_fm_reg = reg_PU_RX_FM_REG_LANE;
    pu_rx = reg_PU_RX_LANE;

	rx_2x_clk_sel = (reg_RX_CLKTOP_RSV_LANE_9_2 & 0x02) >> 1;

        //reg_SFT_RST_ALL_RX_LANE = 1;
	//Exclude rx_if bank from reset list
	reg_SFT_RST_RX_LANE = 1;
	reg_SFT_RST_RX_EQ_LANE = 1;
	reg_SFT_RST_RX_EQ_MON_LANE = 1;
	reg_SFT_RST_RX_SKEW_CAL_LANE = 1;
	reg_SFT_RST_RX_ADC_IF_LANE = 1;
	reg_SFT_RST_RX_DP_LANE = 1;
	reg_SFT_RST_RX_DTL_LANE = 1;
	reg_SFT_RST_TX_TRAIN_IF_LANE = 1;
	//
	...
	//Exclude rx_if bank from reset list
	reg_SFT_RST_RX_LANE = 0;
	reg_SFT_RST_RX_EQ_LANE = 0;
	reg_SFT_RST_RX_EQ_MON_LANE = 0;
	reg_SFT_RST_RX_SKEW_CAL_LANE = 0;
	reg_SFT_RST_RX_ADC_IF_LANE = 0;
	reg_SFT_RST_RX_DP_LANE = 0;
	reg_SFT_RST_RX_DTL_LANE = 0;
	reg_SFT_RST_TX_TRAIN_IF_LANE = 0;
	//
...
//Save and restore for command interface
      reg_PHY_GEN_RX_LANE_5_0 = phy_gen_rx;
      reg_PHY_GEN_RX_FM_REG_LANE = phy_gen_rx_fm_reg;
      reg_RESET_CORE_RX_LANE = reset_core_rx;
      reg_RESET_CORE_RX_FM_REG_LANE = reset_core_rx_fm_reg;

      reg_PU_RX_FM_REG_LANE = pu_rx_fm_reg;
      reg_PU_RX_LANE = pu_rx;
...
//Save and restore for command interface
    uint8_t phy_gen_tx; 
    bool phy_gen_tx_fm_reg, reset_core_tx, reset_core_tx_fm_reg;
    bool pu_tx_fm_reg;
    bool pu_tx;
    phy_gen_tx = reg_PHY_GEN_TX_LANE_5_0;
    phy_gen_tx_fm_reg = reg_PHY_GEN_TX_FM_REG_LANE;
    reset_core_tx = reg_RESET_CORE_TX_LANE;
    reset_core_tx_fm_reg = reg_RESET_CORE_TX_FM_REG_LANE; 
    pu_tx_fm_reg = reg_PU_TX_FM_REG_LANE;
    pu_tx = reg_PU_TX_LANE;
...
//Save and restore for command interface
      reg_PHY_GEN_TX_LANE_5_0 = phy_gen_tx;
      reg_PHY_GEN_TX_FM_REG_LANE = phy_gen_tx_fm_reg;
      reg_RESET_CORE_TX_LANE = reset_core_tx;
      reg_RESET_CORE_TX_FM_REG_LANE = reset_core_tx_fm_reg; 

      reg_PU_TX_FM_REG_LANE = pu_tx_fm_reg;
      reg_PU_TX_LANE = pu_tx;
...
//Save and restore for command interface
    ...
    bool phy_gen_tx_fm_reg, reset_core_tx, reset_core_tx_fm_reg;
    bool phy_gen_rx_fm_reg, reset_core_rx, reset_core_rx_fm_reg;
    bool pu_tx_fm_reg, pu_rx_fm_reg, pu_pll_fm_reg;
    bool pu_tx, pu_rx, pu_pll;
    phy_gen_tx = reg_PHY_GEN_TX_LANE_5_0;
    phy_gen_rx = reg_PHY_GEN_RX_LANE_5_0;
    phy_gen_tx_fm_reg = reg_PHY_GEN_TX_FM_REG_LANE;
    phy_gen_rx_fm_reg = reg_PHY_GEN_RX_FM_REG_LANE;
    ...
    pu_tx_fm_reg = reg_PU_TX_FM_REG_LANE;
    pu_rx_fm_reg = reg_PU_RX_FM_REG_LANE;
    pu_pll_fm_reg = reg_PU_PLL_FM_REG_LANE;
    pu_tx = reg_PU_TX_LANE;
    pu_rx = reg_PU_RX_LANE;
    pu_pll = reg_PU_PLL_LANE;
...
//Save and restore for command interface
      ...
      reg_PU_TX_FM_REG_LANE = pu_tx_fm_reg;
      reg_PU_RX_FM_REG_LANE = pu_rx_fm_reg;
      reg_PU_PLL_FM_REG_LANE = pu_pll_fm_reg;
      reg_PU_TX_LANE = pu_tx;
      reg_PU_RX_LANE = pu_rx;
      reg_PU_PLL_LANE = pu_pll;
...
//Drivers for lab isolation mode SerDes TRX PM/SPD/RST test
...
void drv_iso_trx_phy_gen_chg (void) BANKING_CTRL {
	reg_PHY_GEN_TX_LANE_5_0 = lnx_CLI_ARGS_LANE_15_0_b0;
	reg_PHY_GEN_RX_LANE_5_0 = lnx_CLI_ARGS_LANE_15_0_b0;
}
