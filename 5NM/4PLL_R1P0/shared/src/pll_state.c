...
static uint8_t num_slaves_to_process = 0;
...
static void pll_state_lane_master_init(const uint8_t slave_lane_bit_map, const uint8_t mcuid_local) {

    uint8_t array_index = 0;
    uint8_t i = 0;

    //wait for slaves to initialize so masters can know if they need to manage pll for slave

    reg_MCU_DEBUG0_LANE_7_0 = 0x18;
    reg_MCU_DEBUG1_LANE_7_0 = slave_lane_bit_map;
    for(i = 0; i < NUM_MCU_LANES; ++i) {
        if((1 << i) & slave_lane_bit_map) {
            reg_MCU_DEBUG0_LANE_7_0 = 0x19;
            reg_MCU_DEBUG1_LANE_7_0 = 1 << i;
            while(pll_s_get_current_state(i) == PLL_SLAVE_LANE_STATE__Invalid_State) {
                ;
            }
        }
    }
    ...
    if(pll_is_this_mcu_a_master()) {
        ...
        num_slaves_to_process = 0;
    }
}

static void pll_state_lane_slave_init(const uint8_t mcuid_local) {

    uint8_t array_index = mcuid_local;

    if(pll_is_this_mcu_a_slave()) {
        pll_s__lane_request_semaphore[array_index] = 0;
        pll_s_set_state_xdata(array_index, PLL_SLAVE_LANE_STATE__Initialized_State);
...
void pll_state_lane_init() BANKING_CTRL {
    ...
    if((SPD_CFG == 2) || (SPD_CFG == 3)) {
        switch(mcuid) {
            case MCU_LANE0:
                #ifdef CONFIG_5NM_56G_X1_2PLL
                pll_s_set_state_xdata(MCU_LANE1, PLL_SLAVE_LANE_STATE__WillNeverNeedMaster);
                #endif // #ifdef CONFIG_5NM_56G_X1_2PLL
                pll_state_lane_master_init(1 << MCU_LANE1, mcuid);
                break;

            case MCU_LANE1:
                pll_state_lane_slave_init(mcuid);
                break;

            case MCU_LANE2:
                pll_state_lane_master_init(1 << MCU_LANE3, mcuid);
                break;
        ...
        switch(mcuid) {
            ...
            case MCU_LANE1:
                pll_state_lane_master_init(1 << MCU_LANE0, mcuid);
                break;
        ...
        switch(mcuid) {
            ...
            case MCU_LANE1:
                pll_state_lane_master_init((1 << MCU_LANE0) |
                                           (1 << MCU_LANE2) |
                                           (1 << MCU_LANE3), mcuid);
                break;
...
pll_a_handle_event(const enum PLL_MASTER_LANE_INDICES pll_index,
    ...
    uint8_t lane_responsible_bit_map = 0;
        ...
        case PLL_ACCESS_STATE__Free:
            switch (command) {
                case PLL_ACCESS_COMMAND__RequestPLLAccess:
                    ...
                    lane_responsible_bit_map = pll_get_lane_bit_map(channel_responsible_bit_map);
                    ...
                    reg_MCU_DEBUG2_LANE_7_0 = lane_responsible_bit_map;
                    reg_MCU_DEBUG3_LANE_7_0 = lane_command_pending_bit_map;
                    reg_MCU_DEBUG4_LANE_7_0 = cmx_PLL_SLAVE_CURRENT_STATE_S0_7_0;
                    reg_MCU_DEBUG5_LANE_7_0 = cmx_PLL_SLAVE_CURRENT_STATE_S1_7_0;
                    reg_MCU_DEBUG6_LANE_7_0 = cmx_PLL_SLAVE_CURRENT_STATE_S2_7_0;
                    reg_MCU_DEBUG7_LANE_7_0 = cmx_PLL_SLAVE_CURRENT_STATE_S3_7_0;
                    ...
                    if(PLL_MASTER_LANE_STATE__RequestingPLLAccess == 
                        ...
                        if(pll_s_any_lane_in_state(lane_responsible_bit_map,
                                                   PLL_SLAVE_LANE_STATE__InUse)) {
                            reg_MCU_DEBUG0_LANE_7_0 = 0x94;
                            ret = PLL_REQUEST_RETURN_CODE__MasterSkipPLLAccess;
                        }
                        ...
                        if(pll_any_lane_pll_in_use(lane_command_pending_bit_map,
                            ...
                            for(loop_index = 0; loop_index < MCU_NUM_CHANNELS; loop_index++) {
                                (void) mcu_check_and_execute_return(channel_command_pending_bit_map,
                                                                    PLL_REQUEST_RETURN_CODE__SlaveSkipPLLAccess,
                                                                    loop_index);
                            }
                            ret = PLL_REQUEST_RETURN_CODE__SlaveSkipPLLAccess;
                        }
                        else {
...
static void pll_s_set_state_based_on_seq(const enum PLL_SEQUENCE__SEQUENCES seq,
    ...
    switch(seq) {
        ...
        case PLL_SEQUENCE__Trx_Soft_Reset:
        ...
        case PLL_SEQUENCE__Trx_Soft_Reset_Release:
...
pll_s_handle_event(const uint8_t lane_index,
    ...
    switch (current_state) {
        ...
        case PLL_SLAVE_LANE_STATE__Initialized_State:
            switch (command) {
                case PLL_SLAVE_LANE_COMMAND__RequestPLLAccess:
                    ...
                    switch(mcu_command_ret) {
                        ...
                        case PLL_REQUEST_RETURN_CODE__SlaveSkipPLLAccess:
        ...
        case PLL_SLAVE_LANE_STATE__InUseSyncPLLAccess:
            switch (command) {
                ...
                case PLL_SLAVE_LANE_COMMAND__PllAccessCommand:
                    ...
                    //if pcie, and if not in power up (which has nested frees), 
                    //and if command is execute and free, free pll without waiting for master as an optimization
                    #ifdef _PCIE_BUILD
                    if(!is_first_time_power_up()) {
                        if((data1 == MCU_SPD_CHG_PLL_CAL_TX_AND_FREE) ||
                           (data1 == MCU_SPD_CHG_PLL_CAL_RX_AND_FREE) ||
                           (data1 == MCU_PLL_POWER_ON_TX_AND_FREE)    ||
                           (data1 == MCU_PLL_POWER_ON_AND_FREE)       ||
                           (data1 == MCU_PLL_POWER_OFF_AND_FREE)) {
...
static void pll_m_request_pll_access(const enum PLL_MASTER_LANE_INDICES master_index) {
    ...
    switch(ret) {
        ...
        case PLL_REQUEST_RETURN_CODE__SlaveSkipPLLAccess:
            ...
            //But the slave MUST send a free command to the master when done
            pll_m_set_state(master_index, PLL_MASTER_LANE_STATE__InUseSlaveSkipPLLAccess);
            break;
        case PLL_REQUEST_RETURN_CODE__MasterSkipPLLAccess:
            //Some lane is using the PLL, so the master requesting access will skip it's own PLL accesses.
            pll_m_set_state(master_index, PLL_MASTER_LANE_STATE__InUseMasterSkipPLLAccess);
            break;
    }
}
...
uint8_t pll_m_get_mcuid_from_master_index(const enum PLL_MASTER_LANE_INDICES master_index) {
    ...
    reg_MCU_DEBUG0_LANE_7_0 = 0x92;
    reg_MCU_DEBUG1_LANE_7_0 = mcuid_local;
    return mcuid_local;
}
...
pll_m_handle_event(const enum PLL_MASTER_LANE_INDICES master_index,
    ...
    uint8_t command_found_on_channel = 0;
    ...
    switch (current_state) {
        ...
        case PLL_MASTER_LANE_STATE__InUseMasterSkipPLLAccess:
            switch (command) {
                case PLL_MASTER_LANE_COMMAND__PllAccessCommand:
                    if(current_state == PLL_MASTER_LANE_STATE__InUsePerformPllAccess) {
                        mcu_execute(data1, 0, data2, data3);
                    }
                    //else {
                    //    ;   //don't execute pll commands in skip state since some other lane is using the pll
                    //}
                    break;

                case PLL_MASTER_LANE_COMMAND__CheckIfPLLAccessPending:
                    //Don't check these as we are in the middle of a master sequence and the PLL Access is locked
                    break;
        ...
        case PLL_MASTER_LANE_STATE__InUseSlaveSkipPLLAccess:
            switch (command) {
                ...
                case PLL_MASTER_LANE_COMMAND__CheckIfPLLAccessPending:
                    channel_bit_map = pll_get_mcu_command_channel_to_receive(pll_m_get_mcuid_from_master_index(master_index));
                    mcu_command = mcu_check_and_execute(channel_bit_map, &command_found_on_channel);
                    if((mcu_command == MCU_FREE_PLL)                    ||
                       (mcu_command == MCU_SPD_CHG_PLL_CAL_TX_AND_FREE) ||
                       (mcu_command == MCU_SPD_CHG_PLL_CAL_RX_AND_FREE) ||
                       (mcu_command == MCU_PLL_POWER_OFF_AND_FREE)      ||
                       (mcu_command == MCU_PLL_POWER_ON_TX_AND_FREE)    ||
                       (mcu_command == MCU_PLL_POWER_ON_AND_FREE)) {
                        (void) pll_a_handle_event(master_index, PLL_ACCESS_COMMAND__Free);
                        pll_m_set_state_based_on_seq(pll_m__lane_seq__current_sequence[master_index], master_index);
                    }
                    break;
        ...
        case PLL_MASTER_LANE_STATE__InUsePerformPllAccessWithSync:
            switch (command) {
                ...
                case PLL_MASTER_LANE_COMMAND__PllAccessCommand:
                    ...
                    //if pcie, and if not in power up (which has nested frees), 
                    //and if command is execute and free, free pll without waiting for slave as an optimization
                    #ifdef _PCIE_BUILD
                    if(!is_first_time_power_up()) {
                        if((data1 == MCU_SPD_CHG_PLL_CAL_TX_AND_FREE) ||
                           (data1 == MCU_SPD_CHG_PLL_CAL_RX_AND_FREE) ||
                ...
                case PLL_MASTER_LANE_COMMAND__Free:
                    ...
                    if(pll_m__lane_request_semaphore[master_index] == 0) {
                        channel_bit_map = pll_get_mcu_command_channel_to_receive(pll_m_get_mcuid_from_master_index(master_index));
                        pll_wait_for_valid_command_execute(data1, channel_bit_map);
                        (void) pll_a_handle_event(master_index, PLL_ACCESS_COMMAND__Free);
                        pll_m_set_state_based_on_seq(pll_m__lane_seq__current_sequence[master_index], master_index);
                    }
                    break;
