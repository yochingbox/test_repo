...
void mcu_execute(const uint8_t command,
    ...
    switch(command) {
        ...
        case MCU_LOADSPEEDTBL_RX_PLL:
            switch(pll_sel) {
            case BOTH_ON:
            case RS_ON:
                loadspeedtbl_lc_pll(lnx_RX_PLL_RATE_LANE_7_0, data2, RS_PLL);
            break;	
            case TS_ON:
            case SWAP_BOTH_ON:
                loadspeedtbl_lc_pll(lnx_RX_PLL_RATE_LANE_7_0, data2, TS_PLL);
            break;
            }
            break;

        case MCU_LOADSPEEDTBL_TX_PLL:
            switch(pll_sel) {
            case BOTH_ON:
            case TS_ON:
                loadspeedtbl_lc_pll(lnx_TX_PLL_RATE_LANE_7_0, data2, TS_PLL);
            break;	
            case RS_ON:
            case SWAP_BOTH_ON:
                loadspeedtbl_lc_pll(lnx_TX_PLL_RATE_LANE_7_0, data2, RS_PLL);
            break;
            }
            break;
        ...
        case MCU_SPD_CHG_PLL_CAL_TX_AND_FREE:
            tx_spd_chg_pll_fast_cal_sel();
            break;

        case MCU_SPD_CHG_PLL_CAL_RX:
        case MCU_SPD_CHG_PLL_CAL_RX_AND_FREE:
            rx_spd_chg_pll_fast_cal_sel();
            break;
        ...
        case MCU_PLL_POWER_ON_TX_AND_FREE:
            drv_pll_on();
            turn_on_tx_cont_cal_ts_pll();
            break;

        case MCU_PLL_POWER_ON_AND_FREE:
            drv_pll_on();
            turn_on_tx_cont_cal_ts_pll();
            turn_on_rx_cont_cal_rs_pll();
            break;
        ...
        case MCU_PLL_POWER_OFF_AND_FREE:
            turn_off_tx_cont_cal_ts_pll();
            turn_off_rx_cont_cal_rs_pll();
            drv_pll_off();
            break;
        ...
        case MCU_WAIT_FOR_PLL_LOCK:
            wait_for_pll_lock();
            break;

        case MCU_WAIT_FOR_RX_PLL_LOCK:
            wait_for_rx_pll_lock();
            break;

        case MCU_WAIT_FOR_TX_PLL_LOCK:
            wait_for_tx_pll_lock();
            break;

        case MCU_CHECK_FOR_RX_PLL_LOCK:
            check_rx_pll_lock();
            break;

        case MCU_CHECK_FOR_TX_PLL_LOCK:
            check_tx_pll_lock();
            break;

        case MCU_CHECK_FOR_RX_TX_BOTH_PLL_LOCK:
            check_rx_pll_lock();
            check_tx_pll_lock();
            break;
        ...
        for(channel = 0; channel < MCU_NUM_CHANNELS; channel++) {
            if(channel_responsibility_bit_map & (1 << channel)) {
                if(mcu_check_command_pending(channel)) {
                    *command_found_on_channel = channel;
...
uint8_t mcu_check_and_clear(const uint8_t channel_responsibility_bit_map,
                            uint8_t *command_found_on_channel) {
