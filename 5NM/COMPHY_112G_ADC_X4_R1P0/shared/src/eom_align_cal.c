...
void eom_align_cal(void) BANKING_CTRL{
        ...
        if(reg_PWRON_SEQ == 0) goto eom_cal;
        ...
        do {
         ...
         } while((reg_MCU_DEBUGA_LANE_7_0=0x43)&&loop_cnt < comp_max);
...
eom_cal:         
        ...
        do {
        	...
        	reg_EOM_DPHERCK_LANE = 1;
        	reg_EOM_DPHERCK_LANE = 0;
        } while((reg_MCU_DEBUGA_LANE_7_0=0x44)&& loop_cnt < coarse_max );
        #ifdef _DBG_EOM
        ...
        while((reg_MCU_DEBUGA_LANE_7_0=0x45)&&loop_cnt < align_max){
        	delay(Tus);
        	eom_align_comp_out = eom_comp_out_rd();
        	if(dir != eom_align_comp_out)
        	{
    ...
    err_skip:
        ...
        lnx_cal_eom_dpher[rate] = EOM_DPHER; 
