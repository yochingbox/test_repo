	...
	if (rs_tempc_step_state == 0x0) {
		...
		if((rs_tempc_dir == 0x2a) && (rs_tempc_mux_sel_2c<RS_TEMPC_MUX_SEL_MAX)){ //freq inc
			if( cmx_TEMPC_DAC_MODE_1_0!=1 || rs_pllcal_first_time==1 || RS_TEMPC_DAC_SEL == RS_TEMPC_DAC_MAX ) {
				...
				RS_TEMPC_DAC_SEL = 0x7f;                 
				rs_tempc_step_state = 0x22; 
			     }
			     else 
...
void tempc_cal_cont_fw(void) BANKING_CTRL {
	uint8_t udelay;
	if(!cmx_PLL_TEMP_CAL_CONT_EN || FORCE_CONT_CAL_SKIP) return;
	if(!cmx_CAL_DONE) return;
	...
	if(reg_ANA_PLL_TS_CLK_READY_LANE && pll_sel!=RS_ON) {
		...
		if( timeout2_check(TSTEMPC)) {
			...
			if(lnx_TS_TEMPC_DBG_STEP_MODE_LANE_7_0==1 && udelay>5) {
				lnx_TS_TEMPC_DBG_STEP_LANE_7_0 = 1;
				while(lnx_TS_TEMPC_DBG_STEP_LANE_7_0==1);		
			}		
		}
	}
	...
	if(reg_ANA_PLL_RS_CLK_READY_LANE && pll_sel!=TS_ON ) {
		...
		if( timeout2_check(RSTEMPC)) {
			...
			if(lnx_RS_TEMPC_DBG_STEP_MODE_LANE_7_0==1 && udelay>5) {
				lnx_RS_TEMPC_DBG_STEP_LANE_7_0 = 1;
				while(lnx_RS_TEMPC_DBG_STEP_LANE_7_0==1);		
			}		
		}
	}
