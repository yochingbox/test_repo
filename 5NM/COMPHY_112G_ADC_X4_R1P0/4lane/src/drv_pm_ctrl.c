...
//  short_delay_4g();
  ...
  if (!reg_TX_SFT_RST_CHG_RISING_ISR_LANE && !reg_INT_PHY_GEN_TX_CHG_ISR_LANE && !reg_INT_PU_TX_CHG_FALLING_ISR_LANE) {
    reg_PIN_PLL_READY_TX_LANE = 1;
  }

  if (!reg_RX_SFT_RST_CHG_RISING_ISR_LANE && !reg_INT_PHY_GEN_RX_CHG_ISR_LANE && !reg_INT_PU_RX_CHG_FALLING_ISR_LANE) {
