...
while (<FP_MEM>)
{
	...
	if ( $addr > 0x3FFF ) { $fill_up = 0; } # no fill up for external RAM or ROM
        #$addr &= 0xFFFF;	# Added masking of lower 16-bit to allow processing external RAM file
	$addr &= 0x3FFF;	# Added masking of lower 16-bit to allow processing external RAM file
    for ($i=0; $i<$bytes; $i++)
    {
...
print FP_MEM "\@003FFC $data4 $data3 $data2 $data1\n";
