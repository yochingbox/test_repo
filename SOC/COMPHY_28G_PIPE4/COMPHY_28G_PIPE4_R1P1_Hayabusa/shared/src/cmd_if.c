...
uint8_t cmd_dfe_res(uint8_t gen, uint8_t cmd_code) CMD_BANK {
	...
	if(gen==0x80) {//current
		...
		if(cmd_code==CMD_RX_DFE_RES_GET) {
			...
			//if(lnx_PHY_REMOTE_CTRL_VALUE_LANE_31_0_b2 & 0x02) reg_EN_DFE_F8TO15_LANE = 1;
			//else reg_EN_DFE_F8TO15_LANE = 0;
		}	
	}	
	else {
		if(cmd_code==CMD_RX_DFE_RES_GET) {
			...
			//if(lnx_PHY_REMOTE_CTRL_VALUE_LANE_31_0_b2 & 0x02) {
			//	TRAIN_CONTROL_13.BT.B0 |= (0x01<<gen);
			//}	
			//else {
			//	TRAIN_CONTROL_13.BT.B0 &= ~(0x01<<gen);				
			//}
