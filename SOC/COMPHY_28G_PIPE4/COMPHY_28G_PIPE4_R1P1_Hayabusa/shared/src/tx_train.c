	...
	while (1)
	{
		if( phy_mode == PCIE ) 
		{
			...
			if( reg_EYE_OPEN_EN_RD_LANE ) {
				...
				dis_block_phase_train = 1; //not to block phase train
    ...
    bool* IS_MIN, bool* IS_MAX, void(* update_g)(int8_t))
{
	...
	if(X) {
		...
		lnx_TX_G1_MAXF0T_G1_INDEX_RESULT_LANE_7_0 =  opt_g1_index;
	}	
	else {
		update_g(opt_gn1_index - *IDX);
		lnx_TX_GN1_MAXF0T_GN1_INDEX_RESULT_LANE_7_0 =  opt_gn1_index;
	}	
