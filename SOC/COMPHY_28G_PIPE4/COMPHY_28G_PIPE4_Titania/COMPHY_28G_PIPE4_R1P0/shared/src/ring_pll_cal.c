...
#define min_step_number                 cmx_RINGPLL_DAC_MIN_STEP_NUM_7_0//1
#define Ftarget_code                    2
#define Fdiff_max_value			0x7F
#define dac_fine_max_value		0x7FF
#define fdiff_0					cmx_RINGPLL_FDIFF_7_0 //20
...
adjust:	
    ...
    reg_TESTBUS_DBG_15_0 = ((reg_PIN_PU_PLL_RD_LANE == 1)? 0x8000 : 0) | ((reg_ANA_PLL_LOCK_RING_RD == 1)? 0x4000 : 0) | ringpll_dac_fine ;
	
	if(timeout) { 
		cmx_PLL_CAL_RING_PASS = 0; 
		ringpll_save(PWR); //ringpll_load(PWR);
	}
	else {
		ringpll_save(PWR);	
	}	
	...
	if(reg_ANA_PU_PLL_DLY_RING==0) {
		...
		//		if (((Ftarget_ring - Ftarget_code) <= Fmea) && (Fmea <= (Ftarget_ring + Ftarget_code))) {
                ...
                if((Fmea >= (Ftarget_ring-TARGET_RANGE)) && (Fmea <= (Ftarget_ring+TARGET_RANGE))) {
					...
					delay01(cmx_RINGPLL_TFINEDAC_7_0); //30); //6uSec settling time required after from second DAC_VALID toggle to new DAC value or SHRTR=0 
				}
                i++;
