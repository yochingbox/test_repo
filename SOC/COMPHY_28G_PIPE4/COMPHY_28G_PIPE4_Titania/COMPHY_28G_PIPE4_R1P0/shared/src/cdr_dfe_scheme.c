...
void dfe_fsm(uint16_t lpnum, uint8_t tap_en) BANKING_CTRL {
	...
	//if(dfe_stop==2) {
	//	reg_DFE_UPDATE_EN_LANE_15_0 &= 0x0001;
    //    reg_DFE_UPDATE_F_EN_LANE_5_0 = 0;
    //    reg_DFE_UPDATE_DC_EN_LANE = 0;
	//}	
...
void cal_half_code(void) BANKING_CTRL {
	...
	if(dec_ratio==0) {	
		reg_DFE_EVEN_ODD_SIM_ADAPT_LANE = 1; //reg_MCU_DEBUG_CMN_6_7_0; //1;
...
void dfe_sampler_interval_eq(void)  BANKING_CTRL{
    ...
    if(dfe_sampler_wa__delay_wa) {
        //delay WA 
        dfe_sampler_wa__delay_wa = false;
        switch(cmx_DFE_DC_INTERVAL_WA_START_DELAY_3_0) {
            case 1:
                //no delay, so no timer start
                break;

            case 2:
                timeout2_start((uint16_t)1000);
                return;
                break;

            case 3:
                timeout2_start((uint16_t)10000);
                return;
                break;

            default:
                timeout2_start((uint16_t)5000);
                return;
                break;
        }
    }
    #endif // #ifdef _PCIE_USB_BUILD
	...
	while(lpcnt-->0) {
		for(i= 0;i<4;i++){ 
			if(cmx_DFE_DC_INTERVAL_EQ_SLICE_ONLY && i<2) continue;
	...
	if(cmx_DFE_DC_INTERVAL_EQ_DURATION_SEL_1_0) {
		if(cmx_DFE_DC_INTERVAL_EQ_DURATION_SEL_1_0==1) {
			timeout2_start((uint16_t)5000); //refresh timer 5msec (1usec uint) 	
		}
		else if(cmx_DFE_DC_INTERVAL_EQ_DURATION_SEL_1_0==2) {
			timeout2_start((uint16_t)1000); //refresh timer 5msec (1usec uint) 	
		}
		else {
