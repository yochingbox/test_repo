...
void EOM_clock_analog_align(void) BANKING_CTRL {
	...
	if(reg_RX_SPEED_DIV_LANE_2_0>=0x6){
		dec_num = 1;
		total_cnt = 5000;
	}
	lnx_ESM_VOLTAGE_LANE_7_0 = 0;//get_ec_voltage();
	...
	if(reg_RX_SPEED_DIV_LANE_2_0>=0x6){
		if(cds28.eye_check_pass==1){
			...
			width = i;
			//reg_MCU_DEBUGF_LANE_7_0 = width&0xff;
			//move_eom_phase(i);// move back
			#if 1 // move-1-and-check
			for(i = 1; i < width; ++ i){
				move_eom_phase(1);
				cds_tb = cds_table[CDS_EYECHECK];
				cdr_dfe_scheme();
			...
			//reg_MCU_DEBUGA_LANE_7_0 = width&0xff;
			//reg_MCU_DEBUGB_LANE_7_0 = width>>8&0xff;
