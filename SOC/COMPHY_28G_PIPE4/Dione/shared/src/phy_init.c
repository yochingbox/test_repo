...
void init_reg(void) BANKING_CTRL {
	...
	if(phy_mode == SERDES ) {//Ethernet,
		...
		reg_VTH_RXIMPCAL_2_0 = 2; //Voltron Request : IPCE_COMPHY-1313
		lnx_MIDPOINT_SMALL_THRES_K_LANE_3_0 = 8; //Voltron Request : IPCE_COMPHY-1313
		lnx_MIDPOINT_LARGE_THRES_C_LANE_3_0 = 1; //Voltron Request : IPCE_COMPHY-1313
	...
	reg_DFE_STEP_ACCU_FX_LANE_3_0  = 0x3; //??0x06
    reg_DFE_STEP_ACCU_DC_LANE_3_0  = 0x3;
	reg_DFE_STEP_ACCU_F0_LANE_3_0  = 0x3;
	...
	//R2P1 default <- R1P0
	#ifdef _SATA_SERDES_BUILD
	reg_VTHVCOCAL_2_0 = 0;						//0 <- 1 <- 2 <- 3
	reg_VTHVCOPTAT_2_0 = 5;						//5 <- 6 <- 7 <- 5
	reg_CMN_IPP_SLLP_CUR_SEL_2_0 = 6;			//6 <- 4 <- 2 <- No register
	reg_SEL_IPP_IPTAT_SLLP_RING_1_0 = 0;        //0 <- 2 <- 3
	#else
	reg_VTHVCOCAL_2_0 = 1;						//1 <- 2 <- 3
	reg_VTHVCOPTAT_2_0 = 6;						//6 <- 7 <- 5	
	...
	reg_FFE_PW_SEL_LANE_1_0 = 2; //to improve Voltron A0 HTOL failure
...
void set_timer_cnt() BANKING_CTRL {
	...
	reg_TIMER_3_EN_LANE = 0;
	reg_TIMER3_CLK_SEL_LANE_1_0 = 0; //refclk
	reg_TIMER3_HI_CNT_LANE_15_0 = 1; 
	if(cmx_RXINIT_DONE_DELAY_15_0!=0)
		reg_TIMER3_LO_CNT_LANE_15_0 =  u16mul((cmx_REFCLK_FREQ_7_0>>1), cmx_RXINIT_DONE_DELAY_15_0+1);		
	#endif
}
