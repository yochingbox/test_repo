  ...
  {		
   ...
   if( reg_TX_TRAIN_ON_LANE || reg_PIN_PAPTA_TRAIN_ENABLE_RD_LANE) { 
	...
	remote_status_16 = reg_REMOTE_STATUS_FIELD_RD_LANE_15_0;
		...
		if( (do_train && reg_PIN_TX_TRAIN_ENABLE_MUX_RD_LANE==1) || (tx_apta_state==2 && reg_PIN_TX_TRAIN_ENABLE_MUX_RD_LANE==0 && reg_PIN_PAPTA_TRAIN_ENABLE_RD_LANE==1)) {
...
ext_int:
		...
		{		
			#ifdef SUPPORT_APTA_TX_TRAIN
			if(tx_apta_state && cmx_APTA_LIMITED_STATUS_DIS==0) {
			...
			if( link_train_mode==0 && tx_apta_state==0) {
